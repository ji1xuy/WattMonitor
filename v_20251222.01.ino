#define VER "Program version 20251222.01"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  v_20251222.01 httpResponseの整形をする前にresponse code 503を先に処理する
  最大130048バイトのフラッシュメモリのうち、スケッチが126486バイト（97%）を使っています。
  グローバル変数は5482バイトのRAMを使用しています。
*/
// **************************************************************************************************
...
void printFileViewWindow(int fileNum) {
  ...
  myGLCD.setColor(VGA_GREEN);
  sprintf(buf, "Path: %s", fileNameList[fileNum]->path);
...
bool deleteFiles(int fileNum) {
  ...
  if (fileNum == FILENUM_DELETE_ALLCSVFILES) {
    ...
    //パスが長い場合に短縮する
    //char path[23] = "...";
    //if (strlen(fileNameList[fileNum]->path) > 22) {
    //  strcat(path, fileNameList[fileNum]->path + 19);
    //} else {
