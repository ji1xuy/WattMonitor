#define VER "Program version 20251021.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
const struct BTN btn[] = {
  ...
  // SD CARD
  { "PAGE", "UP", 255, 7, 311, 33 },
  { "CURSOR", "UP", 255, 40, 311, 66 },
  { "CURSOR", "DOWN", 255, 73, 311, 99 },
  { "PAGE", "DOWN", 255, 106, 311, 133 },
...
void config(void) {
  ...
  const char *NAME_AUTO_RESET_DATE = "Auto reset date";
  const char *FORMAT_02D = "%02d";
  const char *NAME_VOLTAGE = "Voltage";
  const char *FORMAT_3DP = "%3d%%";
  const char *NAME_SENSER1 = "Sensor1";
  const char *FORMAT_3DV = "%3dV";
  const char *NAME_SENSER2 = "Sensor2";
  const char *NAME_SENSER3 = "Sensor3";
  const char *NAME_BAR_GRAPH_FULLSCALE = "Bar graph fullscale";
  const char *FORMAT_2DKW = "%2dkW";
  const char *NAME_SCREEN_OFF_TIMER = "Screen off timer";
  const char *FORMAT_2DMIN = "%2dmin";
  const char *NAME_SCREEN_OFF_MODE = "Screen off mode";
  const char *FORMAT_5S = "%5s";
  const char *NAME_SOUND = "Sound";
  const char *FORMAT_3S = "%3s";
  const char *NAME_AUTO_TIME_SYNC = "Auto time sync";
  const char *NAME_TIME_SYNC_HOUR = "Auto time sync hour";
  const char *NAME_TIME_SYNC_MIN = "Auto time sync min";
  ...
  for (index = 0; index < EEPROM_MODE_MAX; index++) {
    switch (mode[index]) {
      ...
      case EEPROM_CT1_PERCENTAGE:
        ...
        item[EEPROM_CT1_PERCENTAGE].format = FORMAT_3DP;
      ...
      case EEPROM_CT2_PERCENTAGE:
        ...
        item[EEPROM_CT2_PERCENTAGE].format = FORMAT_3DP;
      ...
      case EEPROM_CT3_PERCENTAGE:
        ...
        item[EEPROM_CT3_PERCENTAGE].format = FORMAT_3DP;
      ...
      case EEPROM_BAR_GRAPH_FULLSCALE:
        //Bar graph FS
        y += ROW_HEIGHT;
        item[EEPROM_BAR_GRAPH_FULLSCALE].name = NAME_BAR_GRAPH_FULLSCALE;
        ...
        item[EEPROM_BAR_GRAPH_FULLSCALE].format = FORMAT_2DKW;
      ...
      case EEPROM_AUTO_TIME_SYNC:
        //Auto time sync
        y += ROW_HEIGHT;
        item[EEPROM_AUTO_TIME_SYNC].name = NAME_AUTO_TIME_SYNC;
...
void getFileName(File dir, String path, int level) {
  ...
  while (true) {
    ...
    if (entry.isDirectory()) {
      ...
      numberOfFiles++;
      fileUsage = fileUsage + (uint32_t)entry.size();
    }
    entry.close();
  }
}
