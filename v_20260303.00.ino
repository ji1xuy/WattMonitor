#define VER "Program version 20260303.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
void syncTimeWithServerOnceADay(void) {
  char event[MAX_EVENT_LEN];
  int prevScreenMode;
    ...
    if ((t.date != syncDay) && (t.hr == eepromTimeSyncHour) && (t.min == eepromTimeSyncMin)) {
      ...
      if (isFlashAirResetRequested) {
        if (resetFlashAir() == false) {
          sprintf(event, "SYSTEM: [%04lXh]FlashAir reset failed", nextSequenceId - 1);
          saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
          return false;
        }
      }
      if (syncTimeWithServer() == true) {
