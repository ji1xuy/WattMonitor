#define VER "Program version 20260104.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  最大130048バイトのフラッシュメモリのうち、スケッチが127424バイト（97%）を使っています。
  グローバル変数は5520バイトのRAMを使用しています。
*/
// **************************************************************************************************
...
const int MAX_FORMATTEDNUMBER_LEN = 14;
// ファイル一覧作成のための変数
struct FILENAMELIST {
  int num;         // 通し番号(1～)
...
char *truncateString(char *str, int maxLen) {
  ...
  if (len > maxLen) {
    ...
    strcat(tmpStr, str + 3 + (len - maxLen));
    strcpy(str, tmpStr);
  }
  return str;
}
...
void sdcardButtonPress(void) {
  ...
  printFileNameListWindow(fileNum);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (popupWindowCloseTimer == POPUPWINDOWCLOSETIMER_END) {
      break;
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_CURSORUP, x, y)) {
        ...
        if (fileNum > 0) {
          changeCursol(fileNum, fileNum - 1);
          fileNum--;
        } else {
          changeCursol(fileNum, numberOfFiles - 1);
...
void printFileViewWindow(int fileNum) {
  ...
  myGLCD.setColor(VGA_GREEN);
  sprintf(buf, "Path: %s", truncateString(fileNameList[fileNum]->path, 32));
...
void getFileName(File dir, String path, int level) {
  File entry;
  String fullPath = "";
  char buf[FULLSIZE_STR_LEN];
  int strsize;
  ...
  while (true) {
    ...
    strsize = strlen(entry.name()) + 1;
    fileNameList[numberOfFiles]->name = new char[strsize];
    strcpy(fileNameList[numberOfFiles]->name, entry.name());
    if (entry.isDirectory()) {
      fileNameList[numberOfFiles]->attribute = 'D';
      fileNameList[numberOfFiles]->size = 0LU;
      numberOfFiles++;
      getFileName(entry, fullPath, level + 1);
    } else {
      fileNameList[numberOfFiles]->attribute = 'F';
      fileNameList[numberOfFiles]->size = entry.size();  // バイト単位
      numberOfFiles++;
      fileUsage += (uint32_t)entry.size();
    }
    entry.close();
  }
}
...
void printFileName(int fileNum, int x, int y) {
  const int MAX_INDENT = 5;
  char name[3 + MAX_INDENT + MAX_FILENAME_LEN];
  char size[11];
  ...
  if (fileNum < numberOfFiles) {
    sprintf(name, "%03d %s%s", fileNameList[fileNum]->num, 
      indent[min(fileNameList[fileNum]->level, MAX_INDENT)], 
      fileNameList[fileNum]->name);
    if ( fileNameList[fileNum]->attribute == 'D') {
      strcpy(size, "<dir>");
    } else {
...
void printFileNameList(
  int fileNum) {  // 引数 fileNum ファイルの通し番号(0～)
  ...
  for (int i = 0; i < 10; i++) {
    ...
    printFileName(j, X_LEFT, Y_TOP + i * fontYsize);
  }
  // 現在ページ / 総ページ
  myGLCD.setColor(VGA_WHITE);
  ...
  myGLCD.print(buf, X_LEFT + fontXsize * 10, Y_TOP + fontYsize * 11);
  strcpy_P(buf, PSTR("Free:  "));
  if (volumesize != 0LU) {
    sprintf(buf + strlen(buf), "%13skB", formatWithCommas(formattedNumber, volumesize - fileUsage / 1024LU));
    } else {
    strcat(buf, "?????????????kB");
  }
  myGLCD.print(buf, X_LEFT + fontXsize * 10, Y_TOP + fontYsize * 12);

  myGLCD.setColor(VGA_BLACK);
  myGLCD.setBackColor(getFileNameColor(fileNum));
  printFileName(fileNum, X_LEFT, Y_TOP + fontYsize * row);
}

void printFileNameListWindow(int fileNum) {
  printWindowFrame(0, 0, 319, 239);
  printFileNameList(fileNum);
  printButtonEx(BTN_SDCARD_PAGEUP);
  printButtonEx(BTN_SDCARD_CURSORUP);
...
void changeCursol(int currFileNum, int nextFileNum) {
  int currPage, nextPage;
  int currRow, nextRow;
  const int X_LEFT = 8;
  const int Y_TOP = 14;
  ...
  myGLCD.setBackColor(VGA_BLACK);
  printFileName(currFileNum, X_LEFT, Y_TOP + currRow * fontYsize);
  myGLCD.setColor(VGA_BLACK);
  myGLCD.setBackColor(getFileNameColor(nextFileNum));
  printFileName(nextFileNum, X_LEFT, Y_TOP + nextRow * fontYsize);
}
...
bool deleteFile(int fileNum) {
  ...
  if (strlen(fileNameList[fileNum]->path) > 79) {
    sprintf(event, "SDCARD: \'%s\' Pathname too long", truncateString(fileNameList[fileNum]->path, 79));
    saveEventToSdCard(event);
    return false;
  }
