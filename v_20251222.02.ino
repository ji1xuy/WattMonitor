#define VER "Program version 20251222.02"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  最大130048バイトのフラッシュメモリのうち、スケッチが126632バイト（97%）を使っています。
  グローバル変数は5514バイトのRAMを使用しています。
*/
// **************************************************************************************************
...
const char FILENAME_HTTP_RESPONSE[] = "HTTP_RES.TXT";
...
bool syncTimeWithServer(bool isSystemResetEnabled) {
  ...
  for (int i = 1; i <= MAX_REQUEST_TIMES; i++) {
    httpResponse[0] = '\0';
...
unsigned long printFileChar(File file, unsigned long filePos, bool isPrintEnabled) {
  const int X_LEFT = 8;
  const int Y_TOP = 2;
  char ch;
  ...
  if (strlen(buf) > 38) {
    if (strlen(buf) < 42) {
      sprintf(buf, "Page: %03d (Read: %sB / %skB)", page, formattedFilePos, 
        formatWithCommas(formattedFileSize, (unsigned long)(file.size() / 1024LU)));
    } else if (strlen(buf) < 45) {
      sprintf(buf, "Page: %03d (Read: %sB / %sMB)", page, formattedFilePos, 
        formatWithCommas(formattedFileSize, (unsigned long)(file.size() / 1048576LU)));
    } else if (strlen(buf) < 48) {
      sprintf(buf, "Page: %03d (Read: %skB / %sMB)", page, 
        formatWithCommas(formattedFilePos, (unsigned long)(filePos / 1024LU)),
        formatWithCommas(formattedFileSize, (unsigned long)(file.size() / 1048576LU)));
    } else {
      sprintf(buf, "Page: %03d (Read: %sMB / %sMB)", page, 
        formatWithCommas(formattedFilePos, (unsigned long)(filePos / 1048576LU)),
        formatWithCommas(formattedFileSize, (unsigned long)(file.size() / 1048576LU)));
    }
  }
...
void getFileName(File dir, String path, int level) {
  ...
  while (true) {
    ...
    if (entry.isDirectory()) {
      ...
      fileNameList[numberOfFiles]->size = new char[6];
      strcpy(fileNameList[numberOfFiles]->size, "<dir>");
      numberOfFiles++;
      getFileName(entry, fullPath, level + 1);
    } else {
      fileNameList[numberOfFiles]->attribute = 'F';
      fileNameList[numberOfFiles]->size = new char[MAX_FILESIZE_LEN];
      sprintf(fileNameList[numberOfFiles]->size, "%10lu", entry.size());  // バイト単位
      numberOfFiles++;
...
void printFileName(int fileNum, int x, int y) {
  ...
  if (fileNum < numberOfFiles) {
    ...
    sprintf(buf, "%-20s%10s", name, fileNameList[fileNum]->size);
  } else {
    memset(buf, ' ', 30);
    buf[30] = '\0';
  }
  ...
  switch(fileNameList[fileNum]->attribute) {
    case 'F':
      if (strstr(fileNameList[fileNum]->name, ".CSV") != NULL) {
        color = VGA_YELLOW;
      } else if (strstr(fileNameList[fileNum]->name, FILENAME_HTTP_RESPONSE) != NULL) {
        color = VGA_FUCHSIA;
      } else {
        color = VGA_AQUA;
      }
...
bool deleteFiles(int fileNum) {
  ...
  int numberOfDeletionFailedFiles = 0;
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_DELETEFILE_OK, x, y)) {
        ...
        if (numberOfDeletionFailedFiles > 0) {
            printFileDeletionFailedWinodow(numberOfDeletionFailedFiles);
        }
        break;
      }
      // CANCEL
...
void printFileDeletionFailedWinodow(int numberOfDeletionFailedFiles) {
  int x, y;
  char buf[FULLSIZE_STR_LEN];
  ...
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  if (numberOfDeletionFailedFiles == 1) {
    myGLCD.print(F("CANNOT DELETE FILE (OR DIRECTORY)"), 24, 90);
  } else {
    myGLCD.print(F("CANNOT DELETE SOME FILES (OR DIRECTORIES)"), 24, 90);
  }
...
bool deleteFile(int fileNum) {
  ...
  if (fileNameList[fileNum]->attribute == 'F') {
    if (SD.remove(fileNameList[fileNum]->path) == true) {
      sprintf(event, "SDCARD: File \"%s\" deleted", fileNameList[fileNum]->path);
      saveEventToSdCard(event);
      return true;
    } else {
      sprintf(event, "SDCARD: File \"%s\" deletion failed", fileNameList[fileNum]->path);
      saveEventToSdCard(event);
    }
  }
  if (fileNameList[fileNum]->attribute == 'D') {
    if (SD.rmdir(fileNameList[fileNum]->path) == true) {
      sprintf(event, "SDCARD: Directory \"%s\" deleted", fileNameList[fileNum]->path);
      saveEventToSdCard(event);
      return true;
    } else {
      sprintf(event, "SDCARD: Directory \"%s\" deletion failed", fileNameList[fileNum]->path);
      saveEventToSdCard(event);
    }
  }
  return false;
...
void saveHttpResponseToSdCard(String httpResponse, String datetime) {
  ...
  file = SD.open(FILENAME_HTTP_RESPONSE, O_RDWR | O_CREAT | O_TRUNC);
  if (!file) {
    sdSts = SD_OPEN_ERROR;
    printSdStatus();
  } else {
