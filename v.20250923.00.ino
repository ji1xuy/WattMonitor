#define VER "Program version 20250923.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
//bool syncTime(int provider = DEFAULT_PROVIDER);
...
char syncTimeInfo[] = "--/-- --:-- ----h ";
// requestFlashAirReset()実行(FlashAIrリセット要求)時、syncTimeInfo[FLASHAIR_RESET_REQUESTED_MARK]を'R'にセット
// resetFlashAir()実行(FlashAirリセット処理)時、syncTimeInfo[FLASHAIR_RESET_REQUESTED_MARK]を' 'にセット
...
void requestFlashAirReset(void) {
  ...
  syncTimeInfo[FLASHAIR_RESET_REQUESTED_MARK] = 'R';
...
bool resetFlashAir(int sec) {
  ...
  memset(iSDIO_buf, 0, 512);
  nextSequenceId = 0;
  syncTimeInfo[FLASHAIR_RESET_REQUESTED_MARK] = ' ';
