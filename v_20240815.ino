#define VER "Program version 20240815"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
boolean findLine(String s, const char *str, char *val, size_t lineSize, int indexVal = -1);
...
boolean iSDIO_sendHttpRequest(uint32_t sequenceId, int protocol) {
  ...
  printEx(F("\n"));
  printEx(F("Waiting for a response"));
  ...
  if (card.writeExtDataPort(1, 1, 0x000, iSDIO_buf) == false) {
    printEx(F("writeExtDataPort command Failed"));
    return false;
  }
  printEx(F("writeExtDataPort command Succeeded"));
  return true;
}
...
boolean syncTime(void) {
  ...
  if (isTimeSynchronized == false) {
    ...
    isTimeSynchronized = findLine(httpResponse, " JST", datetime, sizeof(datetime), 0);
    if (isTimeSynchronized == false) {
      printEx(F("\nTime information not found"));
    } else {
