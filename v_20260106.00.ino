#define VER "Program version 20260106.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_DELETEFILE_ALLCSVFILES, x, y)) {
        ...
        sdSts = getFileNameList();
        indentOffset = maxIndentOffset = printFileNameListWindow(fileNum);
        continue;
      }
...
void printFileName(int fileNum, int x, int y, int indentOffeet) {
  ...
  if (fileNum < numberOfFiles) {
    //indentは0以下の値　マイナスになればなるほど表示が左にシフト
...
int printFileNameList(int fileNum, int indentOffset) {
  ...
  // 引数 indentOffset ファイルの字下げ位置を左にオフセット
...
int printFileNameListWindow(int fileNum, int indentOffset) {
  printWindowFrame(0, 0, 319, 239);
  indentOffset = printFileNameList(fileNum, indentOffset);
  printButtonEx(BTN_SDCARD_PAGEUP);
  printButtonEx(BTN_SDCARD_CURSORUP);
  printButtonEx(BTN_SDCARD_CURSORDOWN);
  printButtonEx(BTN_SDCARD_PAGEDOWN);
