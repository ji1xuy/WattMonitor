#define VER "Program version 20260103.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  v_20260103.00 SD CARD ファイルサイズ表示をカンマ付に修正 
  最大130048バイトのフラッシュメモリのうち、スケッチが127348バイト（97%）を使っています。
...
bool syncTimeWithServer(bool isSystemResetEnabled) {
  ...
  if (isTimeSynchronized == true) {
    ...
    if (isTimeSynchronized == true) {
      ...
      breakTime(t, tm);
      adjustClock(tmYearToCalendar(tm.Year), tm.Month, tm.Day, tm.Hour, tm.Minute, tm.Second);
    } else {
...
bool syncTimeWithServer(bool isSystemResetEnabled) {
  ...
  if (isTimeSynchronized == false) {
    ...
    if (isTimeSynchronized == false) {
      ...
      adjustClock(year, month, day, hour, minute, second);
    }
  }
  // 2025/2/11現在、正常処理ならず
...
char *truncateString(char *str, int maxLen) {
  ...
  int len = strlen(str);
  if (len > maxLen) {
    strcat(tmpStr, str + (len - maxLen) + 3);
    strcpy(str, tmpStr);
  }
  return str;
}
...
// ************************************************************************************
// その他
void waitSeconds(int waitingTime) {
  char buf[HALFSIZE_STR_LEN];
...
void adjustClock(int year, int month, int day, int hour, int minute, int second) {
