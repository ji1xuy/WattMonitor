...
/*
  ...
  v_20260102.01 パスの長いファイル、ディレクトリ削除 削除されるがフリーズ(SD.h ver.1.1.1)、リセット(SD.h ver.1.3.0)の不具合
   -> 暫定的に、パスが79文字を超えたら削除しない
  v_20260103.00 SD CARD ファイルサイズ表示をカンマ付に修正 
  最大130048バイトのフラッシュメモリのうち、スケッチが127006バイト（97%）を使っています。
  グローバル変数は5558バイトのRAMを使用しています。
*/
// **************************************************************************************************
...
int printMainWindow(int screenMode) {
  ...
  printButtonEx(BTN_SDCARD, STS_NORMAL);
...
void resetButtonPress(void) {
  ...
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_RESET_SYSTEMRESET, STS_NORMAL);
...
void systemReset(void) {
  ...
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_RESET_SYSTEMRESET_OK, STS_NORMAL);
  printButtonEx(BTN_RESET_SYSTEMRESET_CANCEL, STS_NORMAL);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (popupWindowCloseTimer == POPUPWINDOWCLOSETIMER_END) {
      break;
...
void dateTimeButtonPress(void) {
  ...
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_DATETIME_UP, STS_NORMAL);
  printButtonEx(BTN_DATETIME_MINUS, STS_NORMAL);
  printButtonEx(BTN_DATETIME_PLUS, STS_NORMAL);
  printButtonEx(BTN_DATETIME_DOWN, STS_NORMAL);
  if (existsFlashAir) {
    printButtonEx(BTN_DATETIME_SYNC, STS_NORMAL);
  }
  printButtonEx(BTN_DATETIME_OK, STS_NORMAL);
  printButtonEx(BTN_DATETIME_CANCEL, STS_NORMAL);
  mode = TIME_YEAR;
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (popupWindowCloseTimer == POPUPWINDOWCLOSETIMER_END) {
...
void configButtonPress(void) {
  ...
  printButtonEx(BTN_CONFIG_LOADDEFAULT, STS_NORMAL);
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_CONFIG_DOWN, x, y)) {
        ...
        printItem(item[mode[index]], mode[index], STS_NORMAL);
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_PAGEUP, x, y)) {
        ...
        printButtonEx(BTN_SDCARD_PAGEUP, STS_NORMAL);
        continue;
      }
      // CURSOR UP
      if (isButtonPressed(BTN_SDCARD_CURSORUP, x, y)) {
        ...
        printButtonEx(BTN_SDCARD_CURSORUP, STS_NORMAL);
        continue;
      }
      // CURSOR DOWN
      if (isButtonPressed(BTN_SDCARD_CURSORDOWN, x, y)) {
        ...
        printButtonEx(BTN_SDCARD_CURSORDOWN, STS_NORMAL);
        continue;
      }
      // PAGE DOWN
      if (isButtonPressed(BTN_SDCARD_PAGEDOWN, x, y)) {
        ...
        printFileNameList(fileNum);
        printButtonEx(BTN_SDCARD_PAGEDOWN, STS_NORMAL);
        continue;
      }
...
void printFileViewWindow(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGEUP, STS_NORMAL);
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGEDOWN, STS_NORMAL);
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGEEND, STS_NORMAL);
  printButtonEx(BTN_SDCARD_VIEWFILE_CANCEL, STS_NORMAL);
  myGLCD.setColor(VGA_GREEN);
...
void printFileNameListWindow(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_CURSORDOWN, STS_NORMAL);
  printButtonEx(BTN_SDCARD_PAGEDOWN, STS_NORMAL);
  printButtonEx(BTN_SDCARD_DELETEFILE_ALLCSVFILES, STS_NORMAL);
  printButtonEx(BTN_SDCARD_DELETEFILE, STS_NORMAL);
  printButtonEx(BTN_SDCARD_VIEWFILE, STS_NORMAL);
  printButtonEx(BTN_SDCARD_CANCEL, STS_NORMAL);
}
...
bool deleteFiles(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_DELETEFILE_OK, STS_NORMAL);
  printButtonEx(BTN_SDCARD_DELETEFILE_CANCEL, STS_NORMAL);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (offTimerInterval.check() == 1) {
      if (popupWindowCloseTimer < POPUPWINDOWCLOSETIMER_END) {
...
void printUndeletedFilesWinodow(void) {
  ...
  printButtonEx(BTN_SDCARD_DELETEFILE_CLOSE, STS_NORMAL);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (offTimerInterval.check() == 1) {
      if (popupWindowCloseTimer < POPUPWINDOWCLOSETIMER_END) {
