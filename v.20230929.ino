...
/*
  ...
  2023/9/27 saveWattHourTimeperiod()関数他をupdateScreenInterval.check()==1の時に実行するように場所移動
  2023/9/28 修正したmemorysaver.hをいつのまにかまた使用していたため2023/6/6の不具合再発。memorysaver.hを戻す
...
// 汎用の文字列格納用バッファのサイズ
const int BUFFER_LEN_FULL = 41; // 1行の最大の文字数　320 ÷ 8(SmallFontの文字幅) + '\0' = 41
const int BUFFER_LEN_HALF = 21; // 1行の半分の文字数　160 ÷ 8(SmallFontの文字幅) + '\0' = 21
...
//Time-master.zip
#include <TimeLib.h>
char currTime[6] = "\0"; //"hh:mm" + '\0'
char prevTime[6] = "\0"; //"hh:mm" + '\0'
enum {
  TIME_YEAR,
  TIME_MONTH,
  TIME_DAY,
...
int popupWindowOffTimer; // popupWindow自動キャンセル用加算タイマー
...
void printStringExEx(char* s1, char* s2, unsigned int digit, int x, int y, int font, unsigned long c) {
  ...
  switch (font) {
    ...
    */
    ...
    default: // 対象の定数がない場合でもFONT_SMALL_FONTで表示させる
      myGLCD.setFont(SmallFont);
      fontSizeX = 8;
      break;
  }
...
void setScreenMode(void) {
  ...
  if (hasBlackout) {
    ...
    if (screenOffTimerInBlackout == SCREENOFFTIMERINBLACKOUT_END) {
      ...
      //無限ループを避けるため、ここでSCREENOFFTIMERINBLACKOUT_STOPにはしない
      //画面OFF
  ...
  for (index = 0; index < EEPROM_MODE_MAX; index++) {
    switch (mode[index]) {
      ...
      case EEPROM_BAR_GRAPH_FULLSCALE:
        //Bar graph FS
        y += ROW_HEIGHT;
        strcpy(item[EEPROM_BAR_GRAPH_FULLSCALE].name, "Bar graph Fullscale");
...
void saveWattHourTimeperiod(void) {
  ...
  if ((t.date == eepromAutoResetDate) && (strcmp(currTime, "00:00") == 0)) {
    if (needsSaveIntegratedValues) {
      ...
      saveEventToSdCard("SYSTEM: Integrated values reset");
...
void setup() {
  ...
  value = EEPROM.read(EEPROM_VT_PERCENTAGE); if ((value >= 1) && (value <= 254)) eepromVtPercentage = value;
  value = EEPROM.read(EEPROM_CT1_PERCENTAGE); if ((value >= 1) && (value <= 254)) eepromCtPercentage[1] = value;
  value = EEPROM.read(EEPROM_CT2_PERCENTAGE); if ((value >= 1) && (value <= 254)) eepromCtPercentage[2] = value;
  value = EEPROM.read(EEPROM_CT3_PERCENTAGE); if ((value >= 1) && (value <= 254)) eepromCtPercentage[3] = value;
  elapsedDays = 1;
  resetIntegratedValues();
...
void loop() {
  ...
  //saveWattHourTimeperiod()関数の中てSDカードへの書き込みが実行されない不具合発生
  //文字列を表示するコードを関数内に追加すると不具合が発生しない 
  //短時間に実行を繰り返すことが原因とすると、文字列表示によるウエイトが掛かってうまく動作した？
