#define VER "Program version 20231005"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  2023/10/3 saveWattHourTimeperiod()関数 wattHourTimeperiod変数が99999kWhを超えたら積算値を強制的にリセットする機能を追加
    スケッチが115244バイト（88%）、グローバル変数は5453バイト使用
  2023/10/4 config()関数 Screen off timer の設定値に単位(min)も表示
  2023/10/5 setup()関数 電源ONからFLASHAIRの初期化開始までの待ち時間を追加
...
const int BUFFER_LEN_EVENT = 100; //99文字 + '\0' = 100
  ...
  for (index = 0; index < EEPROM_MODE_MAX; index++) {
    switch (mode[index]) {
      ...
      case EEPROM_SCREEN_OFF_TIMER:
        ...
        item[EEPROM_SCREEN_OFF_TIMER].format = "%02dmin";
...
#ifdef FLASH_AIR
  //FlashAirの無線LAN機能の起動
  delay(5000); //電源ONから初期化開始までの待ち時間がないと初期化に失敗する
  //clearScreen();
