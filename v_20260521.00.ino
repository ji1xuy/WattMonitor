#define VER "Program version 20260521.00"
//**************************************************************************************************
/*
  電力モニター WattMonitor
  ■主要部品
  ...
  ■動作確認済みIDE (1.8.5)(1.8.13)(1.8.19)(2.2.1)(2.3.2)(2.3.7)
  ...
  2022/4/15 printFileNameList()関数 32GBのSDカードの空き容量が正しく表示されないのを修正
  ...
  2023/6/8 printFileNameList()関数 ファイル数が10で割り切れる場合に、存在しないファイルが1行余計に表示される不具合を修正
  2023/6/11 定数をenumで定義
  2023/6/16 sdStsMsg[]変数の初期化部分を修正
  2023/6/18 beep()関数 BEEP_HIGH_WITHOUTTIMERの場合、sound off時に800msのdelayを入れる
  2023/6/25 popupWidow表示中に一定時間が経過すると閉じるようにタイマー(popupWindowCloseTimer)を追加
  ...
  2023/11/16 needsResetFlashAir変数の名前をisFlashAirResetRequestedに変更
  2023/11/16 syncTimeInfo[]内の'R'を書き込む位置を直接数値ではなく定数FLASHAIR_RESET_REQUESTED_MARKに変更
  ...
  2025/2/7 API Ninjas(HTTPS)から時刻が取得できないためWorld Clock APIを追加
    ...
    https://seesaawiki.jp/flashair-dev/d/HTTPS/TLS%be%da%cc%c0%bd%f1%cc%e4%c2%eaisTimeSyncronizing
  ...
  画面消灯からタッチで復帰した時に画面真っ白
    ...
    2025/6/17 loop()関数 割り込み中は極力他の処理をしないよう修正
  2025/6/18 loop()関数 修正
  2025/6/18 setup()関数 prevSec,prevDate,elapsedDaysの初期化処理を追加
  2025/6/19 saveWattHourTimeperiodToSdCard()関数 sprintf文を削除してchar buf[20];とprintStringEx文を不具合対策として残す
  30分積算データがSDカードに自動保存されない不具合発生
    2025/6/20 saveWattHourTimeperiodToSdCard()関数 sprintf文を再追加
  時計モード中に画面真っ白
    2025/6/21 割り込み中は極力他の処理をしないよう修正
  v.20250621.00にて不具合発生せず
  v.20250622.00 saveWattHourTimeperiodToSdCard()関数 不具合対策4行のうち,printStringEx文をコメント化
  v.20250622.01 loop()関数 修正
  v.20250622.02 noInterrupts() → Timer1.detachInterrupt()、interrupts() → Timer1.attachInterrupt(Timer1)に修正
  v.20250622.02 getClockTime()関数 引数を追加し割り込み中にはdetachInterrupt(),attachInterrupt()を実行しないように修正
  v.20250622.03 timer1()t関数 getClockTime()の位置を変更
  v.20250622.03 saveWattHourTimeperiodToSdCard()関数 不具合対策4行のうち,myGLCD.setFont(SmallFont)文を追加コメント化
  v.20250623.00 loop()関数を修正
  v.20250623.00 (不具合対策と無関係) FlashAirオフオンの間隔のデフォルト値を5秒から10秒に修正
  v.20250624.00 saveWattHourTimeperiodToSdCard()関数 不具合対策4行のうち,sprintf文を追加コメント化
  -> 不具合発生せず
  CLOCKモードに設定変更できない
    v.20250624.01 EEPROM書き込み時に割り込み禁止にする
    v.20250626.00 EEPROM書き込み時に割り込みが終わるのを待つ
    v.20250627.00 ENABLE_MODE_CLOCKが未定義だと、MODE_CLOCKが無効化されその分のメモリが節約されるように修正したものをもとに戻す
  -> 不具合発生せず スケッチの修正は無関係? ICソケットの接触不良の可能性あり
  v.20250627.00 30分データ保存されず -> saveWattHourTimeperiodToSdCard()関数 不具合対策4行のうち,sprintf文をアンコメント化
  -> 不具合発生せず　char buf[20]; と sprintf(buf, "%s %s", prevTime, currTime); は効果あり
  v.20250703.00 elapsedDays起動直後2日になる -> resetIntegratedValues()関数　修正
  v.20250703.00 elapsedDaysが加算されない -> resetIntegratedValues()関数　修正
  タッチスクリーンが反応しなくなる
  -> LCDからコネクタを一旦外して付け直すと直る
   v.20250705.00～v.20250705.01 割り込み中のif文 割り込み禁止にしてif文の条件式をbool値に代入
  config()関数を起動してから一度でも実行すると、30分積算データがSDカードに自動保存されないかもしれない
  config()関数を起動してから一度でも実行すると、screenOffModeをCLOCKにしても反映されないのかもしれない
  v_20250706.00～v.20250713.00 screenOffModeをCLOCKにしても反映されない不具合 -> FORMAT_2DKW変数宣言時の*漏れ　修正
  他、多数修正
  v_20250713.01～v.20250718.00 30分積算データがSDカードに自動保存されない不具合の修正他
  　-> タイマー割り込みとDS1302への時刻取得処理が競合した場合に、DS1302の返す値が異常になるのが原因と思われる。
  　Time型の変数tをグローバルにして、getClockTime()関数をloop()関数内で1ループ1回のみ実行するように修正。
  　各関数でtの値を使いまわししてDS1302へのアクセスを最低限にした。
  v_20250719.00 timer1()関数の中でもgetClockTime()を実行するよう修正
  v_20250720.00 isInterrupting変数にvolatile修飾子が漏れていたため追加、OFF,ON,MODE_OFF,MODE_CLOCK定数をenumからconst byte型に変更
  v_20250721.00 一部のボタンのbeep音を変更
  v_20250721.01 loop()関数 printBlackout()がいつの間にか抜けていたため追加
  v_20250722.00 loop()関数 updateTarget = UPDATE_DATAとすべきところをupdateTarget++としていたのを修正
  v_20250722.01 printBlackOut()関数 復電持の画面乱れ対策
  v_20250723.00 ファイル削除処理を修正
  v_20250806.00 ファイル削除処理を修正
  v_20250810.00 ファイル削除処理を修正
  v_20250810.01 saveWattHourTimeperiodToSdCard(),resetIntegratedValues()関数　自動リセット日の00:00からリセットを繰り返す不具合を修正
  v_20250811.00 incrElapsedDays()関数  if (t.date == eepromAutoResetDate) { に && (t.hr == 0) && (t.min == 0) を追加
  v_20250811.01 sdCard()関数 fileNameList変数のdelete処理を追加
  v_20250812.00 sdCard()関数 fileNameList変数のdelete処理を修正
  v_20250812.01 deleteFileNameList()関数 delete[] fileNameList;を追加　
  v_20250813.00 syncTime()関数 isTimeSynchronizedがtrueの場合を先に処理するよう修正　
  v_20250815.00 一部の変数名を変更
  v_20250816.00 一部の変数名を変更
  v_20250816.01 一部のメッセージを変更
  v_20250830.00 clockMode切り替えの機能を削除
  v_20250906.00 getClockTime()関数 時刻が進まない不具合を修正、一部の変数名を変更
  v_20250907.00 一部の変数名を変更、CalendarToTmYear(year)マクロを削除
  v_20250911.00 initFlashAir(), onOffFlashAir()関数 戻り値を返すように修正
  v_20250913.00 ルート以外のファイルも削除できるように修正
  v_20250917.00 ディレクトリ階層がわかるように修正
  v_20250917.01 ファイルと ディレクトリを色分けするように修正
  v_20250918.00 ファイルと ディレクトリの表示を修正
  v_20250919.00 ファイルと ディレクトリの表示を修正、DELETE ALL時のdeleteFiles()関数の戻り値を修正
  v_20250920.00 ファイルと ディレクトリの表示を修正、deleteFiles()関数の戻り値を修正
  v_20250920.01 printFileNames()関数 変数の宣言を修正
  v_20250921.00 getFileNameColor()関数 表示色を修正
  v_20250921.01 deleteFile()関数 ディレクトリも削除可能に修正
  v_20250921.02 deleteFile()関数 削除できなかった場合にメッセージを表示するよう修正
  v_20250922.00 deleteFile()関数他 削除できなかった場合のメッセージを修正
  v_20250923.00 ディレクトリ階層がルートをレベル0としてレベル5(5以上は"####"表示)までわかるように修正
  v_20251004.00 変数名を修正　adjustTime -> syncTimeWithServer
  v_20251012.00 SDカード内のファイルの内容を表示する機能を追加
  v_20251013.00 実数値を変数化 fontXsize, fontYsize
  v_20251014.00 FILE VIEW 印字不可の文字は水色のピリオド表示に変更
  v_20251016.00 FILE VIEW '\n'は何もしないように修正
  v_20251016.01 setup()関数 SD.begin()の前に5秒待機、その分FlashAirの初期化前の10秒を5秒に変更
  v_20251016.02 loop()関数 if文の条件見直し。t = getClockTime();の実行タイミングを変更
  v_20251017.00 printFileViewWindow()関数 現在位置から最終ページまでのバイト数が多い場合は[Reading...]を表示する
  v_20251019.00 SC Card初期化待ち時間、FlashAIr初期化待ち時間の変更、syncTimeWithServerOnceADay()関数の修正
  v_20251019.01 FashAIr初期化失敗時の処理を変更
  v_20251021.00 printFileViewWindow()関数 [Reading...]表示に読込バイト数を追加
  v_20251102.00 loop()関数 Heap, Stackの空き領域を表示追加、printFileNameListWindow()関数修正
  v_20251104.00 CONFIGを開いて閉じて、SD CARDを開いてFILE VIEWを実行してメインの画面に戻ると、動作異常になる
   -> 一部の文字列の格納先をSRAMからFLASHメモリーに移動
  v_20251116.00 BTN_SDCARD_VIEWFILE_CLOSEをBTN_SDCARD_VIEWFILE_CANCELに、btn[BTN_SDCARD_VIEWFILE_CANCEL].name1を"CANCEL"に修正
  v_20251121.00 VIEW FILEで特定のファイル(一定以上のサイズ?)を開いてCANCELで戻ると最終行のファイルサイズ表示が異常になる
   -> さらに一部の文字列の格納先をSRAMからFLASHメモリーに移動
  v_20251123.00 メモリーの空き容量を割り込みで表示、LCDの左上をタッチで表示のオンオフを制御
  v_20251124.00 メモリーの空き容量表示のオンオフ制御を改善
  v_20251129.00 メモリーの空き容量拡大のため、FILENAMELIST構造体の文字列のメンバ変数を固定サイズから必要サイズを動的確保に変更
  v_20251201.00 FILENAMELIST構造体の文字列のメンバ変数の領域確保を malloc, free から new, delete に変更
  v_20251201.01 画面描画が乱れるため、タッチ判定処理の中にのみメモリーの空き容量表示を実行
  v_20251202.00 ネットから時刻取得中の間は、メモリーの空き容量表示を割り込み処理の中で実行
  v_20251206.00 clearScreen()関数 画面消去途中に割り込みが入ると消去が中断するのでdelay()を追加、一部の変数名を修正
  v_20251206.01 FILE VIEWの各ページのfile.position()の格納をfilePos[]変数から双方向リストに変更
  v_20251206.01 clearScreen()関数 delay()をやめて画面消去中は割り込み禁止に修正
  v_20251207.00 メモリーの空き容量表示の位置を右上に変更
  v_20251213.00 割り込み中のメモリーの空き容量表示判定を、isPrintFreeMemoryEnabledInInterruption == trueからcurrScreenMode == SCREEN_SETUPに変更
  v_20251215.00 FILE VIEW を実行すると動作が異常になる -> 双方向リストの作り方が間違っていたため修正
  v_20251216.00 initFilePosList()関数 list変数の解放漏れを修正、双方向リストに使う変数名を修正
  v_20251220.00 PAGE DOWN, PAGE END 処理中に空きメモリーが500バイト未満になったら処理を中断するようにする
  v_20251222.00 DELETE FILE OKの次の画面でOKボタンを押した時、ボタンの色を変える
  v_20251222.00 全て画面で、CANCELボタンを押した時、ボタンの色を変える
  v_20251222.01 httpResponseの整形をする前にstatus code 503を先に処理する
  v_20251222.02 FILE VIEW Path: 表示　パスが長い場合に短縮する
  v_20260102.01 パスの長いファイル、ディレクトリ削除 削除されるがフリーズ(SD.h ver.1.1.1)、リセット(SD.h ver.1.3.0)する
   -> 原因わからず。暫定的に、パスが79文字を超えたら削除しない -> SFNのパス名の長さの制限による
  v_20260104.00 SD CARD ファイルサイズ表示をカンマ付に修正
  v_20260104.01 SD CARD ファイル階層表現を改善する (ツリー構造に合わせてファイル名表示の字下げ位置をoffsetする)
  v_20260106.00 SD CARD ファイル階層表現を改善する (画面の左右をタッチすることでoffsetをずらせるようにする)
  v_20260108.00 SD CARD ファイル階層表現を改善する 不具合修正
  v_20260109.00 SD CARD ファイル階層表現を改善する 不具合修正
  v_20260110.00 SD CARD getFileName()関数 処理中にメモリー不足の場合の対処、
    isOutofMemory()関数 isEnoughMemory()に名前変更(戻り値の論理が逆に)、引数(デフォルト値あり)を追加
  v_20260201.00 時刻同期結果の情報(syncTimeWithServerInfo変数)の表示を変更
  最大130048バイトのフラッシュメモリのうち、スケッチが129504バイト（99%）を使っています。
  グローバル変数は5509バイトのRAMを使用しています。
  v_20260303.00 resetFlashAir()関数　引数の渡し方が間違っていたため、第1引数と第2引数の順序を入れ替え
  　(✕第1引数を省略して(できないのに省略したつもりになって)、第2引数に(実際は第1引数として渡される)値を指定)
  v_20260418.00 停電して復電後タッチスクリーンが無反応になったので、復電時LCDの初期化を実施
  v_20260424.00 syncTimeWithServerOnceADay()関数 FlashAirのリセットに失敗した時に画面が正しく再描画されないのを修正
  v_20260425.00 turnOnScreen()関数 LCDの初期化処理を追加
  最大130048バイトのフラッシュメモリのうち、スケッチが129288バイト（99%）を使っています。
  グローバル変数は5479バイトのRAMを使用しています。
  v_20260520.00 isEnoughMemory()関数 変数名訂正 requiredMmemory→requiredMemory
  v_20260521.00 LCDの初期化処理をturnOnScreen()関数から削除してinitScreen()関数として分離
  最大130048バイトのフラッシュメモリのうち、スケッチが129312バイト（99%）を使っています。
  グローバル変数は5479バイトのRAMを使用しています。
*/
// **************************************************************************************************
...
//時刻を取得するサーバの選択
#define PROVIDER_WORLD_CLOCK_API  //使用可
//#define PROVIDER_API_NINJAS     //使用不可 Process Failed C0
//#define PROVIDER_TIME_API       //使用不可 Process Failed C0
//#define PROVIDER_WORLD_TIME_API //使用可
// 汎用の文字列格納用バッファのサイズ
const int FULLSIZE_STR_LEN = 41;  // LCD画面1行の最大の文字数　320 ÷ 8(SmallFontの文字幅) + '\0' = 41
const int HALFSIZE_STR_LEN = 21;  // LCD画面1行の半分の文字数　160 ÷ 8(SmallFontの文字幅) + '\0' = 21
// SDカードに保存するイベント情報格納用バッファのサイズ
const int MAX_EVENT_LEN = 151;  //150文字 + '\0'
...
// SCREEN OFF時、時計を表示 MODE_CLOCK or 表示しない MODE_OFF
const byte DEFAULT_SCREEN_OFF_MODE = MODE_CLOCK;
...
volatile int popupWindowCloseTimer;  // popupWindow自動キャンセル用加算タイマー
...
const int POPUPWINDOWCLOSETIMER_START = 0;
const int POPUPWINDOWCLOSETIMER_END = 2;
...
volatile bool isInterrupting = false;
volatile byte timer1_status;           //状態制御用変数
const byte TIMER1_STS_CALCULATE = 0;   //計測・計算中
const byte TIMER1_STS_CALCULATED = 1;  //計測・計算終了
const byte TIMER1_STS_IDLE = 2;        //画面更新可能
const byte TIMER1_STS_PRINT = 3;       //画面更新中
...
/*  ピン配置
  ...
  11 DB5                        12 DB6
  13 DB7                        14 DB8  -> 14 (D0)
  15 DB9  -> 15 (D1)            16 DB10 -> 16 (D2)
  17 DB11 -> 17 (D3)            18 DB12 ->  1 (D4)
  19 DB13 ->  2 (D5)            20 DB14 ->  3 (D6)
  21 DB15 ->  4 (D7)            22 SDCS
  23 BL   -> 2SC1815 Corrector  24 VDD  -> VDD
  25 VDD                        26 GND  -> GND
  27 GND                        28 nc
  29 MISO -> 23 (D24)           30 MOSI -> 24 (D23)
  31 PEN  -> 22 (D22)           32 F_CS
  33 T_CS -> 25 (D25)           34 CLK  -> 26 (D26)
*/
//DB8～DB15はD0～D7に固定
...
enum {
  ...
  SCREEN_OFF,   // 画面非表示状態/非表示指示(printMainWindow(SCREEN_OFF)を実行することでバックライトのLEDをオフにして非表示にする)
...
// printStringExEx()関係
const unsigned long VGA_NO_CHANGE = 0x10000;
enum {
  FONT_NO_CHANGE,
  FONT_SMALL_FONT,
  FONT_BIG_FONT,
...
enum {
  STS_NORMAL,  // 非選択
  STS_INVERSE  // 選択(STS_NORMALの色を反転)
};
// ボタン番号
enum {
  BTN_NOT_PRESSED, //定数のみの用途
  BTN_PRESSED,     //定数のみの用途
  BTN_SCREEN_ON,   //定数のみの用途
  BTN_RESET,
  BTN_DATE_TIME,
  BTN_CONFIG,
  BTN_SDCARD,
  BTN_SCREEN_OFF,
  BTN_RESET_SYSTEMRESET,
  BTN_RESET_OK,
  BTN_RESET_CANCEL,
  BTN_RESET_SYSTEMRESET_OK,
  BTN_RESET_SYSTEMRESET_CANCEL,
  BTN_DATETIME_UP,
  BTN_DATETIME_DOWN,
  BTN_DATETIME_PLUS,
  BTN_DATETIME_MINUS,
  ...
  BTN_CONFIG_PLUS,
  BTN_CONFIG_MINUS,
  BTN_CONFIG_LOADDEFAULT,
  BTN_CONFIG_OK,
  BTN_CONFIG_CANCEL,
  BTN_SDCARD_PAGEUP,
  BTN_SDCARD_CURSORUP,
  BTN_SDCARD_CURSORDOWN,
  BTN_SDCARD_PAGEDOWN,
  BTN_SDCARD_DELETEFILE_ALLCSVFILES,
  BTN_SDCARD_DELETEFILE,
  BTN_SDCARD_VIEWFILE,
  BTN_SDCARD_CANCEL,
  BTN_SDCARD_DELETEFILE_OK,
  BTN_SDCARD_DELETEFILE_CANCEL,
  BTN_SDCARD_DELETEFILE_CLOSE,
  BTN_SDCARD_VIEWFILE_PAGEUP,
  BTN_SDCARD_VIEWFILE_PAGEDOWN,
  BTN_SDCARD_VIEWFILE_PAGEEND,
  BTN_SDCARD_VIEWFILE_CANCEL,
  BTN_PRINTFREEMEMORY, //printFreeMemoryタッチ判定用　ボタンの描画なし
  BTN_SDCARD_OFFSETLEFT, //OFFSET LEFTタッチ判定用　ボタンの描画なし
  BTN_SDCARD_OFFSETRIGHT, //OFFSET RIGHTタッチ判定用　ボタンの描画なし
};
struct BTN {
  ...
  int left;       //ボタンの左上のx座標
  int top;        //ボタンの左上のy座標
  int right;      //ボタンの右下のx座標
  int bottom;     //ボタンの右下のy座標
};
const struct BTN btn[] = {
  ...
  // SD CARD
  { "PAGE", "UP", 253, 17, 313, 45 },
  { "CURSOR", "UP", 253, 54, 313, 82 },
  { "CURSOR", "DOWN", 253, 91, 313, 119 },
  { "PAGE", "DOWN", 253, 128, 313, 156 },
  { "DELETE", "ALL CSV", 31, 203, 95, 231 },
  { "DELETE", "FILE", 103, 203, 167, 231 },
  { "VIEW", "FILE", 175, 203, 239, 231 },
  { "CANCEL", "", 247, 203, 311, 231 },
  // DELETE FILE
  { "OK", "", 144, 124, 208, 152 },
  { "CANCEL", "", 216, 124, 280, 152 },
  { "CLOSE", "", 216, 124, 280, 152 },
  // VIEW FILE
  { "PAGE", "UP", 31, 203, 95, 231 },
  { "PAGE", "DOWN", 103, 203, 167, 231 },
  { "PAGE", "END", 175, 203, 239, 231 },
  { "CANCEL", "", 247, 203, 311, 231 },
  { "", "", 249, 0, 319, 35 },
  { "", "", 8, 14, 88, 154 },
  { "", "", 168, 14, 248, 154 },
};
bool isPrintFreeMemoryEnabled = false;
int fontXsize, fontYsize;

//デフォルト引数のある関数の宣言
void printStringExEx(char *s1, char *s2, unsigned int digit, int x, int y, int font, unsigned long color = VGA_NO_CHANGE);
void printBarGraphFrame(int screenMode = SCREEN_CURRENT);
void printBarGraph(int screenMode = SCREEN_CURRENT);
void printSyncTimeWithServerInfo(int screenMode = SCREEN_CURRENT);
...
void printWindowFrame(int left, int top, int right, int bottom, unsigned long color = VGA_BLACK);
void printButtonEx(int btnNum, int sts = STS_NORMAL);
void printItem(struct ITEM item, int mode, int sts = STS_NORMAL);
void printlnEx(String s = "");
void printTempTime(Time tt, int m, int status = STS_NORMAL);
...
// 積算値リセット関係
char resetTime[HALFSIZE_STR_LEN];
int elapsedDays;                            //リセット日からの経過日数+1
bool needsSaveIntegratedValues = false;  // trueの時、積算値をリセットする
// 必要な文字のみ表示更新するための変数
...
char prevDateTime[21];
char prevClockHour[3];
char prevClockMin[3];
char prevClockDate[11];
volatile bool hasBlackout = false;
volatile bool hasBrownout = false;
...
const int MAX_DATASTRING_LEN = 501;  // SDカードへ１回に書き込むデータの最大サイズ(500文字 + '\0'(1文字))
// (SDカードの状態)
int sdSts;
//SDカードの状態を表す定数　※ここを変更する場合は、この下のsdStsMsg配列も変更すること
enum {
  SD_SUCCESS = 0,
  SD_CARD_PRESENT = 0,  // SDカードが挿入されている
...
// (ファイル書き込み関係)
char sdLastUpdatedTime[HALFSIZE_STR_LEN];
const int MAX_FILENAME_LEN = 13;        // ファイル名 8+3形式(12桁)+'\0'
const int MAX_FILESIZE_LEN = 11;        // long型変数の最大桁数(10桁)+'\0'
const int MAX_FILEVIEW_PAGE = 990;      // 1ページ当たり10ファイル * 99ページ=990ファイル
const int MAX_FORMATTEDNUMBER_LEN = 14; // 最大値 "4,294,967,295" 13桁

//引用元
//https://elm-chan.org/docs/fat.html#lfn
//FATファイルシステムのしくみと操作法
//短いファイル名
...
const int MAX_PATH_LEN = 79;

// ファイル一覧作成のための変数
struct FILENAMELIST {
  int num;    // 通し番号(1～)
  int level;  // 階層の深さ(0:ルート, 1以上:サブディレクトリ)
  char attr;  // 属性('F':ファイル、'D':ディレクトリ)
  char *path;
  char *name;
  unsigned long size;
};
struct FILENAMELIST **fileNameList; // SD内のファイル情報を格納する変数
int numberOfFiles;                  // SD内のファイル数
uint32_t fileUsage;                 // SD内のファイルサイズの合計
const int FILENUM_DELETE_ALLCSVFILES = -1;  // SD CARD画面で[DELETE ALL]を押した時にdeleteFile()関数に渡す引数(任意の負数)
struct FILEPOSLIST {
  struct FILEPOSLIST *next, *prev;
  unsigned long value;
};
...
const char EVENT_CSV[] = "EVENT.CSV";
const char RESET_CSV[] = "RESET.CSV";
//デフォルト引数のある関数の宣言
void saveEventToSdCard(String event, int screenMode = SCREEN_CURRENT, int printMode = PRINTMODE_WINDOW);
void saveDataToSdCard(int screenMode = SCREEN_CURRENT);
...
int printFileNameList(int fileNum, int indentOffset = 1);
int printFileNameListWindow(int fileNum, int indentOffset = 1);
...
const char HTTP_RESPONSE_TXT[] = "HTTP_RES.TXT";
bool isTimeSynchronized = false;
bool existsFlashAir = false;
char syncTimeWithServerInfo[] = "--/--/-- --:--[----h] ";
// requestFlashAirReset()実行(FlashAIrリセット要求)時、syncTimeWithServerInfo[FLASHAIR_RESET_REQUESTED_MARK]を'R'にセット
// resetFlashAir()実行(FlashAirリセット処理)時、syncTimeWithServerInfo[FLASHAIR_RESET_REQUESTED_MARK]を' 'にセット
const int FLASHAIR_RESET_REQUESTED_MARK = 21;
bool isFlashAirResetRequested;  // trueの時、次の時刻取得の際にFlashAirをリセットする
...
char flashAirIP[16];
//デフォルト引数のある関数の宣言
bool syncTimeWithServer(bool isSystemResetEnabled = false);
bool onOffFlashAir(int sec = 10);
bool initFlashAir(bool isSystemResetEnabled = false);
bool resetFlashAir(bool isSystemResetEnabled = false, int sec = 10);
bool iSDIO_sendHttpRequest(uint32_t sequenceId, int protocol = PROTOCOL_HTTP);
...
// 文字列操作関数
//デフォルト引数のある関数の宣言
//strの中にkeywordが含まれる行が見つかったら、その行をlineに格納する
//戻り値　true:見つかった, false:見つからなかった
//keywordIndex -1:keywordの位置を指定しない(省略可)、0以上:keywordの位置を指定
bool findLineIncludedKeyword(String str, const char *keyword, char *line, size_t lineSize, int keywordIndex = -1);

// ************************************************************************************
// FLASH AIR制御関数
bool iSDIO_status() {
  char buf[HALFSIZE_STR_LEN];
...
bool iSDIO_sendHttpRequest(uint32_t sequenceId, int protocol) {
  char buf[HALFSIZE_STR_LEN];
...
bool iSDIO_waitHttpResponse(uint32_t sequenceId) {
  char buf[HALFSIZE_STR_LEN];

  printEx(F("Waiting HTTP response"));
  uint8_t prev = 0xFF;
  for (int i = 0; i < 20; ++i) {
...
bool iSDIO_readHttpResponse(char* httpResponse, int httpReponseSize) {
  ...
  if (findLineIncludedKeyword(httpResponse, "HTTP/1.1", responseCode, sizeof(responseCode)) == false) {
...
bool syncTimeWithServer(bool isSystemResetEnabled) {
  char buf[HALFSIZE_STR_LEN];
  ...
  tempScreenMode = currScreenMode;
  currScreenMode = SCREEN_SETUP;
  clearScreen();

  // http://worldclockapi.com/api/json/utc/now
  for (int requestCount = 1; requestCount <= MAX_REQUEST_COUNT; requestCount++) {
    httpResponse[0] = '\0';

    if (requestCount == 1) {
      strcpy_P(httpCommandArg1, PSTR("worldclockapi.com"));
      strcpy_P(httpCommandArg2, PSTR(
      ...
      delay(3000);
      if (resetFlashAir(isSystemResetEnabled) == false) {
  ...
  tm.Minute = 0;
  tm.Second = 0;
  if (isTimeSynchronized == true) {
    isTimeSynchronized = findLineIncludedKeyword(httpResponse, "Date:", datetime, sizeof(datetime), 0);
    if (isTimeSynchronized == true) {
      // 503 Service Unavailable, 502 Bad Gateway の場合の処理
      ...
      breakTime(t, tm);
      adjustRTC(tmYearToCalendar(tm.Year), tm.Month, tm.Day, tm.Hour, tm.Minute, tm.Second);
    } else {
      ...
      isTimeSynchronized = findLineIncludedKeyword(httpResponse, "\"currentFileTime\":", datetime, sizeof(datetime), 0);
      if (isTimeSynchronized == true) {
        // 200 OK の場合の処理
  ...
  sprintf(datetime, "%04d/%02d/%02d(%3s) %02d:%02d:%02d",
  ...
  sprintf(syncTimeWithServerInfo, "%02d/%02d/%02d %02d:%02d[%04lXh]", t.yr % 100, t.mon, t.date, t.hr, t.min, nextSequenceId - 1);

  currScreenMode = tempScreenMode;
  return isTimeSynchronized;
}
#endif
...
bool syncTimeWithServer(bool isSystemResetEnabled) {
  ...
  for (int requestCount = 1; requestCount <= MAX_REQUEST_COUNT; requestCount++) {
    if (requestCount == 1) {
      strcpy_P(httpCommandArg1, PSTR("api.api-ninjas.com"));
  ...
  if (isTimeSynchronized == false) {
    ...
    isTimeSynchronized = findLineIncludedKeyword(httpResponse, "\"datetime\":", datetime, sizeof(datetime), 0);
    if (isTimeSynchronized == false) {
      printlnEx(F("Time information not found"));
    } else {
      sscanf(datetime, "\"dateTime\": \"%04d-%02d-%02d %02d-%02d:%02d\"",
             &year, &month, &day, &hour, &minute, &second);
      adjustRTC(year, month, day, hour, minute, second);
    }
  }
  // 2025/2/11現在、正常処理ならず
  ...
  sprintf(syncTimeWithServerInfo, "%02d/%02d %02d:%02d %04lXh ", t.mon, t.date, t.hr, t.min, nextSequenceId - 1);
...
bool syncTimeWithServer(bool isSystemResetEnabled) {
  ...
  for (int requestCount = 1; requestCount <= MAX_REQUEST_COUNT; requestCount++) {
    if (requestCount == 1) {
      strcpy_P(httpCommandArg1, PSTR("www.timeapi.io"));
      strcpy_P(httpCommandArg2, PSTR(
...
bool syncTimeWithServer(bool isSystemResetEnabled) {
  ...
  for (int requestCount = 1; requestCount <= MAX_REQUEST_COUNT; requestCount++) {
    if (requestCount == 1) {
      strcpy_P(httpCommandArg1, PSTR("worldtimeapi.org"));
      strcpy_P(httpCommandArg2, PSTR(
...
bool syncTimeWithServerOnceADay(void) {
  char event[MAX_EVENT_LEN];
  int prevScreenMode;

  if ((hasBlackout) || (existsFlashAir == false) || (eepromAutoTimeSync == OFF)) {
    return  false;
  }
  ...
  turnOnScreen();
  sprintf(event, "SYSTEM: [%04lXh]", nextSequenceId - 1);
  if (isFlashAirResetRequested) {
    if (resetFlashAir() == false) {
      strcat(event, "FlashAir reset failed");
      saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
    }
  } else {
    isTimeSynchronized = syncTimeWithServer();
    if (isTimeSynchronized == true) {
      strcat(event, "Time synchronized");
    } else {
      strcat(event, "Time synchronization failed");
    }
    saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
  }
  initPrevClockValues();
  if (prevScreenMode == SCREEN_ON) {
    prevScreenMode = SCREEN_REDRAW;
  }
  currScreenMode = printMainWindow(prevScreenMode);
  return isTimeSynchronized;
}

bool onOffFlashAir(int sec) {
...
bool initFlashAir(bool isSystemResetEnabled) {
  // Initialize SD card.
  ...
  if (card.init(SPI_HALF_SPEED, PIN_SDCARD_CS)) {
    ...
    printlnEx(F("NG"));
    if (isSystemResetEnabled == false) {
      return false;
    }
    printEx(F("Touch the screen to reset the system"));
...
void requestFlashAirReset(void) {
  isFlashAirResetRequested = true;
  syncTimeWithServerInfo[FLASHAIR_RESET_REQUESTED_MARK] = 'R';
  //popupWindow表示中は画面表示に関わる処理はしない
  if (currScreenMode == SCREEN_POPUP) {
    return;
  }
  if (currScreenMode == SCREEN_ON) {
    printSyncTimeWithServerInfo();
  }
}

bool resetFlashAir(bool isSystemResetEnabled, int sec) {
  if (existsFlashAir == false) {
    return false;
  }
  onOffFlashAir(sec);
  memset(iSDIO_buf, 0, 512);
  nextSequenceId = 0;
  syncTimeWithServerInfo[FLASHAIR_RESET_REQUESTED_MARK] = ' ';
  isFlashAirResetRequested = false;
  if (initFlashAir(isSystemResetEnabled) == true) {
...
bool findLineIncludedKeyword(String str, const char *keyword, char *line, size_t lineSize, int keywordIndex) {
  unsigned int from = 0;
  int index, start, end;

  while (true) {
    index = str.indexOf(keyword, from);
    if (index == -1) {
      return false;
    }
    ...
    str.substring(start, end).toCharArray(line, lineSize);
    //keywordの位置を指定していない
    if (keywordIndex == -1) {
      return true;
    }
    //指定した位置にkeywordが見つかった
    if ((int)(strstr(line, keyword) - line) == keywordIndex) {
      return true;
    }
    //指定した位置にkeywordが見つからない
    //次を検索
    from = end;
  }
}
...
// 数字を3桁区切りの数字を表す文字列に変換
char *formatWithCommas(char *formattedNumber, unsigned long num) {
  String str = "";
  String numStr = String(num); // 数値を文字列に変換
  int len = numStr.length();

  for (int i = 0; i < len; i++) {
    str += numStr[i];
    // 3桁ごと、かつ最後尾でない場合にカンマを追加
    if ((len - 1 - i) % 3 == 0 && i != len - 1) {
      str += ',';
    }
  }
  str.toCharArray(formattedNumber, MAX_FORMATTEDNUMBER_LEN);
  return formattedNumber;
}

//文字列が長い場合に短縮する
char *truncateString(char *dstStr, const char *srcStr, int dstStrSize) {
  int srcLen = strlen(srcStr);
  int dstLen = dstStrSize - 1;

  if (srcLen > dstLen) {
    strcpy(dstStr, "...");
    strcat(dstStr, srcStr + 3 + (srcLen - dstLen));
  } else {
...
void waitSeconds(int sec) {
  ...
  sprintf(buf, "Waiting %d seconds", sec);
  printEx(buf);
  for (int i = 0; i < sec; i++) {
    printEx(F("."));
    delay(1000);
...
Time adjustIntlClock(int year, int month, int day, int hour, int minute, int second) {
  ...
  // tm構造体変数 -> システムの時刻を設定
  setTime(makeTime(tm));
  //HTTPレスポンスの読取開始からRTCへの時刻セットまでの処理に要する時間(秒未満切り上げ)を補正
...
void beep(int mode) {
  switch (mode) {
    ...
    case BEEP_PIPO:  // PC9800シリーズ起動音
      ...
      delay(150);
      break;
    case BEEP_HIGH_WITHOUTTIMER:  // タイマー不使用の TimerFreeTone() を使用
      if (eepromSound == ON) {
    ...
    case TIMER1_STS_IDLE:
      ...
      if (vrms[1] < 1.0) {
        if (hasBlackout == false) {
          ...
          if (currScreenMode == SCREEN_POPUP) {
            popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_END;
          }
          timer1_status = TIMER1_STS_CALCULATED;
          break;
        }
      } else if (vrms[1] < 80.0) {
      ...
      timer1_status = TIMER1_STS_CALCULATED;
      break;
    case TIMER1_STS_CALCULATED:
      if (isPrintFreeMemoryEnabled == true) {
        if (currScreenMode == SCREEN_SETUP) {
          printFreeMemory(278, 0);
        }
      }
    case TIMER1_STS_PRINT:
...
bool isButtonPressed(int btnNum, int x, int y) {
  if ((x >= btn[btnNum].left) && (x <= btn[btnNum].right) && (y >= btn[btnNum].top) && (y <= btn[btnNum].bottom)) {
    return true;
  }
  return false;
}
...
int getButtonNum(void) {
  ...
  if (isScreenTouched()) {
    ...
    if (isButtonPressed(BTN_SDCARD, x, y)) {
      return BTN_SDCARD;
    }
    if (isButtonPressed(BTN_SCREEN_OFF, x, y)) {
      return BTN_SCREEN_OFF;
    }
    if (isButtonPressed(BTN_PRINTFREEMEMORY, x, y)) {
      delay(300);
      while (getButtonNum() != BTN_NOT_PRESSED)
        ;
      return BTN_PRINTFREEMEMORY;
    }
    return BTN_PRESSED;
  }
...
int printFreeMemory(int x, int y) {
  int freeMemory;

  freeMemory = getFreeMemory();
  char buf[FULLSIZE_STR_LEN];
  sprintf(buf, "%5d", freeMemory);
  myGLCD.setFont(SmallFont);
  uint8_t* font = myGLCD.getFont();
  unsigned int color = myGLCD.getColor();
  unsigned int backColor = myGLCD.getBackColor();
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  myGLCD.drawRect(x, y, x + 41, y + 13);
  myGLCD.print(buf, x + 1, y + 1);
  myGLCD.setFont(font);
...
bool isEnoughMemory(int requiredMemory = 500) {
  if (getFreeMemory() < requiredMemory) {
    noInterrupts();
    beep(BEEP_LOW);
    printFreeMemory(278, 0);
    printWindowFrame(80, 82, 240, 138, VGA_YELLOW);
    myGLCD.setColor(VGA_WHITE);
    myGLCD.print("NOT ENOUGH MEMORY!", 88, 96);
    myGLCD.print("TOUCH HERE", 88, 110);
    while (getButtonNum() == BTN_NOT_PRESSED)
      ;
    beep(BEEP_HIGH);
    interrupts();
...
void printString(char *s, int x, int y, int status) {
...
// 桁、座標、文字サイズ、色を指定する
void printStringExEx(char *s1, char *s2, unsigned int digit, int x, int y, int font, unsigned long color) {
  char s[2];
  uint8_t *pFont;
  int fontSizeX;
...
void setScreenMode(void) {
  ...
  if (screenOffTimerInBlackout != SCREENOFFTIMERINBLACKOUT_STOP) {
    ...
    //復電後タッチスクリーンが無反応になる場合があるためLCDを初期化する
    //これで効果があるかはわからない
    //本当はLCDの電源を一定時間切ってから初期化したい
    initScreen();
...
int printMainWindow(int screenMode) {
  ...
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  myGLCD.print(F("Voltage"), 0, 18);
  myGLCD.print(F("No.1         A"), 0, 31);
  myGLCD.print(F("No.2         A"), 0, 44);
  myGLCD.print(F("No.3         A"), 0, 57);
  myGLCD.setColor(VGA_AQUA);
  myGLCD.print(F("No.1         Wh(30m)"), 0, 70);
  myGLCD.print(F("No.2         Wh(30m)"), 0, 83);
  myGLCD.print(F("No.3         Wh(30m)"), 0, 96);
  myGLCD.print(F("Total        Wh(30m)"), 0, 109);
  myGLCD.setColor(VGA_FUCHSIA);
  myGLCD.print(F("No.1       W(30m Pk)"), 0, 122);
  myGLCD.print(F("No.2       W(30m Pk)"), 0, 135);
  myGLCD.print(F("No.3       W(30m Pk)"), 0, 148);
  myGLCD.print(F("Total      W(30m Pk)"), 0, 161);
  // 積算電力量(時間帯別)
  myGLCD.setColor(VGA_BLACK);
  myGLCD.setBackColor(VGA_YELLOW);
  myGLCD.print(F("INTEGRATED VALUES"), 160, 18);
  myGLCD.setColor(VGA_YELLOW);
  myGLCD.setBackColor(VGA_BLACK);
  myGLCD.print(F("Since"), 160, 31);
  myGLCD.print(F(" 7-10            kWh"), 160, 44);
  myGLCD.print(F("10-17            kWh"), 160, 57);
  myGLCD.print(F("17-23            kWh"), 160, 70);
  myGLCD.print(F("23- 7            kWh"), 160, 83);
  myGLCD.print(F("Total kWh"), 160, 96);
  myGLCD.print(F("kWh/Day(Average)"), 184, 161);
  // バーグラフ
  printBarGraphFrame(screenMode);
  // 時刻（日本標準時）取得情報を表示
  printSyncTimeWithServerInfo(screenMode);
  // ファンクションボタン
  printButtonEx(BTN_RESET);
  printButtonEx(BTN_DATE_TIME);
  printButtonEx(BTN_CONFIG);
  printButtonEx(BTN_SDCARD);
  printButtonEx(BTN_SCREEN_OFF);
...
// SmallFont専用
void printButtonEx(int btnNum, int sts = STS_NORMAL) {
  printButton(btn[btnNum].name1, btn[btnNum].name2, btn[btnNum].left, btn[btnNum].top, btn[btnNum].right, btn[btnNum].bottom, sts);
}

//検証・コンパイル時警告発生のためs1, s2の型をchar*からconst char*に修正
...
void printButton(const char *s1, const char *s2, int left, int top, int right, int bottom, int sts) {
  int left1;  // s1の開始X座標
  int left2;  // s2の開始X座標
  ...
  myGLCD.fillRoundRect(left, top, right, bottom);
  if (sts == STS_NORMAL) {
  ...
  left1 = (left + right) / 2 - (strlen(s1) * 8) / 2 + 2;
  left2 = (left + right) / 2 - (strlen(s2) * 8) / 2 + 2;
  if (s2[0] == '\0') {
    myGLCD.print(s1, left1, top + 10);
  } else {
    myGLCD.print(s1, left1, top + 5);
    myGLCD.print(s2, left2, top + 15);
  }
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
}

void printWindowFrame(int left, int top, int right, int bottom, unsigned long color) {
  myGLCD.setColor(color);
  myGLCD.fillRoundRect(left, top, right, bottom);
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  myGLCD.drawRoundRect(left, top, right, bottom);
}
...
void printDateTime(void) {
  ...
  strcpy(prevDateTime, buf);
}

void printSyncTimeWithServerInfo(int screenMode) {
  char buf[FULLSIZE_STR_LEN];
  int x, y;
  ...
  myGLCD.setFont(SmallFont);
  sprintf(buf, "Time sync %s", syncTimeWithServerInfo);
...
void printWattHourTimeperiod(int screenMode) {
  ...
  switch (screenMode) {
    case SCREEN_OFF:
      return;
    case SCREEN_CLOCK:
      printStringExEx(prevWattHourTimeperiod, buf, WATTHOURTIMEPERIOD_LEN, 176, 193, FONT_BIG_FONT);
      myGLCD.setFont(BigFont);
      printStringEx("kWh", 256, 193, VGA_YELLOW, VGA_BLACK);
...
void printSdStatus(int screenMode, int printMode) {
  ...
  if (popupWindowCloseTimer != POPUPWINDOWCLOSETIMER_END) {
    return;
  }
  if (screenMode == SCREEN_CURRENT) {
    screenMode = currScreenMode;
  ...
  x = x + 24 - 4;
  // 一旦ステータスを消去する
  myGLCD.print(sdStsMsg[SD_ERASE_SDSTSMSG], x, y);
  switch (sdSts) {
    case SD_SUCCESS:
...
void popupWindow(void) {
  ...
  switch (btnNum) {
    ...
    case BTN_SCREEN_ON:
      ...
      currScreenMode = printMainWindow(SCREEN_ON);
      break;
    case BTN_PRINTFREEMEMORY:
      beep(BEEP_HIGH);
      isPrintFreeMemoryEnabled = (isPrintFreeMemoryEnabled == false ? true : false);
      break;
    default:
      //何もしない
      break;
  }
  // メニューボタンを押して戻ってきたらSCREEN_OFF(またはSCREEN_CLOCK)にするタイマーをリセットする
...
void resetButtonPress(void) {
  ...
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_RESET_SYSTEMRESET);
  printButtonEx(BTN_RESET_OK);
  printButtonEx(BTN_RESET_CANCEL);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (popupWindowCloseTimer == POPUPWINDOWCLOSETIMER_END) {
      break;
    }
    if (offTimerInterval.check() == 1) {
      popupWindowCloseTimer++;
    }
    if (isScreenTouched()) {
      popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
      myTouch.read();
      ...
      // SYSTEM RESET
      if (isButtonPressed(BTN_RESET_SYSTEMRESET, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_RESET_SYSTEMRESET, STS_INVERSE);
        systemReset();
        break;
      }
      // OK
...
void systemReset(void) {
  ...
  printWindowFrame(24, 62, 296, 160, VGA_RED);
  currScreenMode = SCREEN_POPUP;
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  myGLCD.print("RESET THE SYSTEM?", 32, 76);
  myGLCD.print("IF YOU RESET, INTEGRATED VALUES", 32, 90);
  myGLCD.print("WILL ALSO BE RESET!", 32, 104);
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_RESET_SYSTEMRESET_OK);
  printButtonEx(BTN_RESET_SYSTEMRESET_CANCEL);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (popupWindowCloseTimer == POPUPWINDOWCLOSETIMER_END) {
      break;
    ...
    if (isScreenTouched()) {
      ...
      // OK
      if (isButtonPressed(BTN_RESET_SYSTEMRESET_OK, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_RESET_SYSTEMRESET_OK, STS_INVERSE);
        saveIntegratedValuesToSdCard(SCREEN_OFF);
        saveEventToSdCard(F("RESET : Integrated values reset"), SCREEN_OFF);
        saveEventToSdCard(F("RESET : System reset"), SCREEN_OFF);
        resetSystem();
        //ここへは到達しない
        break;
      }
      // CANCEL
      if (isButtonPressed(BTN_RESET_SYSTEMRESET_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        break;
      }
    }
...
void saveIntegratedValuesToSdCard(int screenMode) {
  ...
  strcpy(fileName, RESET_CSV);
  // ファイルがない場合は、タイトル行を書き込み
  if (!SD.exists(fileName)) {
    file = SD.open(fileName, FILE_WRITE);
    file.println(F("yyyy/mm/dd,hh:mm:ss,7-10(kWh),10-17(kWh),17-23(kWh),23-7(kWh)"));
    file.close();
  }
  // ここからデータ行を書き込み
  file = SD.open(fileName, FILE_WRITE);
...
void dateTimeButtonPress(void) {
  ...
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  myGLCD.print(F("Year"), 32, 34);
  myGLCD.print(F("Month"), 32, 48);
  myGLCD.print(F("Day"), 32, 62);
  myGLCD.print(F("Day of the week"), 32, 76);
  myGLCD.print(F("Hour"), 32, 90);
  myGLCD.print(F("Minute"), 32, 104);
  myGLCD.print(F("Second"), 32, 118);
  t.sec = 0;
  printTempTime(t, TIME_YEAR, STS_INVERSE);
  printTempTime(t, TIME_MONTH);
  printTempTime(t, TIME_DAY);
  printTempTime(t, TIME_DAYNAME);
  printTempTime(t, TIME_HOUR);
  printTempTime(t, TIME_MINUTE);
  printTempTime(t, TIME_SECOND);
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_DATETIME_UP);
  printButtonEx(BTN_DATETIME_MINUS);
  printButtonEx(BTN_DATETIME_PLUS);
  printButtonEx(BTN_DATETIME_DOWN);
  if (existsFlashAir) {
    printButtonEx(BTN_DATETIME_SYNC);
  }
  printButtonEx(BTN_DATETIME_OK);
  printButtonEx(BTN_DATETIME_CANCEL);
  mode = TIME_YEAR;
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (popupWindowCloseTimer == POPUPWINDOWCLOSETIMER_END) {
    ...
    if (isScreenTouched()) {
      ...
      // DOWN
      if (isButtonPressed(BTN_DATETIME_DOWN, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DATETIME_DOWN, STS_INVERSE);
        printTempTime(t, mode);
      ...
      // UP
      if (isButtonPressed(BTN_DATETIME_UP, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DATETIME_UP, STS_INVERSE);
        printTempTime(t, mode);
        mode = changeValue(mode, -1, 0, 6);
        printTempTime(t, mode, STS_INVERSE);
        printButtonEx(BTN_DATETIME_UP);
      ...
      // +
      if (isButtonPressed(BTN_DATETIME_PLUS, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DATETIME_PLUS, STS_INVERSE);
        switch (mode) {
          case TIME_YEAR:
      ...
      // -
      if (isButtonPressed(BTN_DATETIME_MINUS, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DATETIME_MINUS, STS_INVERSE);
        switch (mode) {
          case TIME_YEAR:
            t.yr = changeValue(t.yr, -1, 2000, 2099);
            break;
      ...
      // SYNC
      if (isButtonPressed(BTN_DATETIME_SYNC, x, y)) {
        if (existsFlashAir) {
          beep(BEEP_HIGH);
          printButtonEx(BTN_DATETIME_SYNC, STS_INVERSE);
          currScreenMode = SCREEN_SETUP;
          clearScreen();
          if (isFlashAirResetRequested) {
              resetFlashAir();
          ...
          isTimeSynchronized = syncTimeWithServer();
          sprintf(event, "SYNC  : [%04lXh]Time ", nextSequenceId - 1);
          if (isTimeSynchronized) {
            strcat(event, "synchronized");
          } else {
            strcat(event, "synchronization failed");
          }
          saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
          delay(3000);
        } else {
      ...
      // OK
      if (isButtonPressed(BTN_DATETIME_OK, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DATETIME_OK, STS_INVERSE);
        setClockTime(t);
        break;
      }
      // CANCEL
      if (isButtonPressed(BTN_DATETIME_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DATETIME_CANCEL, STS_INVERSE);
        break;
      }
    }
  }
...
void printTempTime(Time tt, int m, int sts) {
  char buf[HALFSIZE_STR_LEN];
...
void configButtonPress(void) {
  ...
  const int mode[EEPROM_MODE_MAX] = {
    EEPROM_AUTO_RESET_DATE,
    EEPROM_AUTO_TIME_SYNC, EEPROM_TIME_SYNC_HOUR, EEPROM_TIME_SYNC_MIN,
    EEPROM_VT_PERCENTAGE,
    EEPROM_CT1_VOLTAGE, EEPROM_CT1_PERCENTAGE,
    EEPROM_CT2_VOLTAGE, EEPROM_CT2_PERCENTAGE,
    EEPROM_CT3_VOLTAGE, EEPROM_CT3_PERCENTAGE,
    EEPROM_BAR_GRAPH_FULLSCALE,
    EEPROM_SCREEN_OFF_TIMER,
    EEPROM_SCREEN_OFF_MODE,
    EEPROM_SOUND,
  ...
  for (int index = 0; index < EEPROM_MODE_MAX; index++) {
    switch (mode[index]) {
      case EEPROM_AUTO_RESET_DATE:
        //Auto reset date
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_AUTO_RESET_DATE].name, PSTR("Auto reset date"));
        myGLCD.print(item[EEPROM_AUTO_RESET_DATE].name, COL_LEFT, y);
        item[EEPROM_AUTO_RESET_DATE].value = eepromAutoResetDate;
        strcpy_P(item[EEPROM_AUTO_RESET_DATE].format, PSTR("%02d"));
      ...
      case EEPROM_VT_PERCENTAGE:
        //Voltage
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_VT_PERCENTAGE].name, PSTR("Voltage"));
        sprintf(buf, "%s        100V", item[EEPROM_VT_PERCENTAGE].name);
        myGLCD.print(buf, COL_LEFT, y);
        item[EEPROM_VT_PERCENTAGE].value = eepromVtPercentage;
        strcpy_P(item[EEPROM_VT_PERCENTAGE].format, PSTR("%3d%%"));
      ...
      case EEPROM_CT2_VOLTAGE:
        //Sensor2
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_CT2_VOLTAGE].name, PSTR("Sensor2"));
        myGLCD.print(item[EEPROM_CT2_VOLTAGE].name, COL_LEFT, y);
        item[EEPROM_CT2_VOLTAGE].value = eepromCtVoltage[2];
        strcpy(item[EEPROM_CT2_VOLTAGE].format, item[EEPROM_CT1_VOLTAGE].format);
        item[EEPROM_CT2_VOLTAGE].x = COL_LEFT + COL_WIDTH * 15;
        item[EEPROM_CT2_VOLTAGE].y = y;
        break;
      case EEPROM_CT2_PERCENTAGE:
        strcpy(item[EEPROM_CT2_PERCENTAGE].name, item[EEPROM_CT2_VOLTAGE].name);
        item[EEPROM_CT2_PERCENTAGE].value = eepromCtPercentage[2];
        strcpy(item[EEPROM_CT2_PERCENTAGE].format, item[EEPROM_VT_PERCENTAGE].format);
      ...
      case EEPROM_CT3_VOLTAGE:
        //Sensor3
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_CT3_VOLTAGE].name, PSTR("Sensor3"));
        myGLCD.print(item[EEPROM_CT3_VOLTAGE].name, COL_LEFT, y);
        item[EEPROM_CT3_VOLTAGE].value = eepromCtVoltage[3];
        strcpy(item[EEPROM_CT3_VOLTAGE].format, item[EEPROM_CT1_VOLTAGE].format);
        item[EEPROM_CT3_VOLTAGE].x = COL_LEFT + COL_WIDTH * 15;
        item[EEPROM_CT3_VOLTAGE].y = y;
        break;
      case EEPROM_CT3_PERCENTAGE:
        strcpy(item[EEPROM_CT3_PERCENTAGE].name, item[EEPROM_CT3_VOLTAGE].name);
        item[EEPROM_CT3_PERCENTAGE].value = eepromCtPercentage[3];
        strcpy(item[EEPROM_CT3_PERCENTAGE].format, item[EEPROM_VT_PERCENTAGE].format);
      ...
      case EEPROM_BAR_GRAPH_FULLSCALE:
        //Bar graph FS
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_BAR_GRAPH_FULLSCALE].name, PSTR("Bar graph fullscale"));
        ...
        strcpy_P(item[EEPROM_BAR_GRAPH_FULLSCALE].format, PSTR("%2dkW"));
      ...
      case EEPROM_SCREEN_OFF_TIMER:
        //Screen off timer
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_SCREEN_OFF_TIMER].name, PSTR("Screen off timer"));
        myGLCD.print(item[EEPROM_SCREEN_OFF_TIMER].name, COL_LEFT, y);
        item[EEPROM_SCREEN_OFF_TIMER].value = eepromScreenOffTimer;
        strcpy_P(item[EEPROM_SCREEN_OFF_TIMER].format, PSTR("%2dmin"));
      ...
      case EEPROM_SCREEN_OFF_MODE:
        //Screen off mode
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_SCREEN_OFF_MODE].name, PSTR("Screen off mode"));
        myGLCD.print(item[EEPROM_SCREEN_OFF_MODE].name, COL_LEFT, y);
        item[EEPROM_SCREEN_OFF_MODE].value = eepromScreenOffMode;
        strcpy_P(item[EEPROM_SCREEN_OFF_MODE].format, PSTR("%5s"));
      ...
      case EEPROM_SOUND:
        //Sound
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_SOUND].name, PSTR("Sound"));
        myGLCD.print(item[EEPROM_SOUND].name, COL_LEFT, y);
        item[EEPROM_SOUND].value = eepromSound;
        strcpy_P(item[EEPROM_SOUND].format, PSTR("%3s"));
      ...
      case EEPROM_AUTO_TIME_SYNC:
        //Auto time sync
        y += ROW_HEIGHT;
        strcpy_P(item[EEPROM_AUTO_TIME_SYNC].name, PSTR("Auto time sync"));
        sprintf(buf, "%s       :", item[EEPROM_AUTO_TIME_SYNC].name);  //':'は時と分の間のコロン
        myGLCD.print(buf, COL_LEFT, y);
        item[EEPROM_AUTO_TIME_SYNC].value = eepromAutoTimeSync;
        strcpy_P(item[EEPROM_AUTO_TIME_SYNC].format, PSTR("%3s"));
        item[EEPROM_AUTO_TIME_SYNC].x = COL_LEFT + COL_WIDTH * 15;
        item[EEPROM_AUTO_TIME_SYNC].y = y;
        break;
      case EEPROM_TIME_SYNC_HOUR:
        strcpy_P(item[EEPROM_TIME_SYNC_HOUR].name, PSTR("Auto time sync hour"));
        item[EEPROM_TIME_SYNC_HOUR].value = eepromTimeSyncHour;
        strcpy_P(item[EEPROM_TIME_SYNC_HOUR].format, PSTR("%02d"));
        item[EEPROM_TIME_SYNC_HOUR].x = COL_LEFT + COL_WIDTH * 19;
        item[EEPROM_TIME_SYNC_HOUR].y = y;
        break;
      case EEPROM_TIME_SYNC_MIN:
        strcpy_P(item[EEPROM_TIME_SYNC_MIN].name, PSTR("Auto time sync min"));
        item[EEPROM_TIME_SYNC_MIN].value = eepromTimeSyncMin;
        strcpy_P(item[EEPROM_TIME_SYNC_MIN].format, PSTR("%02d"));
  ...
  //ボタンを表示
  printButtonEx(BTN_CONFIG_UP);
  printButtonEx(BTN_CONFIG_MINUS);
  printButtonEx(BTN_CONFIG_PLUS);
  printButtonEx(BTN_CONFIG_DOWN);
  printButtonEx(BTN_CONFIG_LOADDEFAULT);
  printButtonEx(BTN_CONFIG_OK);
  printButtonEx(BTN_CONFIG_CANCEL);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_CONFIG_DOWN, x, y)) {
        ...
        printItem(item[mode[index]], mode[index]);
        index = changeValue(index, 1, 0, EEPROM_MODE_MAX - 1);
        printItem(item[mode[index]], mode[index], STS_INVERSE);
        printButtonEx(BTN_CONFIG_DOWN);
        continue;
      }
      // UP
      ...
      if (isButtonPressed(BTN_CONFIG_PLUS, x, y)) {
        ...
        switch (mode[index]) {
          case EEPROM_SCREEN_OFF_TIMER:
            ...
            break;
          case EEPROM_SCREEN_OFF_MODE:
            item[mode[index]].value = changeValue(item[mode[index]].value, 1, MODE_OFF, MODE_CLOCK);
            break;
          case EEPROM_AUTO_RESET_DATE:
      ...
      // LOAD DEFAULT
      if (isButtonPressed(BTN_CONFIG_LOADDEFAULT, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_CONFIG_LOADDEFAULT, STS_INVERSE);
      ...
      if (isButtonPressed(BTN_CONFIG_OK, x, y)) {
        ...
        eepromAutoResetDate = writeValueToEEPROM(eepromAutoResetDate, item[EEPROM_AUTO_RESET_DATE], EEPROM_AUTO_RESET_DATE);
        eepromCtVoltage[1] = writeValueToEEPROM(eepromCtVoltage[1], item[EEPROM_CT1_VOLTAGE], EEPROM_CT1_VOLTAGE);
        eepromCtVoltage[2] = writeValueToEEPROM(eepromCtVoltage[2], item[EEPROM_CT2_VOLTAGE], EEPROM_CT2_VOLTAGE);
        eepromCtVoltage[3] = writeValueToEEPROM(eepromCtVoltage[3], item[EEPROM_CT3_VOLTAGE], EEPROM_CT3_VOLTAGE);
        eepromVtPercentage = writeValueToEEPROM(eepromVtPercentage, item[EEPROM_VT_PERCENTAGE], EEPROM_VT_PERCENTAGE);
        eepromCtPercentage[1] = writeValueToEEPROM(eepromCtPercentage[1], item[EEPROM_CT1_PERCENTAGE], EEPROM_CT1_PERCENTAGE);
        eepromCtPercentage[2] = writeValueToEEPROM(eepromCtPercentage[2], item[EEPROM_CT2_PERCENTAGE], EEPROM_CT2_PERCENTAGE);
        eepromCtPercentage[3] = writeValueToEEPROM(eepromCtPercentage[3], item[EEPROM_CT3_PERCENTAGE], EEPROM_CT3_PERCENTAGE);
        eepromSound = writeValueToEEPROM(eepromSound, item[EEPROM_SOUND], EEPROM_SOUND);
        eepromScreenOffTimer = writeValueToEEPROM(eepromScreenOffTimer, item[EEPROM_SCREEN_OFF_TIMER], EEPROM_SCREEN_OFF_TIMER);
        eepromScreenOffMode = writeValueToEEPROM(eepromScreenOffMode, item[EEPROM_SCREEN_OFF_MODE], EEPROM_SCREEN_OFF_MODE);
        eepromAutoTimeSync = writeValueToEEPROM(eepromAutoTimeSync, item[EEPROM_AUTO_TIME_SYNC], EEPROM_AUTO_TIME_SYNC);
        eepromTimeSyncHour = writeValueToEEPROM(eepromTimeSyncHour, item[EEPROM_TIME_SYNC_HOUR], EEPROM_TIME_SYNC_HOUR);
        eepromTimeSyncMin = writeValueToEEPROM(eepromTimeSyncMin, item[EEPROM_TIME_SYNC_MIN], EEPROM_TIME_SYNC_MIN);
        eepromBarGraphFullscale = writeValueToEEPROM(eepromBarGraphFullscale, item[EEPROM_BAR_GRAPH_FULLSCALE], EEPROM_BAR_GRAPH_FULLSCALE);
        break;
      }
      // CANCEL
      if (isButtonPressed(BTN_CONFIG_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_CONFIG_CANCEL, STS_INVERSE);
        break;
      }
    }
  }
...
// <param name="item">CONFIGの設定項目表示用のパラメータを保持するITEM構造体のポインタ</param>
...
byte writeValueToEEPROM(byte value, struct ITEM item, int mode) {
  ...
  if (item.value != value) {
    EEPROM.write(mode, item.value);
    formatValue(oldValue, item.format, value, mode);
    formatValue(newValue, item.format, item.value, mode);
    sprintf(event, "CONFIG: %s changed %s -> %s", item.name, oldValue, newValue);
    saveEventToSdCard(event);
  }
  return item.value;
}

void formatValue(char *str, char *format, byte value, int mode) {
  switch (mode) {
    case EEPROM_SCREEN_OFF_MODE:
      sprintf(str, format, (value == MODE_CLOCK) ? "Clock" : "Off");
      break;
    case EEPROM_SOUND:
    case EEPROM_AUTO_TIME_SYNC:
      sprintf(str, format, (value == ON) ? "ON" : "OFF");
      break;
    default:
...
void printItem(struct ITEM item, int mode, int sts) {
  ...
  formatValue(buf, item.format, item.value, mode);
  myGLCD.print(buf, item.x, item.y);
}

void printItems(struct ITEM item[], int mode[], int index) {
  if (index >= EEPROM_MODE_MAX) {
    return;
  }
  for (int i = 0; i < EEPROM_MODE_MAX; i++) {
    printItem(item[mode[i]], mode[i], (i == index) ? STS_INVERSE : STS_NORMAL);
  }
}

// ************************************************************************************
// SD CARDボタン押下
void sdcardButtonPress(void) {
  int x, y;
  int fileNum;
  int currPage, nextPage;
  int maxIndentOffset, indentOffset;

  printButtonEx(BTN_SDCARD, STS_INVERSE);
  currScreenMode = SCREEN_POPUP;
  numberOfFiles = 0;
  fileNum = 0;
  sdSts = getFileNameList();
  maxIndentOffset = printFileNameListWindow(fileNum);
  indentOffset = 0;
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (popupWindowCloseTimer == POPUPWINDOWCLOSETIMER_END) {
    ...
    if (isScreenTouched()) {
      ...
      // OFFSET LEFT
      if (isButtonPressed(BTN_SDCARD_OFFSETLEFT, x, y)) {
        if (indentOffset == maxIndentOffset) {
          continue;
        }
        beep(BEEP_HIGH);
        indentOffset--;
        printFileNameList(fileNum, indentOffset);
        continue;
      }
      // OFFSET RIGHT
      if (isButtonPressed(BTN_SDCARD_OFFSETRIGHT, x, y)) {
        if (indentOffset == 0) {
          continue;
        }
        beep(BEEP_HIGH);
      ...
      // PAGE UP
      if (isButtonPressed(BTN_SDCARD_PAGEUP, x, y)) {
        if (numberOfFiles <= 10) {
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_PAGEUP, STS_INVERSE);
        ...
        maxIndentOffset = printFileNameList(fileNum);
        indentOffset = 0;
        printButtonEx(BTN_SDCARD_PAGEUP);
        continue;
      }
      // CURSOR UP
      if (isButtonPressed(BTN_SDCARD_CURSORUP, x, y)) {
        if (numberOfFiles < 1) {
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_CURSORUP, STS_INVERSE);
        if (fileNum > 0) {
          moveCursor(fileNum, fileNum - 1, indentOffset);
          fileNum--;
        } else {
          moveCursor(fileNum, numberOfFiles - 1, indentOffset);
          fileNum = numberOfFiles - 1;
        }
        printButtonEx(BTN_SDCARD_CURSORUP);
        continue;
      }
      // CURSOR DOWN
      if (isButtonPressed(BTN_SDCARD_CURSORDOWN, x, y)) {
        if (numberOfFiles < 1) {
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_CURSORDOWN, STS_INVERSE);
        if ((fileNum + 1) < numberOfFiles) {
          moveCursor(fileNum, fileNum + 1, indentOffset);
          fileNum++;
        } else {
          moveCursor(fileNum, 0, indentOffset);
          fileNum = 0;
        }
        printButtonEx(BTN_SDCARD_CURSORDOWN);
        continue;
      }
      // PAGE DOWN
      if (isButtonPressed(BTN_SDCARD_PAGEDOWN, x, y)) {
        if (numberOfFiles <= 10) {
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_PAGEDOWN, STS_INVERSE);
        currPage = (int)(fileNum / 10.0) + 1;
        nextPage = (int)((numberOfFiles - 1) / 10.0) + 1;
        ...
        maxIndentOffset = printFileNameList(fileNum);
        indentOffset = 0;
        printButtonEx(BTN_SDCARD_PAGEDOWN);
        continue;
      }

      // DELETE ALL
      if (isButtonPressed(BTN_SDCARD_DELETEFILE_ALLCSVFILES, x, y)) {
        if (numberOfFiles < 1) {
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_DELETEFILE_ALLCSVFILES, STS_INVERSE);
        if (deleteFiles(FILENUM_DELETE_ALLCSVFILES) >= 1) {
          fileNum = 0;
        }
        sdSts = getFileNameList();
        popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
      ...
      // DELETE FILE
      if (isButtonPressed(BTN_SDCARD_DELETEFILE, x, y)) {
        if (numberOfFiles == 0) {
          continue;
        }
        beep(BEEP_HIGH);
        myGLCD.setColor(VGA_FUCHSIA);
        printButtonEx(BTN_SDCARD_DELETEFILE, STS_INVERSE);
        if (deleteFiles(fileNum) == 1) {
          if (fileNum > 0) {
            fileNum--;
          }
        }
      ...
      // VIEW FILE
      if (isButtonPressed(BTN_SDCARD_VIEWFILE, x, y)) {
        if (numberOfFiles == 0) {
          continue;
        }
        if (fileNameList[fileNum]->attr == 'D') {
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEWFILE, STS_INVERSE);
        printFileViewWindow(fileNum);
        printFileNameListWindow(fileNum, indentOffset);
        continue;
      }

      // CANCEL
      if (isButtonPressed(BTN_SDCARD_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_CANCEL, STS_INVERSE);
        break;
      }
    }
  }
...
unsigned long printFileChar(File file, unsigned long filePos, bool isPrintEnabled = true) {
  const int X_LEFT = 8;
  const int Y_TOP = 2;
  char ch;

  file.seek(filePos); //最初に読み込むべき文字の位置をセット
  curX = X_LEFT;
  curY = Y_TOP + fontYsize * 2;
  while (file.available()) {
    if (curX > (fontXsize * 38)) {
      curX = X_LEFT;
      curY += fontYsize;
    }
    if (curY > (fontYsize * 14)) {
      break;
    }
    ch = char(file.read());
    if (ch == '\r') {
    ...
    if (isPrintEnabled == true) {
      ...
      myGLCD.print(String(ch), curX, curY);
    }
    curX += fontXsize;
  }
  return file.position(); //次のページの最初に読み込むべき文字の位置
}

unsigned long printFileView(File file, unsigned long filePos, int page) {
  char buf[80];
  const int X_LEFT = 8;
  const int Y_TOP = 2;
  char formattedFilePos[MAX_FORMATTEDNUMBER_LEN];
  char formattedFileSize[MAX_FORMATTEDNUMBER_LEN];

  myGLCD.setColor(VGA_BLACK);
  myGLCD.fillRoundRect(X_LEFT, Y_TOP + fontYsize, 318, 202);
  myGLCD.setColor(VGA_WHITE);
  filePos = printFileChar(file, filePos);
  myGLCD.setColor(VGA_GREEN);
  formatWithCommas(formattedFilePos, filePos);
  formatWithCommas(formattedFileSize, file.size());
  sprintf(buf, "Page: %03d (Read: %sB / %sB)", page, formattedFilePos, formattedFileSize);
  if (strlen(buf) > 38) {
    if (strlen(buf) < 42) {
      sprintf(buf, "Page: %03d (Read: %sB / %skB)", page, formattedFilePos,
              formatWithCommas(formattedFileSize, file.size() / 1024LU));
    } else if (strlen(buf) < 45) {
      sprintf(buf, "Page: %03d (Read: %sB / %sMB)", page, formattedFilePos,
              formatWithCommas(formattedFileSize, file.size() / 1048576LU));
    } else if (strlen(buf) < 48) {
      sprintf(buf, "Page: %03d (Read: %skB / %sMB)", page,
              formatWithCommas(formattedFilePos, filePos / 1024LU),
              formatWithCommas(formattedFileSize, file.size() / 1048576LU));
    } else {
      sprintf(buf, "Page: %03d (Read: %sMB / %sMB)", page,
              formatWithCommas(formattedFilePos, filePos / 1048576LU),
              formatWithCommas(formattedFileSize, file.size() / 1048576LU));
    }
  }
  myGLCD.print(buf, X_LEFT, Y_TOP + fontYsize);
  return filePos;
}

//原形引用元
//https://hiroyukichishiro.com/list-in-c-language/
//【C言語】連結リストとは【片方向リスト，双方向リスト，双方向循環リスト
struct FILEPOSLIST *initFilePosList(unsigned long value) {
  struct FILEPOSLIST *list;

  list = new struct FILEPOSLIST;
  list->next = list->prev = NULL;
  list->value = value;
  return list;
}

struct FILEPOSLIST *addFilePosNode(struct FILEPOSLIST *list, unsigned long value) {
  struct FILEPOSLIST *node;
  struct FILEPOSLIST *prev_node = list;

  node = new struct FILEPOSLIST;
...
void deleteFilePosList(struct FILEPOSLIST *list) {
  struct FILEPOSLIST *node = list->next;
  struct FILEPOSLIST *next_node;
...
void printFileViewWindow(int fileNum) {
  File file;
  struct FILEPOSLIST *list, *node;
  int page = 1;
  char buf[FULLSIZE_STR_LEN];
  const int X_LEFT = 8;
  const int Y_TOP = 2;
  int x, y;
  char formattedNumber[MAX_FORMATTEDNUMBER_LEN];

  file = SD.open(fileNameList[fileNum]->path, FILE_READ);
  if (!file) {
    beep(BEEP_LOW);
    return;
  }
  printWindowFrame(0, 0, 319, 239);
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGEUP);
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGEDOWN);
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGEEND);
  printButtonEx(BTN_SDCARD_VIEWFILE_CANCEL);
  myGLCD.setColor(VGA_GREEN);
  char path[33];
  sprintf(buf, "Path: %s", truncateString(path, fileNameList[fileNum]->path, sizeof(path)));
  myGLCD.print(buf, X_LEFT, Y_TOP);

  // 1ページ目の開始位置(先頭文字のファイル内現在位置)をセット
  list = initFilePosList(0LU);
  // list->prev = NULL, list->value = 0LU, list->next = NULL
  node = list;
  // node->prev = NULL, node->value = 0LU, node->next = NULL

  // listは、最初のノードのポインタを保持
  // nodeは、表示しているページの開始位置と前後のノードを保持する変数
  // node->next == NULLの時、そのnodeは末尾のnode
  // nodeの各メンバー変数の値
  //  node   ->prev      ->value         ->next
  //  P.1      NULL 　　  P.1の開始位置   P.2のnode
  //  P.2      P.1のnode  P.2の開始位置   P.3のnode
  //  P.3      P.2のnode  P.3の開始位置   P.4のnode
  //  P.4      P.3のnode  P.4の開始位置   P.5のnode
  //  P.5      P.4のnode  P.5の開始位置   P.6のnode
  // ...
  // 最終     前P.のnode  最終P.の開始位置  NULL

  // printFileViewでnode(現在のノード)が指すページを表示し、
  // printFileViewの戻り値(次ページの開始位置)を次のノードを確保し格納
  addFilePosNode(list, printFileView(file, node->value, page));
  // node->next = 新たに確保したノード
  // node->next->prev = node, node->next->value = printFileViewの戻り値, node->next->next = NULL
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      // PAGE UP
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEUP, x, y)) {
        if (page == 1) {
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEUP, STS_INVERSE);
        page--;
        node = node->prev;
        printFileView(file, node->value, page);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEUP);
        continue;
      }

      // PAGE DOWN
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEDOWN, x, y)) {
        if (page == MAX_FILEVIEW_PAGE) {
          continue;
        }
        if (node->next->value >= (file.size() - 1LU)) {
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEDOWN, STS_INVERSE);
        if (node->next->next == NULL) {
          if (isEnoughMemory() == true) {
            addFilePosNode(list, printFileChar(file, node->next->value, false));
          }
          popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
        }
        if (node->next->next != NULL) {
      ...
      // PAGE END
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEEND, x, y)) {
        if (page == MAX_FILEVIEW_PAGE) {
          continue;
        }
        if (node->next->value >= (file.size() - 1LU)) {
        ...
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEEND, STS_INVERSE);
        bool isReading = false;
        if ((file.size() - node->next->value) > 10000LU) {
          printWindowFrame(80, 82, 240, 138);
          myGLCD.setColor(VGA_WHITE);
          myGLCD.print("Reading...", 128, 96);
          isReading = true;
        }
        while (node->next->value < (file.size() - 1LU)) {
          if (page == MAX_FILEVIEW_PAGE) {
            break;
          }
          if (node->next->next == NULL) {
          ...
          if (isReading == true)  {
            if (page % 10 == 0) {
              sprintf(buf, "%sB", formatWithCommas(formattedNumber, node->next->value));
              myGLCD.print(buf, 128, 110);
              if (isPrintFreeMemoryEnabled == true) {
                printFreeMemory(278, 0);
              }
            }
      ...
      // CANCEL
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEWFILE_CANCEL, STS_INVERSE);
        break;
      }
    }
  }

  deleteFilePosList(list);
  file.close();
}

void getNumberOfFiles(File file) {
  File nextFile;

  while (true) {
    nextFile = file.openNextFile();
    if (!nextFile) {
      return;
    }
    numberOfFiles++;
    getNumberOfFiles(nextFile);
    nextFile.close();
  }
}

void getFileName(File file, String path, int level) {
  File nextFile;
  String fullPath = "";
  char buf[FULLSIZE_STR_LEN];
  int strsize;
  ...
  while (true) {
    ...
    fileNameList[numberOfFiles] = new FILENAMELIST;
    fileNameList[numberOfFiles]->num = numberOfFiles + 1;
    fileNameList[numberOfFiles]->level = level;
    fullPath =  path + "/" + nextFile.name();
    strsize = fullPath.length() + 1;
    fileNameList[numberOfFiles]->path = new char[strsize];
    fullPath.toCharArray(fileNameList[numberOfFiles]->path, strsize);
    strsize = strlen(nextFile.name()) + 1;
    fileNameList[numberOfFiles]->name = new char[strsize];
    strcpy(fileNameList[numberOfFiles]->name, nextFile.name());
    if (nextFile.isDirectory()) {
      fileNameList[numberOfFiles]->attr = 'D';
      fileNameList[numberOfFiles]->size = 0LU;
      numberOfFiles++;
      getFileName(nextFile, fullPath, level + 1);
    } else {
      fileNameList[numberOfFiles]->attr = 'F';
      fileNameList[numberOfFiles]->size = nextFile.size();  // バイト単位
      numberOfFiles++;
      fileUsage += (uint32_t)nextFile.size();
    }
    nextFile.close();
  }
}

int getFileNameList(void) {
  File root;
  ...
  fileNameList = new FILENAMELIST*[numberOfFiles];
  fileUsage = 0LU;
  numberOfFiles = 0;
  root.rewindDirectory();  // openNextFile() が参照するすファイル位置をディレクトリの最初のファイルに戻す
  getFileName(root, "", 0);
...
void deleteFileNameList(void) {
  if (numberOfFiles == 0) {
    return;
  }
  for (int i = 0; i <  numberOfFiles; i++) {
    delete fileNameList[i]->path;
    delete fileNameList[i]->name;
...
void printFileName(int fileNum, int x, int y, int indentOffeet) {
  char name[20];
  char size[11];
  char buf[FULLSIZE_STR_LEN];
  char formattedNumber[MAX_FORMATTEDNUMBER_LEN];
  int indent;
  char leftAsterisk, rightAsterisk;

  if (fileNum < numberOfFiles) {
    //indent=0なら左端にファイル名を表示
    //プラスになればなるほど表示が右にずれる
    //マイナスになればなるほど表示が左にずれる
    indent = fileNameList[fileNum]->level + indentOffeet;
    //ファイル名の頭か切れる場合に*を表示
    leftAsterisk = indent < 0 ? '*' : ' ';
    sprintf(name, "%03d%c               ", fileNameList[fileNum]->num, leftAsterisk);
    rightAsterisk = '*';
    for (int i = 0; i < 13; i++) {
      if (fileNameList[fileNum]->name[i] == '\0') {
        rightAsterisk = ' ';
        break;
      }
      if ((indent + i) > 14) {
      ...
      if ((indent + i) >= 0) {
        name[4 + indent + i] = fileNameList[fileNum]->name[i];
      }
    }
    if (fileNameList[fileNum]->attr == 'D') {
      strcpy(size, "<dir>");
    } else {
      unsigned long number, fraction;
      if (fileNameList[fileNum]->size < 1023998977LU) { // 1023998976/1024 -> 999999 -> 999,999kB
        //kB未満を四捨五入
        number = fileNameList[fileNum]->size / 1024LU;
        fraction = fileNameList[fileNum]->size % 1024LU;
        if (fraction >= 512LU) {
          number += 1;
        }
        formatWithCommas(formattedNumber, number);
        sprintf(size, "%skB", formattedNumber);
      } else {
       //MB未満を四捨五入
        number = fileNameList[fileNum]->size / 1048576LU;
        fraction = fileNameList[fileNum]->size % 1048576LU;
        if (fraction >= 524288LU) {
          number += 1;
        }
        formatWithCommas(formattedNumber, number);
        sprintf(size, "%sMB", formattedNumber);
      }
    }
    sprintf(buf, "%-19s%c%10s", name, rightAsterisk, size);
  } else {
    memset(buf, ' ', 30);
    buf[30] = '\0';
  }
...
int printFileNameList(int fileNum, int indentOffset) {
  // 引数 fileNum ファイルの通し番号(0～)
  // 引数 indentOffset ファイルの字下げ位置をオフセットする文字数を指定
  //  1(省略可):自動で適切な値を設定 / 0以下:左にオフセットする文字数を指定
  ...
  if (numberOfFiles == 0) {
    msg = sdStsMsg[SD_NO_FILES];
  }
  if (msg != NULL) {
    myGLCD.print(msg, X_LEFT, Y_TOP);
    return;
  }
  currPage = (int)(fileNum / 10.0) + 1;
  ...
  if (indentOffset == 1) {
    ...
    for (int i = 0; i < 10; i++) {
      j = (currPage - 1) * 10 + i;
      if (j < numberOfFiles) {
        maxLevel = max(maxLevel, fileNameList[j]->level);
      }
    }
    maxIndentOffset = -maxLevel;
    indentOffset = 0;
  }
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (j == fileNum) {
      myGLCD.setColor(VGA_BLACK);
      myGLCD.setBackColor(getFileNameColor(j));
    } else {
      myGLCD.setColor(getFileNameColor(j));
      myGLCD.setBackColor(VGA_BLACK);
    }
    printFileName(j, X_LEFT, Y_TOP + i * fontYsize, indentOffset);
  }
  // 現在ページ / 総ページ
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  totalPage = (int)((numberOfFiles - 1) / 10.0) + 1;
  myGLCD.print(F("Page:"),  X_LEFT, Y_TOP + fontYsize * 11);
  sprintf(buf, "%02d/%02d", currPage, totalPage);
  myGLCD.print(buf, X_LEFT, Y_TOP + fontYsize * 12);
  ...
  if (volume.init(card)) {
    ...
    volumesize /= 2LU;                       // SD card blocks are always 512 bytes (2 blocks are 1KB)
  }
  sprintf(buf, "Usage: %13skB", formatWithCommas(formattedNumber, fileUsage / 1024LU));  // 使用容量(キロバイト単位)
  myGLCD.print(buf, X_LEFT + fontXsize * 8, Y_TOP + fontYsize * 11);
  if (volumesize != 0LU) {
    sprintf(buf, "Free:  %13skB", formatWithCommas(formattedNumber, volumesize - fileUsage / 1024LU));
    myGLCD.print(buf, X_LEFT + fontXsize * 8, Y_TOP + fontYsize * 12);
  }

  return maxIndentOffset;
}

int printFileNameListWindow(int fileNum, int indentOffset) {
  int maxIndentOffset;
  
  printWindowFrame(0, 0, 319, 239);
  maxIndentOffset = printFileNameList(fileNum, indentOffset);
  printButtonEx(BTN_SDCARD_PAGEUP);
  printButtonEx(BTN_SDCARD_CURSORUP);
  printButtonEx(BTN_SDCARD_CURSORDOWN);
  printButtonEx(BTN_SDCARD_PAGEDOWN);
  printButtonEx(BTN_SDCARD_DELETEFILE_ALLCSVFILES);
  printButtonEx(BTN_SDCARD_DELETEFILE);
  printButtonEx(BTN_SDCARD_VIEWFILE);
  printButtonEx(BTN_SDCARD_CANCEL);

  return maxIndentOffset;
}

unsigned int getFileNameColor(int fileNum) {
  unsigned int color;

  if (fileNum >= numberOfFiles) {
    return myGLCD.getColor();
  }
  switch (fileNameList[fileNum]->attr) {
    case 'F':
      if (strstr(fileNameList[fileNum]->name, ".CSV") != NULL) {
        color = VGA_YELLOW;
      } else if (strstr(fileNameList[fileNum]->name, HTTP_RESPONSE_TXT) != NULL) {
        color = VGA_FUCHSIA;
      } else {
        color = VGA_AQUA;
      }
...
void moveCursor(int currFileNum, int nextFileNum, int indentOffeet) {
  int currPage, nextPage;
  int currRow, nextRow;
  const int X_LEFT = 8;
  const int Y_TOP = 14;
  ...
  if (currPage == nextPage) {
    myGLCD.setColor(getFileNameColor(currFileNum));
    myGLCD.setBackColor(VGA_BLACK);
    printFileName(currFileNum, X_LEFT, Y_TOP + currRow * fontYsize, indentOffeet);
  }
  myGLCD.setColor(VGA_BLACK);
  myGLCD.setBackColor(getFileNameColor(nextFileNum));
  printFileName(nextFileNum, X_LEFT, Y_TOP + nextRow * fontYsize, indentOffeet);
}

bool deleteFile(int fileNum) {
  char event[MAX_EVENT_LEN];

  if (strstr(fileNameList[fileNum]->name, "~") != NULL) {
    //Google AIによる概要
    //Windowsのファイルシステム（NTFSやFAT32）において、ファイル名に「~」（チルダ）が含まれるショートファイル名（8.3形式）が存在する場合、
    //それはほぼ間違いなく対応する長いファイル名（LFN: Long File Name）が存在することを示しています。
    //まとめ：
    //システムが生成した「8.3形式のショート名」に含まれるチルダであれば、その背後には必ずLFNが存在します。
    //しかし、単にファイル名にチルダが入っているだけでは、それがLFNの代替（ショート名）なのか、単なる記号（一時ファイルやユーザー命名）なのかは判別できません。
    sprintf(event, "SDCARD: \'%s\' Probably long file name", fileNameList[fileNum]->name);
    saveEventToSdCard(event);
    saveEventToSdCard("SDCARD: Deletion aborted");
    return false;
  }
  if (strlen(fileNameList[fileNum]->path) > MAX_PATH_LEN) {
    char path[MAX_PATH_LEN + 1];
    sprintf(event, "SDCARD: Path \'%s\' too long", truncateString(path, fileNameList[fileNum]->path, sizeof(path)));
    saveEventToSdCard(event);
    saveEventToSdCard("SDCARD: Deletion aborted");
    return false;
  }

  if (fileNameList[fileNum]->attr == 'F') {
    sprintf(event, "SDCARD: File  \'%s\' ", fileNameList[fileNum]->path);
    if (SD.remove(fileNameList[fileNum]->path)) {
      strcat(event, "deleted");
      saveEventToSdCard(event);
      return true;
    } else {
      strcat(event, "failed to delete");
      saveEventToSdCard(event);
    }
  }
  if (fileNameList[fileNum]->attr == 'D') {
    sprintf(event, "SDCARD: Directory  \'%s\' ", fileNameList[fileNum]->path);
    if (SD.rmdir(fileNameList[fileNum]->path)) {
      strcat(event, "deleted");
      saveEventToSdCard(event);
      return true;
    } else {
...
bool deleteFiles(int fileNum) {
  int x, y;
  char buf[FULLSIZE_STR_LEN];
  int numberOfDeletedFiles = 0;
  int numberOfUndeletedFiles = 0;

  printWindowFrame(16, 62, 296, 160);
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  if (fileNum == FILENUM_DELETE_ALLCSVFILES) {
    strcpy(buf, "DELETE ALL CSV FILES ? ");
  } else {
    sprintf(buf, "DELETE \'%s\' ?", fileNameList[fileNum]->name);
  }
  myGLCD.print(buf, 24, 90);
  printButtonEx(BTN_SDCARD_DELETEFILE_OK);
  printButtonEx(BTN_SDCARD_DELETEFILE_CANCEL);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (offTimerInterval.check() == 1) {
      if (popupWindowCloseTimer < POPUPWINDOWCLOSETIMER_END) {
    ...
    if (isScreenTouched()) {
      ...
      // OK
      if (isButtonPressed(BTN_SDCARD_DELETEFILE_OK, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_DELETEFILE_OK, STS_INVERSE);
        if (fileNum == FILENUM_DELETE_ALLCSVFILES) {
          for (int i = 1; i < numberOfFiles; i++) {
            if (fileNameList[fileNum]->attr != 'F') {
              continue;
            }
            if (strstr(fileNameList[i]->name, ".CSV") == NULL) {
              continue;
            }
            if (strcmp(fileNameList[i]->name, RESET_CSV) == 0) {
              continue;
            }
            if (strcmp(fileNameList[i]->name, EVENT_CSV) == 0) {
              continue;
            }
            if (deleteFile(i) == true) {
              numberOfDeletedFiles++;
            } else {
              numberOfUndeletedFiles++;
            }
          }
        } else {
        ...
        if (numberOfUndeletedFiles > 0) {
          printUndeletedFilesWinodow();
        }
        break;
      }
      // CANCEL
      if (isButtonPressed(BTN_SDCARD_DELETEFILE_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_DELETEFILE_CANCEL, STS_INVERSE);
        break;
      }
    }
  }
...
void printUndeletedFilesWinodow(void) {
  int x, y;
  char buf[FULLSIZE_STR_LEN];
  ...
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  myGLCD.print(F("FAILED TO DELETE FILE(S)"), 24, 76);
  myGLCD.print(F(" OR DIRECTORY(IES)"), 24, 90);
  sprintf(buf, "SEE '%s' FOR DETAILS", EVENT_CSV);
  myGLCD.print(buf, 24, 104);
  printButtonEx(BTN_SDCARD_DELETEFILE_CLOSE);
  popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
  while (true) {
    if (offTimerInterval.check() == 1) {
      if (popupWindowCloseTimer < POPUPWINDOWCLOSETIMER_END) {
    ...
    if (isScreenTouched()) {
      ...
      // OK
      if (isButtonPressed(BTN_SDCARD_DELETEFILE_CLOSE, x, y)) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_DELETEFILE_CLOSE, STS_INVERSE);
        break;
      }
    }
  }
...
void saveEventToSdCard(String event, int screenMode, int printMode) {
  ...
  strcpy(fileName, EVENT_CSV);
  // ファイルがない場合は、タイトル行を書き込み
  if (!SD.exists(fileName)) {
    file = SD.open(fileName, FILE_WRITE);
    file.println(F("yyyy/mm/dd,hh:mm:ss,event"));
    file.close();
  }
  // ここからデータ行を書き込み
  file = SD.open(fileName, FILE_WRITE);
...
void saveHttpResponseToSdCard(String httpResponse, String datetime) {
  ...
  file = SD.open(HTTP_RESPONSE_TXT, O_RDWR | O_CREAT | O_TRUNC);
  if (!file) {
    sdSts = SD_OPEN_ERROR;
    printSdStatus();
  } else {
    if ((file.println(F("--- httpResponse ---")) > 0) &&
        (file.println(httpResponse) > 0) &&
        (file.println("") > 0) &&
        (file.println(F("--- datetime ---")) > 0) &&
        (file.println(datetime) > 0)) {
      sdSts = SD_SUCCESS;
    } else {
      sdSts = SD_WRITE_ERROR;
      printSdStatus();
...
void saveWattHourTimeperiodToSdCard(void) {
  ...
  isTimeToSaveDataToSdCard = ((t.min == 0 || t.min == 30) && ((t.hr != prevHour) || (t.min != prevMin)));
  if (isTimeToSaveDataToSdCard) {
    saveDataToSdCard();
    prevHour = t.hr;
    prevMin = t.min;
  }
  //計量日の00:00になったら積算値をSDカードに保存してリセット
  if ((t.date == eepromAutoResetDate) && (t.hr == 0) && (t.min == 0)) {
...
void setup() {
  ...
  myGLCD.setFont(SmallFont);
  fontXsize = myGLCD.getFontXsize();     //フォントの幅
  fontYsize = myGLCD.getFontYsize() + 2; //フォントの高さ + 行間(2ドット)
  myGLCD.setFont(SmallFont);
  ...
  if ((value == MODE_CLOCK) || (value == MODE_OFF)) eepromScreenOffMode = value;
  ...
  // 電源ONからSD Cardイニシャライズ開始までの待ち時間がないと処理に失敗する
  // 電源ONからFlashAirイニシャライズ開始までの待ち時間がないと処理に失敗する
  waitSeconds(10);

  printlnEx();
  if (SD.begin(PIN_SDCARD_CS)) {
    printlnEx(F("SD Card initialized"));
  } else {
    printlnEx(F("SD Card initialization failed"));
    printEx(F("Touch the screen to reset the system"));
    while (getButtonNum() == BTN_NOT_PRESSED)
      ;
    beep(BEEP_LOW);
  ...
  currScreenMode = SCREEN_SETUP;
  if (t.sec == ds1302.time().sec) {
    sprintf(buf, "RTC: %04d/%02d/%02d %3s %02d:%02d:%02d",
            t.yr, t.mon, t.date, dayShortStr(t.day), t.hr, t.min, t.sec);
    printlnEx(buf);
    printlnEx(F("DS1302 is not running!"));
    printEx(F("Touch the screen to reset the system"));
    while (getButtonNum() == BTN_NOT_PRESSED)
      ;
    beep(BEEP_LOW);
  ...
  //FlashAir関係
#ifdef FLASH_AIR
  existsFlashAir = initFlashAir(true);
  if (existsFlashAir == true) {
    //日本標準時の取得
    isTimeSynchronized = syncTimeWithServer(true);
    sprintf(event, "SETUP : [%04lXh]Time ", nextSequenceId - 1);
    if (isTimeSynchronized) {
      strcat(event, "synchronized");
      saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
    } else {
  ...
  //タイマー割り込み
  timer1_status = TIMER1_STS_IDLE;
  Timer1.initialize(500000);       // 割り込み間隔(μs)
...
void loop() {
  static int updateTarget = UPDATE_DATA;

  if ((timer1_status == TIMER1_STS_CALCULATED) || (timer1_status == TIMER1_STS_PRINT)) {
    popupWindow();  // ボタンにタッチした時の処理
  }
  if (timer1_status != TIMER1_STS_CALCULATED) {
    return;
  }
  ...
  switch (updateTarget) {
    case UPDATE_DATA:
      ...
      syncTimeWithServerOnceADay();      // 1日1回RTCとシステム時刻を日本標準時に合わせる
      updateTarget = UPDATE_SCREEN;
      break;
    case UPDATE_SCREEN:
