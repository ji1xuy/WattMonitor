#define VER "Program version 20251207.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
bool isPrintFreeMemoryEnabledInInterruption = false;
...
const char FILENAME_EVENT[] = "EVENT.CSV";
const char FILENAME_RESET[] = "RESET.CSV";
//デフォルト引数のある関数の宣言
...
bool findLine(String str, const char *val, char *line, size_t lineSize, int indexVal = -1);
...
bool syncTimeWithServer(bool isSystemResetEnabled) {
  ...
  clearScreen();
  isPrintFreeMemoryEnabledInInterruption = true;
  ...
  if (isTimeSynchronized == true) {
    ...
    isTimeSynchronized = findLine(httpResponse, "\"currentFileTime\":", datetime, sizeof(datetime), 0);
    if (isTimeSynchronized == true) {
      // 200 OK の場合の処理
  ...
  isPrintFreeMemoryEnabledInInterruption = false;
  return isTimeSynchronized;
}
#endif
...
bool findLine(String str, const char *val, char *line, size_t lineSize, int indexVal) {
  unsigned int from = 0;
  ...
  switch (timer1_status) {
    ...
    case TIMER1_STS_CALCULATED:
      if (isPrintFreeMemoryEnabled == true) {
        if (isPrintFreeMemoryEnabledInInterruption == true) {
...
void saveIntegratedValuesToSdCard(int screenMode) {
  ...
  strcpy(fileName, FILENAME_RESET);
      ...
      if (isButtonPressed(BTN_SDCARD_DELETEFILE_OK, x, y)) {
        ...
        if (fileNum == FILENUM_DELETE_ALLCSVFILES) {
          for (int i = 1; i < numberOfFiles; i++) {
            ...
            if (strcmp(fileNameList[i]->name, FILENAME_EVENT) == 0) {
              continue;
            }
...
void printFileDeletionFailedWinodow(int numberOfDeletionFailedFiles) {
  ...
  sprintf(buf, "SEE %s FOR DETAILS", FILENAME_EVENT);
...
void saveEventToSdCard(String event, int screenMode, int printMode) {
  ...
  strcpy(fileName, FILENAME_EVENT);
  // ファイルがない場合は、タイトル行を書き込み
  if (!SD.exists(fileName)) {
    file = SD.open(fileName, FILE_WRITE);
