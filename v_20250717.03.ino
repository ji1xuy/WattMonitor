#define VER "Program version 20250717.03"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  他、多数修正
  v_20250713.01～v.20250717.03 30分積算データがSDカードに自動保存されない不具合 -> DS1302に短時間にアクセスを繰り返したのが原因と判明
  　Time型の変数tをグローバルにして、getClockTime()関数をloop()関数内で1ループ1回のみ実行するように修正、各関数でtの値を使いまわし 
*/
// **************************************************************************************************
...
Time getClockTime(byte isInterrupting = false);
...
Time getClockTime(byte isInterrupting) {
...
void incrElapsedDays(void) {
  //CONFIGメニューを開いて閉じると初回だけelapsedDaysが加算される不具合の確認用
  if (prevDate != t.date) {
    myGLCD.setFont(SmallFont);
    char buf[6];
    sprintf(buf, "%02d %02d", prevDate,  t.date);
