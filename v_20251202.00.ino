#define VER "Program version 20251202.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  v_20251201.01 画面描画が乱れるため、メモリーの空き容量表示を割り込み処理の外で実行
  v_20251202.00 画面に応じて、メモリーの空き容量表示を割り込み処理の中と外とで使い分け
*/
// **************************************************************************************************
...
const struct BTN btn[] = {
  ...
  { "CANCEL", "", 247, 203, 311, 231 },
  //printFreeMemory実行用　ボタンの描画なし
  { "", "", 0, 0, 70, 35 },
};
bool isPrintFreeMemory = false;
bool isPrintFreeMemoryInInterrupting = false;
...
// (SDカードの状態)
int sdSts;
const int SDCARD_PRESENT = 0;  // SDカードが挿入されている
...
unsigned long printFileChar(File file, unsigned long filePos, bool printChar = true);
...
bool syncTimeWithServer(bool systemReset) {
  ...
  clearScreen();
  isPrintFreeMemoryInInterrupting = true;
  ...
  isPrintFreeMemoryInInterrupting = false;
  return isTimeSynchronized;
}
#endif
...
void timer1(void) {
  ...
  switch (timer1_status) {
    ...
    case TIMER1_STS_CALCULATED:
      if (isPrintFreeMemory == true) {
        if (isPrintFreeMemoryInInterrupting == true) {
          printFreeMemory(0, 0);
        }
      }
...
void popupWindow(void) {
  ...
  switch (btnNum) {
    ...
    case BTN_PRINTFREEMEMORY:
      beep(BEEP_HIGH);
      isPrintFreeMemory = (isPrintFreeMemory == false ? true : false); 
      break;
    default:
      //何もしない
      break;
...
void formatValue(char *str, char *format, byte value, int mode) {
  switch (mode) {
    case EEPROM_SCREEN_OFF_MODE:
      sprintf(str, format, (value == MODE_CLOCK) ? "Clock" : "Blank");
      break;
    case EEPROM_SOUND:
    case EEPROM_AUTO_TIME_SYNC:
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE, x, y)) {
        ...
        //  (strstr(fileNameList[fileNum]->name, ".TXT") == NULL)) {
        //  continue;
        //}
        beep(BEEP_HIGH);
...
unsigned long printFileChar(File file, unsigned long filePos, bool printChar) {
  const int X_LEFT = 8;
  const int Y_TOP = 2;
  char ch;
...
void printFileViewWindow(int fileNum) {
  File file;
  unsigned long filePos[MAX_FILEVIEW_PAGE + 1];
  int page = 1;
  char buf[BUFFER_LEN_FULL];
  const int X_LEFT = 8;
  const int Y_TOP = 2;
  ...
  filePos[page] = file.position();
  filePos[page + 1]  = printFileView(file, filePos[page], page);
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEEND, x, y)) {
        ...
        if ((file.size() - filePos[page]) > 10000LU) {
          printWindowFrame(80, 82, 240, 138);
          myGLCD.setColor(VGA_WHITE);
          myGLCD.print("Reading...", 128, 96);
          isReading = true;
        } 
        while (filePos[page + 1] < (file.size() - 1)) {
          page++;
          if (page == MAX_FILEVIEW_PAGE) {
            break;
          }
          filePos[page + 1]  = printFileChar(file, filePos[page], false);
          if (isReading == true)  {
            if (page % 10 == 0) {
              sprintf(buf, "%sB", formatWithCommas(formattedNumber, filePos[page + 1]));
              myGLCD.print(buf, 128, 110);
            }
          }
        }
        printFileView(file, filePos[page], page);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEEND, STS_NORMAL);
        continue;
      }
