...
void sdCard(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // VIEW FILE
      if (isButtonPressed(BTN_SDCARD_VIEW_FILE, x, y)) {
        if (fileNameList[fileNum]->attribute == 'D') {
          beep(BEEP_LOW);
          continue;
        }
        ...
        //}
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEW_FILE, STS_INVERSE);
...
void printFileNameListWindow(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_VIEW_FILE, STS_NORMAL);
