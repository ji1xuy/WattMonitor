#define VER "Program version 20251013.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  v_20251013.00 実数値を変数化 FONT_W, FONT_H
*/
// **************************************************************************************************
...
const int FONT_W = 8; //SmallFontの幅
const int FONT_H = 14;//SmallFontの高さ(2ドットの行間を含む)
...
const struct BTN btn[] = {
  ...
  { "CURSOR", "UP", 255, 37, 311, 63 },
  { "CURSOR", "DOWN", 255, 67, 311, 93 },
  { "PAGE", "DOWN", 255, 97, 311, 123 },
...
unsigned long printFileChar(File file, unsigned long filePos, bool printChar) {
  ...
  while (file.available()) {
    ...
    if (iscntrl(c)) { //制御文字を除外
      continue;
    }
    if (printChar == true) {
      myGLCD.print(String(c), curX, curY);
    }
    curX += X_LEFT;
  }
...
void getFileName(File dir, String path, int level) {
  ...
  while (true) {
    ...
    if (entry.isDirectory()) {
      ...
      sprintf(fileNameList[numberOfFiles]->size, "%8u", entry.size());  // バイト単位
      numberOfFiles++;
...
void printFileName(int fileNum, int x, int y) {
  ...
  if (fileNum < numberOfFiles) {
    ...
    sprintf(buf, "%-21s%8s", name, fileNameList[fileNum]->size);
  } else {
    memset(buf, ' ', 29);
    buf[29] = '\0';
  }
...
void printFileNameList(
  ...
  for (int i = 0; i < 10; i++) {
    ...
    printFileName(j, X_LEFT, Y_TOP + i * FONT_H);
  }
  // 現在ページ / 総ページ
  myGLCD.setColor(VGA_WHITE);
  totalPage = (int)((numberOfFiles - 1) / 10.0) + 1;
  myGLCD.print("Page:", 255, 140);
  sprintf(buf, "%02d / %02d", currPage, totalPage);
  myGLCD.print(buf, 255, 154);
  ...
  if (volumesize != 0LU) {
    ...
    myGLCD.print(buf, X_LEFT, Y_TOP + 154);
    sprintf(buf, "Free:  %10lukB", volumesize - fileUsage / 1024LU);
    myGLCD.print(buf, X_LEFT, Y_TOP + 168);
    } else {
    strcpy(buf, "Cannot get SD card capacity");
    myGLCD.print(buf, X_LEFT, Y_TOP + 154);
  }

  myGLCD.setColor(VGA_BLACK);
  myGLCD.setBackColor(getFileNameColor(fileNum));
  printFileName(fileNum, X_LEFT, Y_TOP + row * FONT_H);
}
...
void changeCursol(int currFileNum, int nextFileNum) {
  ...
  myGLCD.setBackColor(VGA_BLACK);
  printFileName(currFileNum, X_LEFT, Y_TOP + currRow * FONT_H);
  myGLCD.setColor(VGA_BLACK);
  myGLCD.setBackColor(getFileNameColor(nextFileNum));
  printFileName(nextFileNum, X_LEFT, Y_TOP + nextRow * FONT_H);
}
