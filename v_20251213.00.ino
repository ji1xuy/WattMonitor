#define VER "Program version 20251213.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  v_20250720.00 isInterrupting変数にvolatile修飾子が漏れていたため追加、OFF,ON,MODE_BLANK,MODE_CLOCK定数をenumからconst byte型に変更
...
void printFreeMemory(int x, int y) {
  ...
  printStringEx(buf, x, y, VGA_BLACK, VGA_YELLOW);
  myGLCD.setFont(font);
...
void initFilePosList(struct FILEPOSLIST *list)
{
  list->next = list->prev = NULL;
  list->value = 0LU;
}
 
void addFilePosNode(struct FILEPOSLIST *list, unsigned long value) {
...
void clearFilePosList(struct FILEPOSLIST *list) {
  ...
  initFilePosList(list);
}

void printFileViewWindow(int fileNum) {
  File file;
  unsigned long filePos, filePrevPos, fileNextPos;
  ...
  addFilePosNode(list, filePos);
  fileNextPos = printFileView(file, filePos, page);
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEUP, x, y)) {
        ...
        page--;
        filePos = fileNextPos;
        fileNextPos = printFileView(file, filePrevPos, page);
        filePrevPos = getFilePosNode(list, filePos)->prev;
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEUP, STS_NORMAL);
        continue;
      }
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEDOWN, x, y)) {
        ...
        filePrevPos = filePos;
        filePos = fileNextPos;
        fileNextPos = printFileView(file, fileNextPos, page);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEDOWN, STS_NORMAL);
        continue;
      }
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEEND, x, y)) {
        ...
        if ((file.size() - filePos) > 10000LU) {
          printWindowFrame(80, 82, 240, 138);
          myGLCD.setColor(VGA_WHITE);
          myGLCD.print("Reading...", 128, 96);
          isReading = true;
        } 
        while (fileNextPos < (file.size() - 1LU)) {
          page++;
          if (page == MAX_FILEVIEW_PAGE) {
            break;
          }
          ...
          if (isReading == true)  {
            if (page % 10 == 0) {
              sprintf(buf, "%sB", formatWithCommas(formattedNumber, fileNextPos));
              myGLCD.print(buf, 128, 110);
            }
          }
        }
        printFileView(file, filePos, page);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEEND, STS_NORMAL);
        continue;
      }
