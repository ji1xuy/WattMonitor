#define VER "Program version 20250921.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
const int MAX_PATHNAME_LEN = 50;    // フォルダ名
...
void printWindowFrame(int left, int top, int right, int bottom) {
  myGLCD.setColor(VGA_BLACK);
  myGLCD.fillRoundRect(left, top, right, bottom);
  myGLCD.setColor(VGA_WHITE);
  myGLCD.setBackColor(VGA_BLACK);
  ...
  switch(fileNameList[fileNum]->attribute) {
    case 'F':
      if (strstr(fileNameList[fileNum]->name, ".CSV") != NULL) {
        c = VGA_AQUA;
      } else if (strstr(fileNameList[fileNum]->name, "HTTP_RES.TXT") != NULL) {
        c = VGA_FUCHSIA;
      } else {
        c = VGA_YELLOW;
      }
        ...
        if (fileNum == FILENUM_DELETE_ALL) {
          for (int i = 1; i < numberOfFiles; i++) {
            if ((fileNameList[fileNum]->attribute == 'F') &&
                (strstr(fileNameList[i]->name, ".CSV") != NULL) && (strcmp(fileNameList[i]->name, "EVENT.CSV") != 0)) {
              if (SD.remove(fileNameList[i]->path) == true) {
                //popupWindowCloseTimer = POPUPWINDOWCLOSETIMER_START;
...
bool deleteFile(char* pathName) {
  char event[BUFFER_LEN_EVENT];

  if (SD.remove(pathName) == true) {
    sprintf(event, "SDCARD: File \"%s\" deleted", pathName);
    saveEventToSdCard(event);
    return true;
  }
  return false;
