#define VER "Program version 20251004.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
void sdCard(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // DELETE
      if (isButtonPressed(BTN_SDCARD_DELETE, x, y)) {
        ...
        beep(BEEP_HIGH);
        myGLCD.setColor(VGA_FUCHSIA);
        printButtonEx(BTN_SDCARD_DELETE, STS_INVERSE);
...
void getFileName(File dir, String path, int level) {
  ...
  while (true) {
    ...
    if (entry.isDirectory()) {
      ...
      sprintf(fileNameList[numberOfFiles]->size, "%5u", entry.size() / 1024LU);  // キロバイト単位
      numberOfFiles++;
...
void printFileName(int fileNum, int x, int y) {
  ...
  if (fileNum < numberOfFiles) {
    ...
    sprintf(buf, "%-20s%5s", name, fileNameList[fileNum]->size);
  } else {
    memset(buf, ' ', 25);
    buf[25] = '\0';
  }
...
void printFileNameList(
  ...
  if (volumesize != 0LU) {
    sprintf(buf, "%9lu", volumesize - fileUsage / 1024LU);  // 総容量-使用容量=空き容量(キロバイト単位)
  } else {
    strcpy(buf, " ????????");
  }
  myGLCD.print(buf, 224, 146);
  myGLCD.print(F("  kB Free"), 224, 160);

  myGLCD.setColor(VGA_BLACK);
  myGLCD.setBackColor(getFileNameColor(fileNum));
  printFileName(fileNum, 24, 34 + row * 14);
}
...
void changeCursol(int currFileNum, int nextFileNum) {
  ...
  myGLCD.setBackColor(VGA_BLACK);
  printFileName(currFileNum, 24, 34 + currRow * 14);
  myGLCD.setColor(VGA_BLACK);
  myGLCD.setBackColor(getFileNameColor(nextFileNum));
  printFileName(nextFileNum, 24, 34 + nextRow * 14);
}
...
void printFileNameListWindow(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_DELETE, STS_NORMAL);
