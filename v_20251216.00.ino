#define VER "Program version 20251215.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
struct FILEPOSLIST *addFilePosNode(struct FILEPOSLIST *list, unsigned long value) {
  ...
  prev_node = new struct FILEPOSLIST;
...
void printFileViewWindow(int fileNum) {
  ...
  // 1ページ目の先頭文字のファイル内現在位置をセット
  list = initFilePosList(0LU);
  
  // nodeは、表示しているページの先頭文字のファイル内現在位置と前後のノードを保持する変数
  // node->prev = NULL, node->value = 0LU, node->next = NULL 
  // node->next == NULLの時、そのnodeは末尾のnode 
  node = list;
  
  // printFileViewで1ページ目を表示し、
  // printFileViewの戻り値(ページの最後の文字の次の文字(2ページ目の先頭の文字)のファイル内現在位置)を
  // node->next構造体変数に格納
