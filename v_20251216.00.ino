#define VER "Program version 20251216.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
void printFreeMemory(int x, int y) {
  ...
  sprintf(buf, "%5d", (int)&v - (__brkval == 0 ? (int)&__heap_start : (int)__brkval));
  myGLCD.setFont(SmallFont);
...
void printFileViewWindow(int fileNum) {
  ...
  // printFileViewで1ページ目を表示し、
  // printFileViewの戻り値(ページの最後の文字の次の文字(次のページの先頭の文字)のファイル内現在位置)を
  // node->next構造体変数に格納
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEDOWN, x, y)) {
        ...
        page++;
        node = node->next;
        unsigned long fileNextPos = printFileView(file, node->value, page);
        if (node->next == NULL) {
          addFilePosNode(list, fileNextPos);
        }
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGEDOWN, STS_NORMAL);
        continue;
      }
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEEND, x, y)) {
        ...
        while (node->next->value < (file.size() - 1LU)) {
          ...
          node = node->next;
          if (node->next == NULL) {
            addFilePosNode(list, printFileChar(file, node->value, false));
          }
          if (isReading == true)  {
            if (page % 10 == 0) {
...
void printFileNameList(
  ...
  //printFreeMemory(X_LEFT, Y_TOP +fontYsize * 10);
}
