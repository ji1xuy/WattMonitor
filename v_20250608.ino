#define VER "Program version 20250608"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  2025/5/20 不具合解消せず　-> IDE Ver.2.2.1でコンパイル  
  2025/5/22 不具合解消せず　-> IDE Ver.2.3.2でコンパイル(Ver.2.3.3以降はエラー発生し使用不可)
  ...
  2025/5/24 不具合解消せず -> saveWattHourTimeperiodToSdCard()関数 ダミーでないprintStringEx文に修正
  2025/5/24 不具合解消せず -> saveWattHourTimeperiodToSdCard()関数 ダミーでないprintStringEx文を取り、sprintf文を追加
  -> 不具合発生せず
  2025/5/24 saveWattHourTimeperiodToSdCard()関数 sprintf文を取り、noInterrupts,interrupts文を追加
  2025/5/24 不具合解消せず -> saveWattHourTimeperiodToSdCard()関数 snoInterrupts,interrupts文を取り、if文を早期returnに修正
  2025/5/24 不具合解消せず -> saveWattHourTimeperiodToSdCard()関数 if文の前にdelay(100)を追加
  2025/5/24 不具合解消せず -> saveWattHourTimeperiodToSdCard()関数 delay(100)を取り、sprintf文を追加 -> 不具合発生せず
  2025/5/26 sprintf文を削除、saveWattHourTimeperiodToSdCard()関数内に記述している処理をloop()関数内に直に記述するように修正 -> 不具合発生せず
...
enum { MODE_BLANK,
       MODE_CLOCK };  //SCREEN OFFボタンを押した時のモードの選択
//時刻を取得するサーバの選択
...
//Temwr2 TimerFreeToneライブラリ使用によりtone()関数不使用のため、不使用
