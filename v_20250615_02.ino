#define VER "Program version 20250615.02"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  30分積算データがSDカードに自動保存されない不具合発生
    2025/6/13 割り込み中に画面描画をしないように変更
    2025/6/14 割り込み中に画面描画をしないように変更
    2025/6/15 currTime,prevTime変数にvolatile就職しを追加
  -> 不具合発生せず
*/
...
//Time-master.zip
#include <TimeLib.h>
volatile char currTime[6] = "";  //"hh:mm" + '\0'
volatile char prevTime[6] = "";  //"hh:mm" + '\0'
enum {
  TIME_YEAR,
  TIME_MONTH,
  TIME_DAY,
...
void timer1(void) {
  if (timer1_status != TIMER1_STS_CALCULATE) {
...
void saveWattHourTimeperiodToSdCard(void) {
  ...
  //myGLCD.setFont(SmallFont);
  //printStringEx("test----", 168, 174 - 14, VGA_WHITE, VGA_BLACK);
  ...
  //if ((t.min == 0 || t.min == 30) && (String(currTime) != prevTime)) {
    //myGLCD.setFont(SmallFont);
    //printStringEx("----test", 168, 174 - 14, VGA_WHITE, VGA_BLACK);
    saveDataToSdCard();
  ...
  //if ((t.date == eepromAutoResetDate) && (String(currTime) == "00:00")) {
