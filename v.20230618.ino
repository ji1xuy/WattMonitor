#define VER "Program version 20230618"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
#include <Metro.h>
Metro screenOffTimerInterval = Metro(60000); // タイマー用変数増加間隔(ms)
Metro updateScreenInterval = Metro(2000); // 画面表示更新間隔(ms)
int screenOffTimer; // 画面自動消灯タイマー用
int screenOffTimerOnBattery; // 画面自動消灯タイマー用(停電時)
const int TIMER_RESET = -2; //screenOffTimerOnBatteryの状態を表す
const int TIMER_STOP  = -1; //screenOffTimerOnBatteryの状態を表す
  ...
  if ((existsFlashAir == true) && (eepromAutoTimeSync == ON)) {
    ...
    if ((t.date != syncDay) && (t.hr == eepromTimeSyncHour) && (t.min == eepromTimeSyncMin)) {
      ...
      if (syncTime() == true) {
        ...
        sprintf(event, "[%04lX][ime synchronization failed (automatically)", nextSequenceId - 1);
        saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
      }
      initPrevClockValues();
...
void countupTimer(void) {
  if (screenOffTimerInterval.check() == 1) {
    if (hasBlackout == true) { //停電中
      //screenOffTimerOnBatteryを1分毎に増加させる
      if (screenOffTimerOnBattery >= 0) {
        screenOffTimerOnBattery++;
      }
    } else {
      if ((eepromScreenOffTimer > 0) && (currScreenMode != SCREEN_BLANK)) {
        screenOffTimer++;
      }
    }
  }
...
void setScreenMode(void) {
  if ((currScreenMode == SCREEN_BLANK) || (currScreenMode == SCREEN_CLOCK)) {
    ...
    if ((hasBlackout == true) && (screenOffTimerOnBattery == TIMER_RESET)) {
      // 停電が発生した場合
      currScreenMode = printMainWindow(SCREEN_ON);
    }
    if ((hasBlackout == false) && (screenOffTimerOnBattery != TIMER_RESET)) {
      // 復電した場合
      currScreenMode = printMainWindow(SCREEN_ON);
    }
  } else {
    if (hasBlackout == true) {
      ...
      // 通電中
      if ((eepromScreenOffTimer != 0) && (screenOffTimer >= eepromScreenOffTimer)) {
...
int printMainWindow(int screenMode) {
  ...
  printElapsedDays();
  printResetTime();
  screenOffTimerOnBattery = TIMER_RESET;
  return screenMode;
}
...
void popupWindow(void) {
  ...
  // メニューボタンを押して戻ってきたらSCREEN_BLANK(またはSCREEN_CLOCK)にするタイマーをセットする
  if (btnNum != BTN_NOT_PRESSED) {
    screenOffTimer = 0;
    screenOffTimerInterval.reset();
  }
}
...
void printBlackout(void) {
  ...
  if (hasBlackout == true) {
    ...
    if (screenOffTimerOnBattery < 3) { //3分未満はビープ音を鳴らす
      beep(BEEP_800MS);
    } else { //3分になったらビープ音を止めてSCREENOFF
      screenOffTimerOnBattery = TIMER_STOP;
      if (eepromScreenOffTimer > 0) {
