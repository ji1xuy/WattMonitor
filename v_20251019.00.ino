#define VER "Program version 20251019.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
unsigned long printFileView(File file, unsigned long filePos, int page) {
  ...
  sprintf(buf, "Page: %03d (Read: %luB / %luB)", page, filePos, file.size());
    ...
    if (myTouch.dataAvailable()) {
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGEEND, x, y)) {
        ...
        if ((file.size() - filePos[page]) > 10000LU) {
          printWindowFrame(80, 90, 240, 130);
          myGLCD.setColor(VGA_WHITE);
          //myGLCD.setBackColor(VGA_BLACK);
          myGLCD.print("Reading...", 128, 104);
        } 
...
void printFileNameList(
  ...
  sprintf(buf, "Usage: %10lukB", fileUsage / 1024LU);  // 使用容量(キロバイト単位)
  myGLCD.print(buf, X_LEFT + fontXsize * 13, Y_TOP + fontYsize * 11);
  strcpy(buf, "Free:  ");
  if (volumesize != 0LU) {
    sprintf(buf + strlen(buf), "%10lukB", volumesize - fileUsage / 1024LU);
    } else {
    strcat(buf, "??????????kB");
  }
  myGLCD.print(buf, X_LEFT + fontXsize * 13, Y_TOP + fontYsize * 12);
