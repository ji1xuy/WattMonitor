#define VER "Program version 20230922"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  2023/9/20 printBlackout()関数内の仮面操作部分をsetCsreenMode()関数に移動して修正
...
const byte DEFAULT_CT1_PERCENTAGE = 141;
const byte DEFAULT_CT2_PERCENTAGE = 141;
...
void syncTimeOnceADay(void) {
  ...
  if (existsFlashAir && (eepromAutoTimeSync == ON)) {
    Time t = getClockTime();
    if ((t.date != syncDay) && (t.hr == eepromTimeSyncHour) && (t.min == eepromTimeSyncMin)) {
      syncDay = t.date;
      prevScreenMode = currScreenMode;
      turnOnScreen();
      if (needsResetFlashAir) {
        resetFlashAir();
      }
      if (syncTime() == true) {
        sprintf(event, "SYSTEM: [%04lX]Time synchronized", nextSequenceId - 1);
        saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
      } else {
        sprintf(event, "SYSTEM: [%04lX][Time synchronization failed", nextSequenceId - 1);
        saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
      }
      initPrevClockValues();
    ...
    case TIMER1_STS_CALCULATE:
      ...
      if (vrms[1] < 1.0) {
        if (hasBlackout == false) {
          hasBlackout = true;
          saveEventToSdCard(F("SYSTEM: Power down"), currScreenMode);
          break;
        }
      } else if (vrms[1] < 80.0) {
        if (hasBrownout == false) {
          hasBrownout = true;
          saveEventToSdCard(F("SYSTEM: Power voltage dropped"), currScreenMode);
          break;
        }
      }
      // 復電、電源電圧復帰検出
      if (vrms[1] >= 90.0) {
        if (hasBlackout) {
          ...
          hasBrownout = false;
          requestFlashAirReset();
          saveEventToSdCard(F("SYSTEM: Power restored"), currScreenMode);
        }
        if (hasBrownout) {
          hasBrownout = false;
          requestFlashAirReset();
          saveEventToSdCard(F("SYSTEM: Power voltage recovered"), currScreenMode);
        }
      }
      break;
    default:
...
void countupTimer(void) {
  if (offTimerInterval.check() == 1) {
    if (hasBlackout) { //停電中
      //screenOffTimerInBlackoutを1分毎に増加させる
...
void setScreenMode(void) {
  ...
  //復電時1回だけ実行
  if ((hasBlackout == false) && (screenOffTimerInBlackout != SCREENOFFTIMERINBLACKOUT_STOP)) {
    beep(BEEP_OFF);
...
void printBlackout(void) {
  ...
  if (hasBlackout) {
    ...
    if (screenOffTimerInBlackout < SCREENOFFTIMERINBLACKOUT_END) {
      //SCREENOFFTIMERINBLACKOUT_END分未満はビープ音を鳴らす
      beep(BEEP_800MS);
...
void reset(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      y = myTouch.getY();
      // SYSTEM RESET
      if (isButtonPressed(BTN_RST_SYSTEM_RESET, x, y)) {
        beep(BEEP_HIGH);
      ...
      // OK
      if (isButtonPressed(BTN_RST_OK, x, y)) {
        beep(BEEP_HIGH);
      ...
      // CANCEL
      if (isButtonPressed(BTN_RST_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        break;
      }
    }
...
void systemReset(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      y = myTouch.getY();
      // OK
      if (isButtonPressed(BTN_SR_OK, x, y)) {
        beep(BEEP_HIGH);
      ...
      // CANCEL
      if (isButtonPressed(BTN_SR_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        break;
      }
    }
...
void adjustDateTime(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // DOWN
      if (isButtonPressed(BTN_DT_DOWN, x, y)) {
        beep(BEEP_HIGH);
      ...
      // UP
      if (isButtonPressed(BTN_DT_UP, x, y)) {
        beep(BEEP_HIGH);
      ...
      // +
      if (isButtonPressed(BTN_DT_PLUS, x, y)) {
        beep(BEEP_HIGH);
      ...
      // -
      if (isButtonPressed(BTN_DT_MINUS, x, y)) {
        beep(BEEP_HIGH);
      ...
      // SYNC
      if (isButtonPressed(BTN_DT_SYNC, x, y)) {
        if (existsFlashAir) {
          beep(BEEP_HIGH);
          ...
          if (isSynchronized) {
            sprintf(event, "SYNC  : [%04lX]Time synchronized", nextSequenceId - 1);
            saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
          } else {
            sprintf(event, "SYNC  : [%04lX]Time synchronization failed", nextSequenceId - 1);
            saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
          }
          delay(3000);
        }
      ...
      // OK
      if (isButtonPressed(BTN_DT_OK, x, y)) {
        beep(BEEP_HIGH);
      ...
      // CANCEL
      if (isButtonPressed(BTN_DT_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        break;
      }
    }
  ...
  for (index = 0; index < EEPROM_MODE_MAX; index++) {
    switch (mode[index]) {
      ...
      case EEPROM_CT1_PERCENTAGE:
        ...
        strcpy(item[EEPROM_CT1_PERCENTAGE].format, "%3d%%");
      ...
      case EEPROM_CT2_PERCENTAGE:
        ...
        strcpy(item[EEPROM_CT2_PERCENTAGE].format, "%3d%%");
      ...
      case EEPROM_CT3_PERCENTAGE:
        ...
        strcpy(item[EEPROM_CT3_PERCENTAGE].format, "%3d%%");
      ...
      case EEPROM_BAR_GRAPH_FULLSCALE:
        //Bar graph FS
        y += ROW_HEIGHT;
        strcpy(item[EEPROM_BAR_GRAPH_FULLSCALE].name, "Bar graph FS");
        ...
        strcpy(item[EEPROM_BAR_GRAPH_FULLSCALE].format, "%2dkW");
      ...
      case EEPROM_AUTO_TIME_SYNC:
        //Auto time sync
        y += ROW_HEIGHT;
        strcpy(item[EEPROM_AUTO_TIME_SYNC].name, "Auto time sync");
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (myTouch.dataAvailable()) {
        ...
        y = myTouch.getY();
        // DOWN
        if (isButtonPressed(BTN_CFG_DOWN, x, y)) {
          beep(BEEP_HIGH);
        ...
        // UP
        if (isButtonPressed(BTN_CFG_UP, x, y)) {
          beep(BEEP_HIGH);
        ...
        // +
        if (isButtonPressed(BTN_CFG_PLUS, x, y)) {
          beep(BEEP_HIGH);
        ...
        // -
        if (isButtonPressed(BTN_CFG_MINUS, x, y)) {
          beep(BEEP_HIGH);
        ...
        // LOAD DEFAULT
        if (isButtonPressed(BTN_CFG_LOAD_DEFAULT, x, y)) {
          beep(BEEP_HIGH);
        ...
        // OK
        if (isButtonPressed(BTN_CFG_OK, x, y)) {
          beep(BEEP_HIGH);
        ...
        // CANCEL
        if (isButtonPressed(BTN_CFG_CANCEL, x, y)) {
          beep(BEEP_HIGH);
          break;
        }
      }
    }
    if (isLoadDefaultButtonPressed) {
...
byte writeValueToEEPROM(byte value, struct ITEM *item, int mode) {
  ...
  if (item[mode].value != value) {
    switch (mode) {
      ...
      case EEPROM_CT2_PERCENTAGE:
      case EEPROM_CT3_PERCENTAGE:
        EEPROM.write(mode, item[mode].value - 1);
      default:
    ...
    saveEventToSdCard(event, currScreenMode);
  }
...
void sdCard(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // PAGE UP
      if (isButtonPressed(BTN_SD_PAGE_UP, x, y)) {
        if (numberOfFiles <= 10) {
          beep(BEEP_LOW);
          continue;
        }
      ...
      // CURSOR UP
      if (isButtonPressed(BTN_SD_CURSOR_UP, x, y)) {
        if (numberOfFiles < 1) {
          beep(BEEP_LOW);
          continue;
        }
      ...
      // CURSOR DOWN
      if (isButtonPressed(BTN_SD_CURSOR_DOWN, x, y)) {
        if (numberOfFiles < 1) {
          beep(BEEP_LOW);
          continue;
        }
      ...
      // PAGE DOWN
      if (isButtonPressed(BTN_SD_PAGE_DOWN, x, y)) {
        if (numberOfFiles <= 10) {
          beep(BEEP_LOW);
          continue;
        }
      ...
      // DELETE ALL
      if (isButtonPressed(BTN_SD_DELETE_ALL, x, y)) {
        beep(BEEP_HIGH);
      ...
      // DELETE
      if (isButtonPressed(BTN_SD_DELETE, x, y)) {
      ...
      // CANCEL
      if (isButtonPressed(BTN_SD_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        break;
...
boolean deleteFile(int fileNum) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      y = myTouch.getY();
      // OK
      if (isButtonPressed(BTN_DF_OK, x, y)) {
        beep(BEEP_HIGH);
        //DELETE ALL
        if (fileNum == FILENUM_DELETE_ALL) {
      ...
      // CANCEL
      if (isButtonPressed(BTN_DF_CANCEL, x, y)) {
        beep(BEEP_HIGH);
        return false;
      }
    }
...
void saveWattHourTimeperiod(void) {
  ...
  if ((t.date == eepromAutoResetDate) && (strcmp(currTime, "00:00") == 0)) {
    if (needsSaveIntegratedValues) {
      ...
      saveEventToSdCard("SYSTEM: Integrated values reset", currScreenMode);
...
void setup() {
  ...
  if (getCardStatus() == true) {
    ...
    isSynchronized = syncTime();
    if (isSynchronized) {
      sprintf(event, "SETUP : [%04lX]Time synchronized", nextSequenceId - 1);
      saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
    } else {
      sprintf(event, "SETUP : [%04lX]Time synchronization failed", nextSequenceId - 1);
      saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
    }
  }
  delay(3000);
