#define VER "Program version 20250816.01"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
bool isResetFlashAirRequested;  // trueの時、次の時刻取得の際にFlashAirをリセットする
...
bool iSDIO_sendHttpRequest(uint32_t sequenceId, int protocol) {
  ...
  clearScreen();
  printlnEx(F("HTTP Request sending..."));
  ...
  int waitingTime = 10;
  sprintf(buf, "%d seconds waiting", waitingTime);
  printEx(buf);
...
bool iSDIO_waitHttpResponse(uint32_t sequenceId) {
  ...
  printEx(F("HTTP response waiting"));
  uint8_t prev = 0xFF;
  for (int i = 0; i < 20; ++i) {
...
bool iSDIO_readHttpResponse(char *httpResponse, int httpReponseSize) {
  ...
  printlnEx(F("HTTP response reading..."));
  httpResponse[0] = '\0';
...
bool initFlashAir(void) {
  // Initialize SD card.
  printEx(F("FlashAir initializing..."));
