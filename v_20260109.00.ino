#define VER "Program version 20260109.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  ■IDE (1.8.5)(1.8.13)(1.8.19)(2.2.1)(2.3.2)
  ...
  最大130048バイトのフラッシュメモリのうち、スケッチが127744バイト（98%）を使っています。
  グローバル変数は5594バイトのRAMを使用しています。。
*/
// **************************************************************************************************
...
bool isOutofMemory(void) {
  if (getFreeMemory() < 500) {
    ...
    myGLCD.setColor(VGA_WHITE);
    myGLCD.print("Out of Memory :(", 96, 96);
    myGLCD.print("Touch here", 96, 110);
    while (getButtonNum() == BTN_NOT_PRESSED)
      ;
    beep(BEEP_HIGH);
    interrupts();
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_VIEWFILE, x, y)) {
        ...
        if (fileNameList[fileNum]->attribute == 'D') {
          continue;
        }
        beep(BEEP_HIGH);
...
void getFileName(File file, String path, int level) {
  ...
  while (true) {
    ...
    if (nextFile.isDirectory()) {
      fileNameList[numberOfFiles]->attribute = 'D';
      fileNameList[numberOfFiles]->size = 0LU;
      numberOfFiles++;
      getFileName(nextFile, fullPath, level + 1);
    } else {
      fileNameList[numberOfFiles]->attribute = 'F';
...
unsigned int getFileNameColor(int fileNum) {
  ...
  switch (fileNameList[fileNum]->attribute) {
    case 'F':
...
bool deleteFile(int fileNum) {
  ...
  if (strlen(fileNameList[fileNum]->path) > 79) {
    char path[80];
    sprintf(event, "SDCARD: \'%s\' Pathname too long", truncateString(path, fileNameList[fileNum]->path, sizeof(path)));
    saveEventToSdCard(event);
    return false;
  }

  if (fileNameList[fileNum]->attribute == 'F') {
    if (SD.remove(fileNameList[fileNum]->path)) {
      sprintf(event, "SDCARD: File \'%s\' deleted", fileNameList[fileNum]->path);
      saveEventToSdCard(event);
      return true;
    } else {
      sprintf(event, "SDCARD: File \'%s\' failed to delete", fileNameList[fileNum]->path);
      saveEventToSdCard(event);
    }
  }
  if (fileNameList[fileNum]->attribute == 'D') {
    if (SD.rmdir(fileNameList[fileNum]->path)) {
      sprintf(event, "SDCARD: Directory \'%s\' deleted", fileNameList[fileNum]->path);
      saveEventToSdCard(event);
      return true;
    } else {
      sprintf(event, "SDCARD: Directory \'%s\' failed to delete", fileNameList[fileNum]->path);
      saveEventToSdCard(event);
    }
  }
  return false;
