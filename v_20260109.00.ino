...
/*
  ...
  最大130048バイトのフラッシュメモリのうち、スケッチが127728バイト（98%）を使っています。
...
bool isOutofMemory(void) {
  if (getFreeMemory() < 500) {
    ...
    myGLCD.setColor(VGA_WHITE);
    myGLCD.print("Out of Memory :(", 112, 96);
    myGLCD.print("Touch here", 112, 110);
    while (getButtonNum() == BTN_NOT_PRESSED)
      ;
    beep(BEEP_HIGH);
    interrupts();
...
void printFileName(int fileNum, int x, int y, int indentOffeet) {
  ...
  if (fileNum < numberOfFiles) {
    ...
    if (fileNameList[fileNum]->attribute == 'D') {
      ...
      if (fileNameList[fileNum]->size < 10000000LU) { // 9999999 -> 9,999,999B
        formatWithCommas(formattedNumber, fileNameList[fileNum]->size);
        sprintf(size, "%sB", formattedNumber);
      } else if (fileNameList[fileNum]->size < 1023998977LU) { // 1023998976/1024 -> 999999 -> 999,999kB
        formatWithCommas(formattedNumber, fileNameList[fileNum]->size / 1024LU); 
        sprintf(size, "%skB", formattedNumber);
      } else {  
        formatWithCommas(formattedNumber, fileNameList[fileNum]->size / 1048576LU); 
...
int printFileNameListWindow(int fileNum, int indentOffset) {
  ...
  return printFileNameList(fileNum, indentOffset);
}
