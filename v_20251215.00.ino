...
/*
  ...
  v_20251215.00 FILE VIEW を実行すると動作が異常になる -> 双方向リストの使い方が間違っていたため修正
*/
// **************************************************************************************************
...
struct FILEPOSLIST *getFilePosNode(struct FILEPOSLIST *list, unsigned long value) {
  struct FILEPOSLIST *node = list;
...
unsigned long getFileNextPos(struct FILEPOSLIST *list, File file, unsigned long filePos) {
  struct FILEPOSLIST *node = list;
  unsigned long fileNextPos;
  
  node = getFilePosNode(list, filePos);
  if (node == NULL) {
    fileNextPos = printFileChar(file, filePos, false);
    addFilePosNode(list, fileNextPos);
  } else {
    fileNextPos = getFilePosNode(list, filePos)->next;
  }
  return fileNextPos;
}

void deleteFilePosList(struct FILEPOSLIST *list) {
  struct FILEPOSLIST *prev_node = list->next;
  struct FILEPOSLIST *node;
