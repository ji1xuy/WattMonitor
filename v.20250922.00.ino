#define VER "Program version 20250922.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
void printFileName(int fileNum, int x, int y) {
  ...
  char name[3 + MAX_INDENT + 1 + MAX_FILENAME_LEN];
  char* indent[MAX_INDENT + 1] = {"", " ", "  ", "   ", "    "};
  char buf[BUFFER_LEN_FULL];
  
  if (fileNum < numberOfFiles) {
    sprintf(name, "%03d%s %s", fileNameList[fileNum]->num, indent[min(fileNameList[fileNum]->level, MAX_INDENT)], fileNameList[fileNum]->name);
...
void printFileNameList(int fileNum) {
...
void printFileDeletionFailedWinodow(int numberOfDeletionFailedFiles) {
  ...
  myGLCD.print(F("SEE EVENT.CSV FOR DETAILS"), 24, 104);
