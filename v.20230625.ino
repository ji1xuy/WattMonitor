#define VER "Program version 20230625"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  2023/6/6 memorysaver.h 修正するとフラッシュメモリの使用量は減る(88%)が
  　動作が不安定(30分値途中から記録されず。LCDの表示が異常になる。)になるため修正しない
...
enum {OFF, ON};
enum {MODE_BLANK, MODE_CLOCK}; //SCREEN OFFボタンを押した時のモードの選択
//時刻を取得するサーバの選択
enum {
  PROVIDER_WORLD_TIME_API,
  PROVIDER_NICT //サーバ停止のため使用不可
};
// 汎用の文字列格納用バッファのサイズ
...
Metro updateScreenInterval = Metro(2000); // 画面表示更新間隔(ms) 2000=2秒
int screenOffTimer; // 画面自動消灯用加算タイマー
int screenOffTimerOnBattery; // 停電時画面自動消灯用加算タイマー
int popupWindowOffTimer; // popupWindow自動キャンセル用減算タイマー
const int SCREENOFFTIMER_START = 0;
const int SCREENOFFTIMERONBATTERY_RESET = -2;
const int SCREENOFFTIMERONBATTERY_STOP  = -1;
const int SCREENOFFTIMERONBATTERY_START  = 0;
const int SCREENOFFTIMERONBATTERY_END  = 3;
const int POPUPWINDOWOFFTIMER_START = 3;
const int POPUPWINDOWOFFTIMER_END = 0;
...
enum {
  TIMER1_STS_STOP,     //計測停止
  TIMER1_STS_SENSOR1,  //電流センサー No.1のサンプリング
  TIMER1_STS_SENSOR2,  //電流センサー No.2のサンプリング
  TIMER1_STS_SENSOR3,  //電流センサー No.3のサンプリング
  TIMER1_STS_CALCULATE //電力の計算
};
...
void syncTimeOnceADay(void) {
  ...
  if ((existsFlashAir == true) && (eepromAutoTimeSync == ON)) {
    ...
    if ((t.date != syncDay) && (t.hr == eepromTimeSyncHour) && (t.min == eepromTimeSyncMin)) {
      ...
      if (syncTime() == true) {
        sprintf(event, "[%04lX]Time synchronized (automatically)", nextSequenceId - 1);
        saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
      } else {
        sprintf(event, "[%04lX][Time synchronization failed (automatically)", nextSequenceId - 1);
        saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
      }
      initPrevClockValues();
...
void timer1(void) {
  switch (timer1_status) {
    case TIMER1_STS_STOP:
      break;
    case TIMER1_STS_SENSOR1:
      timer1_status = TIMER1_STS_SENSOR2;
      sampleFromSensor(1);
      break;
    case TIMER1_STS_SENSOR2:
      timer1_status = TIMER1_STS_SENSOR3;
      sampleFromSensor(2);
      break;
    case TIMER1_STS_SENSOR3:
      timer1_status = TIMER1_STS_CALCULATE;
      sampleFromSensor(3);
      break;
    case TIMER1_STS_CALCULATE:
      timer1_status = TIMER1_STS_SENSOR1;
...
int getButtonNum(void) {
  ...
  if (myTouch.dataAvailable()) {
    ...
    x = myTouch.getX();
    y = myTouch.getY();
    if (isButtonPressed(BTN_RESET, x, y) == true) {
      return BTN_RESET;
    }
    if (isButtonPressed(BTN_DATE_TIME, x, y) == true) {
      return BTN_DATE_TIME;
    }
    if (isButtonPressed(BTN_CONFIG, x, y) == true) {
      return BTN_CONFIG;
    }
    if (isButtonPressed(BTN_SD_CARD, x, y) == true) {
      return BTN_SD_CARD;
    }
    if (isButtonPressed(BTN_SCREEN_OFF, x, y) == true) {
...
void countupTimer(void) {
  if (offTimerInterval.check() == 1) {
    if (hasBlackout == true) { //停電中
      //screenOffTimerOnBatteryを1分毎に増加させる
      if (screenOffTimerOnBattery >= SCREENOFFTIMERONBATTERY_START) {
        screenOffTimerOnBattery++;
      }
    } else {
      if ((eepromScreenOffTimer != NEVER_SCREEN_OFF) && (currScreenMode != SCREEN_BLANK)) {
        screenOffTimer++;
      }
    }
  }
...
void setScreenMode(void) {
  if ((currScreenMode == SCREEN_BLANK) || (currScreenMode == SCREEN_CLOCK)) {
    // SCREEN_BLANK(またはSCREEN_CLOCK)状態で
    if ((hasBlackout == true) && (screenOffTimerOnBattery == SCREENOFFTIMERONBATTERY_RESET)) {
      // 停電が発生した場合
      currScreenMode = printMainWindow(SCREEN_ON);
    }
    if ((hasBlackout == false) && (screenOffTimerOnBattery != SCREENOFFTIMERONBATTERY_RESET)) {
      // 復電した場合
      currScreenMode = printMainWindow(SCREEN_ON);
    }
  } else {
    if (hasBlackout == true) {
      ...
      // 通電中
      if ((eepromScreenOffTimer != NEVER_SCREEN_OFF) && (screenOffTimer >= eepromScreenOffTimer)) {
        // 最後の操作から一定時間が経過したら表示を消す
        if (eepromScreenOffMode == ON) {
          currScreenMode = SCREEN_CLOCK;
        } else {
          currScreenMode = SCREEN_BLANK;
        }
...
int printMainWindow(int screenMode) {
  ...
  printElapsedDays();
  printResetTime();
  screenOffTimerOnBattery = SCREENOFFTIMERONBATTERY_RESET;
  return screenMode;
}
...
void printBlackout(void) {
  ...
  if ((hasBlackout == false) && (screenOffTimerOnBattery != SCREENOFFTIMERONBATTERY_RESET)) {
    beep(BEEP_OFF);
    screenOffTimerOnBattery = SCREENOFFTIMERONBATTERY_RESET;
    if (currScreenMode == SCREEN_BLANK) {
      currScreenMode = printMainWindow(SCREEN_ON);
    }
    //screenOffTimer = SCREENOFFTIMER_START;
    offTimerInterval.reset();
  ...
  if (hasBlackout == true) {
    ...
    if (screenOffTimerOnBattery < SCREENOFFTIMERONBATTERY_END) {
      //SCREENOFFTIMERONBATTERY_END分未満はビープ音を鳴らす
      beep(BEEP_800MS);
    } else { //3分になったらビープ音を止めてSCREENOFF
      screenOffTimerOnBattery = SCREENOFFTIMERONBATTERY_STOP;
...
void reset(void) {
  ...
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_RST_SYSTEM_RESET, STS_NORMAL);
  printButtonEx(BTN_RST_OK, STS_NORMAL);
  printButtonEx(BTN_RST_CANCEL, STS_NORMAL);
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      y = myTouch.getY();
      // SYSTEM RESET
      if (isButtonPressed(BTN_RST_SYSTEM_RESET, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_RST_SYSTEM_RESET, STS_INVERSE);
        systemReset();
        break;
      }
      // OK
      if (isButtonPressed(BTN_RST_OK, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_RST_OK, STS_INVERSE);
        saveIntegratedValuesToSdCard(SCREEN_BLANK);
        saveEventToSdCard(F("Integrated values reset (manually)"), SCREEN_BLANK);
        break;
      }
      // CANCEL
      if (isButtonPressed(BTN_RST_CANCEL, x, y) == true) {
        beep(BEEP_HIGH);
        break;
      }
    }
...
void systemReset(void) {
  ...
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_SR_OK, STS_NORMAL);
  printButtonEx(BTN_SR_CANCEL, STS_NORMAL);
  popupWindowOffTimer = POPUPWINDOWOFFTIMER_START;
  while (1) {
    if (hasBlackout == true) {
      break;
    ...
    if (myTouch.dataAvailable()) {
      ...
      y = myTouch.getY();
      // OK
      if (isButtonPressed(BTN_SR_OK, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_SR_OK, STS_INVERSE);
        saveIntegratedValuesToSdCard(SCREEN_BLANK);
        saveEventToSdCard(F("Integrated values reset (manually)"), SCREEN_BLANK);
        saveEventToSdCard(F("System reset"), SCREEN_BLANK);
        resetSystem();
        //ここへは到達できない
        break;
      }
      // CANCEL
      if (isButtonPressed(BTN_SR_CANCEL, x, y) == true) {
        beep(BEEP_HIGH);
        break;
      }
    }
...
void adjustDateTime(void) {
  ...
  myGLCD.setColor(VGA_LIME);
  printButtonEx(BTN_DT_UP, STS_NORMAL);
  printButtonEx(BTN_DT_MINUS, STS_NORMAL);
  printButtonEx(BTN_DT_PLUS, STS_NORMAL);
  printButtonEx(BTN_DT_DOWN, STS_NORMAL);
  printButtonEx(BTN_DT_SYNC, STS_NORMAL);
  printButtonEx(BTN_DT_OK, STS_NORMAL);
  printButtonEx(BTN_DT_CANCEL, STS_NORMAL);
  mode = TIME_YEAR;
  popupWindowOffTimer = POPUPWINDOWOFFTIMER_START;
  while (1) {
    if (hasBlackout == true) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // DOWN
      if (isButtonPressed(BTN_DT_DOWN, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DT_DOWN, STS_INVERSE);
      ...
      // UP
      if (isButtonPressed(BTN_DT_UP, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DT_UP, STS_INVERSE);
        printTempTime(t, mode, STS_NORMAL);
        mode = changeValue(mode, -1, 0, 6);
        printTempTime(t, mode, STS_INVERSE);
        printButtonEx(BTN_DT_UP, STS_NORMAL);
      ...
      // +
      if (isButtonPressed(BTN_DT_PLUS, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DT_PLUS, STS_INVERSE);
        switch (mode) {
          case TIME_YEAR:
      ...
      // -
      if (isButtonPressed(BTN_DT_MINUS, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DT_MINUS, STS_INVERSE);
        switch (mode) {
          case TIME_YEAR:
            t.yr = changeValue(t.yr, -1, 2000, 2099);
            break;
      ...
      // SYNC
      if (isButtonPressed(BTN_DT_SYNC, x, y) == true) {
        if (existsFlashAir == true) {
          beep(BEEP_HIGH);
          printButtonEx(BTN_DT_SYNC, STS_INVERSE);
          ...
          if (isSynchronized == true) {
            sprintf(event, "[%04lX]Time synchronized (manually)", nextSequenceId - 1);
            saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
          } else {
            sprintf(event, "[%04lX]Time synchronization failed (manually)", nextSequenceId - 1);
            saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
          }
          delay(3000);
        }
      ...
      // OK
      if (isButtonPressed(BTN_DT_OK, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_DT_OK, STS_INVERSE);
        setClockTime(t);
        break;
      }
      // CANCEL
      if (isButtonPressed(BTN_DT_CANCEL, x, y) == true) {
        beep(BEEP_HIGH);
        break;
      }
    }
...
void config(void) {
  int x, y;
  char buf[BUFFER_LEN_HALF];
  boolean isLoadDefaultButtonPressed; //LOAD DEFAULTボタンを押した時にtrueにしてループ継続要否を判定する
  const int COL_LEFT = 16;
  const int COL_WIDTH = 8;
  const int ROW_TOP = 26;
  const int ROW_HIGHT = 14;
  int index;
  //要素を行単位で入れ替えることで項目の表示順を変更可能
  ...
  for (index = 0; index < EEPROM_MODE_MAX; index++) {
    switch (mode[index]) {
      case EEPROM_AUTO_RESET_DATE:
        //Auto reset date
        y += ROW_HIGHT;
        myGLCD.print("Auto reset date", COL_LEFT, y);
      ...
      case EEPROM_VT_PERCENTAGE:
        //Voltage
        y += ROW_HIGHT;
        myGLCD.print(F("Voltage        100V"), COL_LEFT, y);
        item[EEPROM_VT_PERCENTAGE].value = eepromVtPercentage;
        item[EEPROM_VT_PERCENTAGE].format = F("%3d%%");
      ...
      case EEPROM_CT1_VOLTAGE:
        //Sensor1
        y += ROW_HIGHT;
        myGLCD.print(F("Sensor1"), COL_LEFT, y);
        item[EEPROM_CT1_VOLTAGE].value = eepromCtVoltage[1];
        item[EEPROM_CT1_VOLTAGE].format = F("%3dV");
      ...
      case EEPROM_CT1_PERCENTAGE:
        ...
        item[EEPROM_CT1_PERCENTAGE].format = F("%3d%%");
      ...
      case EEPROM_CT2_VOLTAGE:
        //Sensor2
        y += ROW_HIGHT;
        myGLCD.print(F("Sensor2"), COL_LEFT, y);
        item[EEPROM_CT2_VOLTAGE].value = eepromCtVoltage[2];
        item[EEPROM_CT2_VOLTAGE].format = F("%3dV");
      ...
      case EEPROM_CT2_PERCENTAGE:
        ...
        item[EEPROM_CT2_PERCENTAGE].format = F("%3d%%");
      ...
      case EEPROM_CT3_VOLTAGE:
        //Sensor3
        y += ROW_HIGHT;
        myGLCD.print(F("Sensor3"), COL_LEFT, y);
        item[EEPROM_CT3_VOLTAGE].value = eepromCtVoltage[3];
        item[EEPROM_CT3_VOLTAGE].format = F("%3dV");
      ...
      case EEPROM_CT3_PERCENTAGE:
        ...
        item[EEPROM_CT3_PERCENTAGE].format = F("%3d%%");
      ...
      case EEPROM_BAR_GRAPH_FULLSCALE:
        //Bar graph FS
        y += ROW_HIGHT;
        myGLCD.print(F("Bar graph FS"), COL_LEFT, y);
        item[EEPROM_BAR_GRAPH_FULLSCALE].value = eepromBarGraphFullscale;
        item[EEPROM_BAR_GRAPH_FULLSCALE].format = F("%2dkW");
      ...
      case EEPROM_SCREEN_OFF_TIMER:
        //Screen off timer
        y += ROW_HIGHT;
        myGLCD.print(F("Screen off timer"), COL_LEFT, y);
        item[EEPROM_SCREEN_OFF_TIMER].value = eepromScreenOffTimer;
        item[EEPROM_SCREEN_OFF_TIMER].format = F("%02d");
        item[EEPROM_SCREEN_OFF_TIMER].x = COL_LEFT + COL_WIDTH * 22;
        item[EEPROM_SCREEN_OFF_TIMER].y = y;
        break;
      case EEPROM_SCREEN_OFF_MODE:
        //Screen off mode
        y += ROW_HIGHT;
        myGLCD.print(F("Screen off mode"), COL_LEFT, y);
        item[EEPROM_SCREEN_OFF_MODE].value = eepromScreenOffMode;
        item[EEPROM_SCREEN_OFF_MODE].format = F("%5s");
      ...
      case EEPROM_SOUND:
        //Sound
        y += ROW_HIGHT;
        myGLCD.print(F("Sound"), COL_LEFT, y);
        item[EEPROM_SOUND].value = eepromSound;
        item[EEPROM_SOUND].format = F("%3s");
      ...
      case EEPROM_AUTO_TIME_SYNC:
        //Auto time sync
        y += ROW_HIGHT;
        myGLCD.print(F("Auto time sync       :"), COL_LEFT, y); //':'は時と分の間のコロン
        item[EEPROM_AUTO_TIME_SYNC].value = eepromAutoTimeSync;
        item[EEPROM_AUTO_TIME_SYNC].format = F("%3s");
      ...
      case EEPROM_TIME_SYNC_HOUR:
        ...
        item[EEPROM_TIME_SYNC_HOUR].format = F("%02d");
      ...
      case EEPROM_TIME_SYNC_MIN:
        ...
        item[EEPROM_TIME_SYNC_MIN].format = F("%02d");
  ...
  //ボタンを表示
  printButtonEx(BTN_CFG_UP, STS_NORMAL);
  printButtonEx(BTN_CFG_MINUS, STS_NORMAL);
  printButtonEx(BTN_CFG_PLUS, STS_NORMAL);
  printButtonEx(BTN_CFG_DOWN, STS_NORMAL);
  printButtonEx(BTN_CFG_LOAD_DEFAULT, STS_NORMAL);
  printButtonEx(BTN_CFG_OK, STS_NORMAL);
  printButtonEx(BTN_CFG_CANCEL, STS_NORMAL);
  //タッチスクリーンを読み取り
  popupWindowOffTimer = POPUPWINDOWOFFTIMER_START;
  while (1) {
    isLoadDefaultButtonPressed = false;
    while (1) {
      if (hasBlackout == true) {
        break;
      }
      ...
      if (myTouch.dataAvailable()) {
        ...
        y = myTouch.getY();
        // DOWN
        if (isButtonPressed(BTN_CFG_DOWN, x, y) == true) {
          beep(BEEP_HIGH);
          printButtonEx(BTN_CFG_DOWN, STS_INVERSE);
        ...
        // UP
        if (isButtonPressed(BTN_CFG_UP, x, y) == true) {
          beep(BEEP_HIGH);
          printButtonEx(BTN_CFG_UP, STS_INVERSE);
          printItem(item, mode[index], STS_NORMAL);
          index = changeValue(index, -1, 0, EEPROM_MODE_MAX - 1);
          printItem(item, mode[index], STS_INVERSE);
          printButtonEx(BTN_CFG_UP, STS_NORMAL);
        ...
        // +
        if (isButtonPressed(BTN_CFG_PLUS, x, y) == true) {
          beep(BEEP_HIGH);
          printButtonEx(BTN_CFG_PLUS, STS_INVERSE);
          switch (mode[index]) {
            case EEPROM_SCREEN_OFF_TIMER:
            ...
            case EEPROM_CT2_PERCENTAGE:
            case EEPROM_CT3_PERCENTAGE:
              item[mode[index]].value = changeValue(item[mode[index]].value, 1, 1, 255);
              break;
            case EEPROM_SOUND:
            case EEPROM_AUTO_TIME_SYNC:
        ...
        // -
        if (isButtonPressed(BTN_CFG_MINUS, x, y) == true) {
          beep(BEEP_HIGH);
          printButtonEx(BTN_CFG_MINUS, STS_INVERSE);
          switch (mode[index]) {
            case EEPROM_SCREEN_OFF_TIMER:
              item[mode[index]].value = changeValue(item[mode[index]].value, -1, 0, 99);
              break;
        ...
        // LOAD DEFAULT
        if (isButtonPressed(BTN_CFG_LOAD_DEFAULT, x, y) == true) {
          beep(BEEP_HIGH);
          printButtonEx(BTN_CFG_LOAD_DEFAULT, STS_INVERSE);
          printItem(item, mode[index], STS_NORMAL);
          ...
          printItems(item, mode[index]);
          printButtonEx(BTN_CFG_LOAD_DEFAULT, STS_NORMAL);
          isLoadDefaultButtonPressed = true;
          break;
        }
        // OK
        if (isButtonPressed(BTN_CFG_OK, x, y) == true) {
          beep(BEEP_HIGH);
          printButtonEx(BTN_CFG_OK, STS_INVERSE);
          eepromScreenOffTimer = item[EEPROM_SCREEN_OFF_TIMER].value;
          eepromAutoResetDate = item[EEPROM_AUTO_RESET_DATE].value;
          eepromCtVoltage[1] = item[EEPROM_CT1_VOLTAGE].value;
          eepromCtVoltage[2] = item[EEPROM_CT2_VOLTAGE].value;
          eepromCtVoltage[3] = item[EEPROM_CT3_VOLTAGE].value;
          eepromVtPercentage = item[EEPROM_VT_PERCENTAGE].value;
          eepromCtPercentage[1] = item[EEPROM_CT1_PERCENTAGE].value;
          eepromCtPercentage[2] = item[EEPROM_CT2_PERCENTAGE].value;
          eepromCtPercentage[3] = item[EEPROM_CT3_PERCENTAGE].value;
          eepromSound = item[EEPROM_SOUND].value;
          eepromScreenOffMode = item[EEPROM_SCREEN_OFF_MODE].value;
          eepromAutoTimeSync = item[EEPROM_AUTO_TIME_SYNC].value;
          eepromTimeSyncHour = item[EEPROM_TIME_SYNC_HOUR].value;
          eepromTimeSyncMin = item[EEPROM_TIME_SYNC_MIN].value;
          eepromBarGraphFullscale = item[EEPROM_BAR_GRAPH_FULLSCALE].value;
          EEPROM.write(EEPROM_SCREEN_OFF_TIMER, eepromScreenOffTimer);
          ...
          EEPROM.write(EEPROM_SCREEN_OFF_MODE, eepromScreenOffMode);
          EEPROM.write(EEPROM_AUTO_TIME_SYNC, eepromAutoTimeSync);
        ...
        // CANCEL
        if (isButtonPressed(BTN_CFG_CANCEL, x, y) == true) {
          beep(BEEP_HIGH);
          break;
        }
      }
    }
    if (isLoadDefaultButtonPressed == true) {
      //外側のwhileループの先頭に戻りCONFIG処理継続
      continue;
    } else {
      //外側のwhileループを抜けてCONFIG処理終了
      break;
    }
  }
  popupWindowOffTimer = POPUPWINDOWOFFTIMER_END;
...
void printItem(struct ITEM *item, int mode, int sts) {
  ...
  item[mode].format.toCharArray(format, FORMAT_LEN);
  switch (mode) {
    case EEPROM_SCREEN_OFF_MODE:
      sprintf(buf, format, (item[mode].value == MODE_CLOCK) ? "Clock" : "Blank");
      break;
    case EEPROM_SOUND:
    case EEPROM_AUTO_TIME_SYNC:
      sprintf(buf, format, (item[mode].value == ON) ? "ON" : "OFF");
      break;
    default:
      sprintf(buf, format, item[mode].value);
      break;
  }
...
void sdCard(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // PAGE UP
      if (isButtonPressed(BTN_SD_PAGE_UP, x, y) == true) {
        if (numberOfFiles <= 10) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SD_PAGE_UP, STS_INVERSE);
        ...
        printButtonEx(BTN_SD_PAGE_UP, STS_NORMAL);
        continue;
      }
      // CURSOR UP
      if (isButtonPressed(BTN_SD_CURSOR_UP, x, y) == true) {
        if (numberOfFiles < 1) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SD_CURSOR_UP, STS_INVERSE);
        if (fileNum > 0) {
        ...
        printButtonEx(BTN_SD_CURSOR_UP, STS_NORMAL);
        continue;
      }
      // CURSOR DOWN
      if (isButtonPressed(BTN_SD_CURSOR_DOWN, x, y) == true) {
        if (numberOfFiles < 1) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SD_CURSOR_DOWN, STS_INVERSE);
        ...
        printButtonEx(BTN_SD_CURSOR_DOWN, STS_NORMAL);
        continue;
      }
      // PAGE DOWN
      if (isButtonPressed(BTN_SD_PAGE_DOWN, x, y) == true) {
        if (numberOfFiles <= 10) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SD_PAGE_DOWN, STS_INVERSE);
        currPage = (int)(fileNum / 10.0) + 1;
        nextPage = (int)((numberOfFiles - 1) / 10.0) + 1;
        ...
        printFileList(fileNum);
        printButtonEx(BTN_SD_PAGE_DOWN, STS_NORMAL);
        continue;

      }
      // DELETE ALL
      if (isButtonPressed(BTN_SD_DELETE_ALL, x, y) == true) {
        beep(BEEP_HIGH);
        printButtonEx(BTN_SD_DELETE_ALL, STS_INVERSE);
      ...
      // DELETE
      if (isButtonPressed(BTN_SD_DELETE, x, y) == true) {
        ...
        beep(BEEP_HIGH);
        myGLCD.setColor(VGA_FUCHSIA);
        printButtonEx(BTN_SD_DELETE, STS_INVERSE);
      ...
      // CANCEL
      if (isButtonPressed(BTN_SD_CANCEL, x, y) == true) {
        beep(BEEP_HIGH);
        break;
...
boolean deleteFile(int fileNum) {
  ...
  printButtonEx(BTN_DF_OK, STS_NORMAL);
  printButtonEx(BTN_DF_CANCEL, STS_NORMAL);
  popupWindowOffTimer = POPUPWINDOWOFFTIMER_START;
  while (1) {
    if (hasBlackout == true) {
      break;
    ...
    if (myTouch.dataAvailable()) {
      ...
      y = myTouch.getY();
      // OK
      if (isButtonPressed(BTN_DF_OK, x, y) == true) {
        beep(BEEP_HIGH);
        //DELETE ALL
        if (fileNum == FILENUM_DELETE_ALL) {
      ...
      // CANCEL
      if (isButtonPressed(BTN_DF_CANCEL, x, y) == true) {
        beep(BEEP_HIGH);
        return false;
      }
    }
...
void printFileListWindow(int fileNum) {
  ...
  printButtonEx(BTN_SD_CURSOR_DOWN, STS_NORMAL);
  printButtonEx(BTN_SD_PAGE_DOWN, STS_NORMAL);
  printButtonEx(BTN_SD_DELETE_ALL, STS_NORMAL);
  printButtonEx(BTN_SD_DELETE, STS_NORMAL);
  printButtonEx(BTN_SD_CANCEL, STS_NORMAL);
}
...
void setup() {
  ...
  value = EEPROM.read(EEPROM_SCREEN_OFF_TIMER); if ((value >= 0) && (value <= 99)) eepromScreenOffTimer = value;
  ...
  value = EEPROM.read(EEPROM_SCREEN_OFF_MODE); if ((value == MODE_CLOCK) || (value == MODE_BLANK)) eepromScreenOffMode = value;
  value = EEPROM.read(EEPROM_AUTO_TIME_SYNC); if ((value == OFF) || (value == ON)) eepromAutoTimeSync = value;
  ...
  // EEPROMへはデータを-1したうえで書き込み、 EEPROM読み出し時は+1した値をCONFIGデータとして使用する
  // EEPROMデータ| CONFIGデータ
  // ------------+----------------------------
  // 0～254      | 1～255のデータを表す
  // 255         | データ消去状態を表す
  ...
  if (getCardStatus() == true) {
    ...
    isSynchronized = syncTime();
    if (isSynchronized == true) {
      sprintf(event, "[%04lX]Time synchronized (setup)", nextSequenceId - 1);
      saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
    } else {
      sprintf(event, "[%04lX]Time synchronization failed (setup)", nextSequenceId - 1);
      saveEventToSdCard(event, currScreenMode, PRINTMODE_LINE);
    }
  }
  delay(3000);
