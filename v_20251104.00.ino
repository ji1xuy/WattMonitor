#define VER "Program version 20251104.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
void printFileViewWindow(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_VIEWFILE_CLOSE, STS_NORMAL);
  myGLCD.setColor(VGA_GREEN);
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // CLOSE
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_CLOSE, x, y)) {
        beep(BEEP_HIGH);
        break;
      }
    }
