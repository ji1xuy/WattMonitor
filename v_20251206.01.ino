#define VER "Program version 20251206.01"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  v_20251206.01 clearScreen()関数 delay()をやめて画面消去中は割り込み禁止に変更
*/
// **************************************************************************************************
...
const struct BTN btn[] = {
  ...
  // SD CARD
  { "PAGE", "UP", 253, 7, 313, 35 },
  { "CURSOR", "UP", 253, 44, 313, 72 },
  { "CURSOR", "DOWN", 253, 81, 313, 109 },
  { "PAGE", "DOWN", 253, 118, 313, 146 },
...
char flashAirIP[16];
//デフォルト引数のある関数の宣言
bool syncTimeWithServer(bool systemReset = false);
bool onOffFlashAir(int sec = 10);
bool initFlashAir(bool systemReset = false);
bool resetFlashAir(int sec = 10, bool systemReset = false);
...
bool syncTimeWithServer(bool systemReset) {
  char buf[BUFFER_LEN_HALF];
  ...
  for (int i = 1; i <= MAX_REQUEST_TIMES; i++) {
    ...
    if (i == 1) {
      ...
      delay(3000);
      if (resetFlashAir(systemReset) == false) {
...
bool initFlashAir(bool systemReset) {
  // Initialize SD card.
...
bool resetFlashAir(int sec, bool systemReset) {
  if (existsFlashAir == false) {
    return false;
  }
  //clearScreen();
  ...
  if (initFlashAir(systemReset) == true) {
...
byte writeValueToEEPROM(byte value, struct ITEM item, int mode) {
  ...
  //while (timer1_status != TIMER1_STS_CALCULATED) {
