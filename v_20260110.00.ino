#define VER "Program version 20260110.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  最大130048バイトのフラッシュメモリのうち、スケッチが129442バイト（99%）を使っています。
  グローバル変数は5491バイトのRAMを使用しています。
*/
// **************************************************************************************************
...
const int FLASHAIR_RESET_REQUESTED_MARK = 17;
...
// 文字列操作関数
//デフォルト引数のある関数の宣言
//strの中にkeyworddが含まれる行が見つかったら、その行をlineに格納する
...
void printSyncTimeWithServerInfo(int screenMode) {
  ...
  myGLCD.setFont(SmallFont);
  sprintf(buf, "T%s", syncTimeWithServerInfo);
  ...
  while (true) {
    ...
    if (isScreenTouched()) {
      ...
      if (isButtonPressed(BTN_SDCARD_CURSORUP, x, y)) {
        ...
        if (fileNum > 0) {
          moveCursol(fileNum, fileNum - 1, indentOffset);
          fileNum--;
        } else {
          moveCursol(fileNum, numberOfFiles - 1, indentOffset);
      ...
      if (isButtonPressed(BTN_SDCARD_CURSORDOWN, x, y)) {
        ...
        if ((fileNum + 1) < numberOfFiles) {
          ...
          moveCursol(fileNum, 0, indentOffset);
          fileNum = 0;
        }
...
void moveCursol(int currFileNum, int nextFileNum, int indentOffeet) {
  int currPage, nextPage;
  int currRow, nextRow;
  const int X_LEFT = 8;
  const int Y_TOP = 14;
