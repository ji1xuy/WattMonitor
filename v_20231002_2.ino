#define VER "Program version 20231002_2"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  2023/9/27 saveWattHourTimeperiod()関数他をupdateScreenInterval.check()==1の時に実行するように場所移動
    ...
    updateScreenInterval.check()==1の時に実行するように場所移動
  2023/9/28 修正したmemorysaver.hをいつのまにか使っていたため、修正前のmemorysaver.hに戻す
  2023/9/29 countupTimer()関数だけ元の場所に戻す
  2023/9/30 細かい修正
  2023/10/1 修正したmemorysaver.hを使うと2023/6/6、2023/9/27の不具合が発生するが原因ではない様子。
    memorysaver.hを調べるも原因は見つからず。
    修正したmemorysaver.hでコンパイルすると、CONFIGを実行しない限り不具合発生せず。
    2023/5/27の修正によりCONFIG実行後メモリの空きが減るようになって不具合が発生するのかもしれない。原因はconfig()関数のコード？
    修正していないmemorysaver.hのほうがプログラムメモリ使用量が多いのにCONFIGを実行してもなぜ安定しているのかは不明。
    config()関数内のitem[]変数の定義(struct ITEM item[EEPROM_MODE_MAX];)をグローバルに移動。修正したmemorysaver.hを使用
  2023/10/1 syncTime()関数 #define,#ifdef,#endif文でどちらかの関数を呼び出すように修正、他 細かい修正
    スケッチが114906バイト（88%）、グローバル変数は6040バイト
...
enum {
  CLOCK_DS1302,   // DS1302をメインの時計として使用
  CLOCK_INTERNAL  // 内蔵時計機能をメインの時計として使用
};
...
boolean syncTime(void) {
  ...
  if (isTimeSynchronized == false) {
    ...
    isTimeSynchronized = strtokEx(httpResponse, "datetime: ", datetime, sizeof(datetime));
    if (isTimeSynchronized == false) {
...
boolean syncTime(void) {
  ...
  if (isTimeSynchronized == false) {
    ...
    isTimeSynchronized = strtokEx(httpResponse, " JST", datetime, sizeof(datetime));
    if (isTimeSynchronized == false) {
      printEx(F("\nTime information not found"));
    } else {
  ...
  for (index = 0; index < EEPROM_MODE_MAX; index++) {
    switch (mode[index]) {
      ...
      case EEPROM_SCREEN_OFF_TIMER:
        ...
        item[EEPROM_SCREEN_OFF_TIMER].format = "%02d";
