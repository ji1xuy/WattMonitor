#define VER "Program version 20250312"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  2025/3/12 World Clock APIでレスポンスのデータを整形する処理を修正
  2025/3/12 iSDIO_waitHttpResponse()関数 "Process Timed Out"の文字列を"Process timed out"に修正、
*/
// **************************************************************************************************
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    if (iSDIO_buf[addr] & 0x01) {
      ...
      switch (iSDIO_buf[addr + 8]) {
        case 0x00: printlnEx(); printEx(F("Initial")); break;
        case 0x01: printlnEx(); printEx(F("Command Processing")); break;
        case 0x02: printlnEx(); printEx(F("Command Rejected")); break;
        case 0x03: printlnEx(); printEx(F("Process Succeeded")); break;
        case 0x04: printlnEx(); printEx(F("Process Terminated")); break;
        default:
          printlnEx();
...
boolean syncTime(void) {
  ...
  const char months[][4] = {"Jan", "Feb", "Mar", "Apl", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
  ...
  if (isTimeSynchronized == false) {
    ...
    if (isTimeSynchronized == false) {
      ...
      if (isTimeSynchronized == false) {
        ...
        sscanf(datetime, "Date: %[^,], %02d %s %04d %02d:%02d:%02d GMT",
...
boolean syncTime(void) {
  ...
  for (numberOfTimes = 1; numberOfTimes <= MAX_NUMBER_OF_TIMES; numberOfTimes++) {
    ...
    if (iSDIO_sendHttpRequest(nextSequenceId, PROTOCOL_HTTPS)
        && iSDIO_waitHttpResponse(nextSequenceId)) {
      if (iSDIO_readHttpResponse(httpResponse, sizeof(httpResponse))) {
        isTimeSynchronized = true;
        nextSequenceId++;
...
void setup() {
  ...
  if (sdSts != SD_SUCCESS) {
    printEx(F("\n"));
    printEx(sdStsMsg[sdSts]);
    printEx(F("\nIf the screen touched, the system reset"));
    while (getButtonNum() == BTN_NOT_PRESSED);
    resetSystem();
    //ここへは到達しない
    abort();
