#define VER "Program version 20260102.01"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
const int MAX_EVENT_LEN = 101;  //100文字 + '\0' = 101
...
void printFileViewWindow(int fileNum) {
  ...
  //パスが長い場合に短縮する
  char path[33] = "...";
  int pathLen = strlen(fileNameList[fileNum]->path);
  if (pathLen > 32) {
    strcat(path, fileNameList[fileNum]->path + pathLen - 29);
  } else {
    strcpy(path, fileNameList[fileNum]->path);
  }
...
void printFileName(int fileNum, int x, int y) {
  ...
  if (fileNum < numberOfFiles) {
    ...
    if ( fileNameList[fileNum]->attribute == 'D') {
      ...
      if (fileNameList[fileNum]->size < 100000000LU) {
        sprintf(size, "%8lu B", fileNameList[fileNum]->size);
      } else if (fileNameList[fileNum]->size < 102400000000LU) {
        sprintf(size, "%8lukB", fileNameList[fileNum]->size / 1024LU);
      } else {
        sprintf(size, "%8luMB", fileNameList[fileNum]->size / 1048576LU);
      }
    }
...
bool deleteFile(int fileNum) {
  ...
  if (strlen(fileNameList[fileNum]->path) > 79) {
    sprintf(event, "SDCARD: \'%s\' Pathname too long", fileNameList[fileNum]->name);
    saveEventToSdCard(event);
    return false;
  }
