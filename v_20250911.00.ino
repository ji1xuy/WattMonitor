#define VER "Program version 20250911.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  v_20250811.01 sdCard()関数 fileList変数のdelete処理を追加 
  v_20250812.00 sdCard()関数 fileList変数のdelete処理を修正 
  v_20250812.01 deleteFileList()関数 delete[] fileList;を追加　
      ...
      if (isButtonPressed(BTN_DELETE_FILE_OK, x, y)) {
        ...
        if (fileNum == FILENUM_DELETE_ALL) {
          for (int i = 1; i < numberOfFiles; i++) {
            if ((fileList[fileNum]->attribute == ' ') &&
              (strstr(fileList[i]->name, ".CSV") != NULL) && (strcmp(fileList[i]->name, "EVENT.CSV") != 0)) {
              if (SD.remove(fileList[i]->name) == true) {
                 fileNum = 0;
              }
            }
          }
          ...
          if (fileList[fileNum]->attribute == ' ') {
            if (SD.remove(fileList[fileNum]->name) == true) {
              fileNum--;
            }
          }
        }
...
bool deleteFile(char* fileName) {
  ...
  if (SD.remove(fileName) == true) {
    sprintf(event, "SDCARD: File \"%s\" deleted", fileName);
    saveEventToSdCard(event);
    return true;
  }
  return false;
