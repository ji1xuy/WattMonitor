#define VER "Program version 20250920.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
...
const int MAX_PATH_NAME_LEN = 50;    // フォルダ名
...
void getFileName(File dir, String path, int level) {
  ...
  while (true) {
    ...
    fullPath.toCharArray(fileNameList[numberOfFiles]->path, MAX_PATH_NAME_LEN);
...
void printFileName(int fileNum, int x, int y) {
  char name[7 + MAX_FILE_NAME_LEN];
  char* indent[5] = {"", " ", "  ", "   ", "    "};
  char buf[BUFFER_LEN_FULL];
  
  if (fileNum < numberOfFiles) {
    //sprintf(name, "%s%03d %s", indent[min(fileNameList[fileNum]->level, 3)], fileNameList[fileNum]->num, fileNameList[fileNum]->name);
    sprintf(name, "%03d%s %s", fileNameList[fileNum]->num, indent[min(fileNameList[fileNum]->level, 4)], fileNameList[fileNum]->name);
