...
void sdCard(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // DELETE ALL
      if (isButtonPressed(BTN_SDCARD_DELETEFILE_ALLCSV, x, y)) {
        beep(BEEP_LOW);
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_DELETEFILE_ALLCSV, STS_INVERSE);
...
void printFileNameListWindow(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_CURSORDOWN, STS_NORMAL);
  printButtonEx(BTN_SDCARD_PAGEDOWN, STS_NORMAL);
  printButtonEx(BTN_SDCARD_DELETEFILE_ALLCSV, STS_NORMAL);
