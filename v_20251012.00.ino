...
enum {
  ...
  BTN_CONFIG_CANCEL,
  BTN_SDCARD_PAGE_UP,
  BTN_SDCARD_CURSOR_UP,
  BTN_SDCARD_CURSOR_DOWN,
  BTN_SDCARD_PAGE_DOWN,
  BTN_SDCARD_DELETE_ALL,
  BTN_SDCARD_DELETE_FILE,
  BTN_SDCARD_VIEW_FILE,
  BTN_SDCARD_CANCEL,
  BTN_SDCARD_DELETEFILE_OK,
  BTN_SDCARD_DELETEFILE_CANCEL,
  BTN_SDCARD_DELETEFILE_CLOSE,
  BTN_SDCARD_VIEWFILE_PAGE_UP,
  BTN_SDCARD_VIEWFILE_PAGE_DOWN,
  BTN_SDCARD_VIEWFILE_PAGE_END,
...
void sdCard(void) {
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // PAGE UP
      if (isButtonPressed(BTN_SDCARD_PAGE_UP, x, y)) {
        if (numberOfFiles <= 10) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_PAGE_UP, STS_INVERSE);
        ...
        printButtonEx(BTN_SDCARD_PAGE_UP, STS_NORMAL);
        continue;
      }
      // CURSOR UP
      if (isButtonPressed(BTN_SDCARD_CURSOR_UP, x, y)) {
        if (numberOfFiles < 1) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_CURSOR_UP, STS_INVERSE);
        if (fileNum > 0) {
        ...
        printButtonEx(BTN_SDCARD_CURSOR_UP, STS_NORMAL);
        continue;
      }
      // CURSOR DOWN
      if (isButtonPressed(BTN_SDCARD_CURSOR_DOWN, x, y)) {
        if (numberOfFiles < 1) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_CURSOR_DOWN, STS_INVERSE);
        ...
        printButtonEx(BTN_SDCARD_CURSOR_DOWN, STS_NORMAL);
        continue;
      }
      // PAGE DOWN
      if (isButtonPressed(BTN_SDCARD_PAGE_DOWN, x, y)) {
        if (numberOfFiles <= 10) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_PAGE_DOWN, STS_INVERSE);
        currPage = (int)(fileNum / 10.0) + 1;
        nextPage = (int)((numberOfFiles - 1) / 10.0) + 1;
        ...
        printFileNameList(fileNum);
        printButtonEx(BTN_SDCARD_PAGE_DOWN, STS_NORMAL);
        continue;
      }

      // DELETE ALL
      if (isButtonPressed(BTN_SDCARD_DELETE_ALL, x, y)) {
        beep(BEEP_LOW);
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_DELETE_ALL, STS_INVERSE);
        if (deleteFiles(FILENUM_DELETE_ALL) >= 1) {
          fileNum = 0;
        }
        printFileNameListWindow(fileNum);
        continue;
      ...
      // DELETE FILE
      if (isButtonPressed(BTN_SDCARD_DELETE_FILE, x, y)) {
        ...
        beep(BEEP_HIGH);
        myGLCD.setColor(VGA_FUCHSIA);
        printButtonEx(BTN_SDCARD_DELETE_FILE, STS_INVERSE);
...
void printFileViewWindow(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGE_UP, STS_NORMAL);
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGE_DOWN, STS_NORMAL);
  printButtonEx(BTN_SDCARD_VIEWFILE_PAGE_END, STS_NORMAL);
  ...
  while (1) {
    ...
    if (myTouch.dataAvailable()) {
      ...
      // PAGE UP
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGE_UP, x, y)) {
        if (filePos[page] == 0) {
          beep(BEEP_LOW);
          continue;
        }
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGE_UP, STS_INVERSE);
        page--;
      ...
      // PAGE DOWN
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGE_DOWN, x, y)) {
        ...
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGE_DOWN, STS_INVERSE);
        page++;
        filePos[page + 1]  = printFileView(file, filePos[page], page) + 1;
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGE_DOWN, STS_NORMAL);
        continue;
      ...
      // PAGE END
      if (isButtonPressed(BTN_SDCARD_VIEWFILE_PAGE_END, x, y)) {
        if (page == MAX_FILEVIEW_PAGE) {
          beep(BEEP_LOW);
          continue;
        }
        ...
        beep(BEEP_HIGH);
        printButtonEx(BTN_SDCARD_VIEWFILE_PAGE_END, STS_INVERSE);
...
void printFileNameListWindow(int fileNum) {
  ...
  printButtonEx(BTN_SDCARD_CURSOR_DOWN, STS_NORMAL);
  printButtonEx(BTN_SDCARD_PAGE_DOWN, STS_NORMAL);
  printButtonEx(BTN_SDCARD_DELETE_ALL, STS_NORMAL);
  printButtonEx(BTN_SDCARD_DELETE_FILE, STS_NORMAL);
