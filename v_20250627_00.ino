#define VER "Program version 20250627.00"
//**************************************************************************************************
/*
  電力モニター Watt_Monitor
  ■主要部品
  ...
  v.20250623.00 loop()関数を修正、FlashAirオフオンの間隔のデフォルト値を5秒から10秒に修正
...
Time getClockTime(bool interrption = NOT_IN_INTERRUPTION);
...
enum {
  TIMER1_STS_CALCULATE,   //計測・計算中
  TIMER1_STS_CALCULATED,  //計測・計算終了
...
Time getClockTime(bool interruption) {
  if (clockMode == CLOCK_DS1302) {
    if (interruption == NOT_IN_INTERRUPTION) {
...
void timer1(void) {
  Time t = getClockTime(IN_INTERRUPTION);
